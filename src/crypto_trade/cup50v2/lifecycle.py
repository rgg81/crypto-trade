...
def compile_leaderboard(
    *,
    ...
    for team_id in TEAM_IDS:
        ...
        fold_3x = min(float(cells["3"]["q"]) for cells in centre_score["fold_cost_cells"].values())
...
def atomic_release(
    ...
    *,
    ...
    path = Path(destination)
    if path.exists():
        raise FileExistsError("CUP-50 v2 leaderboard is already released")
    ...
    if sorted(published_teams) != sorted(TEAM_IDS):
        raise ValueError("leaderboard does not contain each CUP-50 v2 team exactly once")
    body = {
        "schema_version": 1,
        "namespace": "cup50v2",
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary_dir = Path(tempfile.mkdtemp(prefix=".cup50v2-release.", dir=path.parent))
    try:
