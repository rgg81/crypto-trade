"""Checksum-bound reuse and exact terminal-gap acquisition for CUP-50 v2."""
...
from crypto_trade.cup50v2.replay import _normalise_funding, decision_grid
...
def _fetch(endpoint: str, params: Mapping[str, object]) -> tuple[object, bytes]:
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "cup50v2-acquisition-v1"})
...
def acquire_terminal_gaps(
    *,
    ...
    if output.exists():
        raise FileExistsError("CUP-50 v2 acquisition destination must be new")
    ...
    if len(active) != 50:
        raise ValueError(f"terminal CUP-50 v2 membership has {len(active)} names")
    ...
    file_entries = []
    logical_by_filename = {Path(str(entry["path"])).name: name for name, entry in entries.items()}
...
def _execution_gaps(
    *,
    ...
    interval_left = normalized_funding["settlement_time"].dt.ceil("8h") - pd.Timedelta(hours=8)
    funded = {
    ...
    active_rows &= bar_frame["open_time"].ge(bar_frame["symbol"].astype(str).map(first_membership))
    active_bar_symbols = {
    ...
    for decision in decisions:
        ...
        coverage_symbols = (active | set(active_bar_symbols.get(decision, ()))) - unavailable
        missing_bars.extend(
            (decision, symbol)
...
def acquire_execution_gaps(
    *,
    ...
    if output.exists():
        raise FileExistsError("CUP-50 v2 coverage acquisition destination must be new")
    if workers < 1 or workers > 16:
        raise ValueError("acquisition workers must be in 1..16")
    ...
    def download_mark(item: tuple[tuple[str, str], set[pd.Timestamp]]) -> dict[str, object]:
        ...
        try:
            archive, checksum, digest = _download_verified_archive(url, cache_dir=download_cache)
    ...
    def download_transaction(
        ...
        rows, missing = _transaction_archive_rows(archive, symbol=symbol, required=required)
        return {
            "archive": archive,
            "checksum": checksum,
            "digest": digest,
    ...
    transaction_results.sort(key=lambda item: (str(item["symbol"]), str(item["year_month"])))
    ...
    def download_transaction_daily(
        ...
        (symbol, date), required = item
        filename = f"{symbol}-1h-{date}.zip"
        url = f"https://data.binance.vision/data/futures/um/daily/klines/{symbol}/1h/{filename}"
        archive, checksum, digest = _download_verified_archive(url, cache_dir=download_cache)
        rows, missing = _transaction_archive_rows(archive, symbol=symbol, required=required)
        if missing:
            raise ValueError(
    ...
    daily_transaction_results.sort(key=lambda item: (str(item["symbol"]), str(item["date"])))

    acquired_mark_rows = [
        row for result in [*mark_results, *daily_mark_results] for row in result["rows"]
    ]
    ...
    for symbol, decisions in funding_by_symbol.items():
        for decision in decisions:
            ...
            funding_archive_required.setdefault((symbol, settlement.strftime("%Y-%m")), set()).add(
                decision
            )
    ...
    funding_archive_results.sort(key=lambda item: (str(item["symbol"]), str(item["year_month"])))
    archive_covered = {
    ...
    for symbol, all_decisions in sorted(funding_by_symbol.items()):
        decisions = [
            decision for decision in all_decisions if (symbol, decision) not in archive_covered
        ]
        if not decisions:
            continue
        lower = min(decisions)
        ...
        covered = {
            pd.Timestamp(row["settlement_time"]).ceil("8h") - pd.Timedelta(hours=8) for row in rows
        }
        required = set(decisions)
        if not required <= covered:
            raise ValueError(f"funding REST gap remains for {symbol}: {sorted(required - covered)}")
        funding_rest_results.append({"symbol": symbol, "params": params, "raw": raw, "rows": rows})
    ...
    for result in transaction_results:
        if result["archive"] is None:
            continue
        raw_dir = output / "raw" / "coverage-archives" / "transaction_1h" / str(result["symbol"])
    ...
    for result in daily_transaction_results:
        raw_dir = (
            output / "raw" / "coverage-archives" / "transaction_1h_daily" / str(result["symbol"])
        )
        raw_dir.mkdir(parents=True, exist_ok=True)
        archive_path = raw_dir / str(result["filename"])
        archive_path.write_bytes(result["archive"])
    ...
    for result in daily_mark_results:
        if result["archive"] is None:
            continue
        raw_dir = output / "raw" / "coverage-archives" / "mark-daily" / str(result["symbol"])
        raw_dir.mkdir(parents=True, exist_ok=True)
        archive_path = raw_dir / str(result["filename"])
        archive_path.write_bytes(result["archive"])
        (raw_dir / f"{result['filename']}.CHECKSUM").write_bytes(result["checksum"])
    ...
    for result in funding_archive_results:
        if result["archive"] is None:
            continue
        raw_dir = output / "raw" / "coverage-archives" / "funding" / str(result["symbol"])
        raw_dir.mkdir(parents=True, exist_ok=True)
        archive_path = raw_dir / str(result["filename"])
        archive_path.write_bytes(result["archive"])
        (raw_dir / f"{result['filename']}.CHECKSUM").write_bytes(result["checksum"])
    ...
    body = {
        ...
        "coverage_archive_count": sum(result["archive"] is not None for result in mark_results),
