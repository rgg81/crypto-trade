"""Paper-trading projection of the exact frozen Team 12 continuous replay."""
...
from crypto_trade.team12.live_data import LiveMarketData, exact_boundary
...
def run_live_replay(
    ...
    *,
    ...
    if live_boundary not in replay.targets.index or live_boundary not in result.positions.index:
        raise RuntimeError("Team 12 replay did not produce the requested live boundary")
    if result.returns.index.max() != live_boundary:
        raise RuntimeError("Team 12 live replay has an unexpected terminal boundary")
...
def persist_paper_tick(
    ...
    integrity = {
        "schema_version": "team12-paper-integrity-v2",
        "status": "PASS",
        "candidate_id": CANDIDATE_ID,
        "paper_only": True,
        "boundary": tick.boundary.isoformat(),
        "forming_terminal_return_discarded": True,
        "shared_backtest_live_path": "crypto_trade.team12.backtest.run_replay",
...
def _position_frame(tick: Team12PaperTick) -> pd.DataFrame:
...
def _append_boundary_fills(tick: Team12PaperTick, path: Path) -> pd.DataFrame:
...
def _boundary_payload(
    ...
    return {
        "schema_version": "team12-paper-boundary-v1",
        "candidate_id": CANDIDATE_ID,
        "paper_only": True,
        "boundary": tick.boundary.isoformat(),
    ...
    if path.is_file():
        for raw_line in path.read_text(encoding="utf-8").splitlines():
            ...
            if not isinstance(payload, dict):
                raise ValueError("Team 12 run log contains a non-object")
            rows.append(payload)
