"""Append-invariant live-data extension for the frozen Team 12 snapshot."""
...
CACHE_SCHEMA_VERSION = "team12-live-cache-v2"
_KLINE_COLUMNS = (
    "open_time",
    "open",
    "high",
...
def exact_boundary(value: object) -> pd.Timestamp:
    ...
    if timestamp != boundary:
        raise ValueError("Team 12 live boundary must be an exact 8-hour UTC boundary")
    if boundary < BRIDGE_START:
        raise ValueError(f"Team 12 live boundary cannot precede {BRIDGE_START.isoformat()}")
    return boundary
...
def build_live_market_data(
    frozen: Team12MarketData,
    *,
    boundary: object,
    cache_dir: str | Path,
    client: Team12PublicDataClient | None,
    ...
    if anchors.empty:
        raise RuntimeError("Team 12 bridge has no anchor membership")
...
def extend_membership(
    ...
    *,
    ...
    if present != expected:
        ...
        raise RuntimeError(f"empty live Team 12 membership reconstitutions: {missing}")
    for timestamp, group in live.groupby("reconstitution_time", observed=True):
        ...
        if len(group) != 12 or ranks != list(range(1, 13)):
            raise RuntimeError(
                "Team 12 live membership is not an exact Top12 at "
...
def validate_live_cache_domains(
    ...
    if not candidates:
        raise RuntimeError("Team 12 live candidate registry is empty")
    observed = (
    ...
    if extras:
        raise RuntimeError(
            f"Team 12 live cache contains symbols outside its registry: {sorted(extras)}"
        )
    rejected = {
    ...
    if rejected:
        raise RuntimeError(
            f"Team 12 candidate registry violates pure-crypto policy: {rejected}"
        )
    if set(lifecycle) != candidates:
        raise RuntimeError(
            "Team 12 lifecycle registry differs from the candidate registry"
        )
...
def validate_live_cache_coverage(
    ...
    *,
    ...
    if missing_opens:
        raise RuntimeError(
            "Team 12 current members/carried symbols lack exact transaction "
    ...
    if missing_marks:
        raise RuntimeError(
            "Team 12 current members/carried symbols lack exact boundary "
    ...
    if (funding_times > boundary + pd.Timedelta(seconds=1)).any():
        raise RuntimeError("Team 12 funding cache contains future settlements")
    ...
    if missing_complete:
        raise RuntimeError(
            "Team 12 current members/carried symbols lack the latest completed bar: "
...
def verify_live_cache_manifest(
    ...
    *,
    ...
    if not manifest_path.is_file():
        ...
        if required:
            raise FileNotFoundError(
                f"Team 12 live cache manifest is missing: {manifest_path}"
            )
        return None
    payload = _load_json_mapping(manifest_path)
    if payload.get("schema_version") != CACHE_SCHEMA_VERSION:
        raise RuntimeError("Team 12 live cache manifest schema drift")
    if payload.get("pure_crypto_policy_id") != POLICY_ID:
        raise RuntimeError("Team 12 live cache pure-crypto policy drift")
    entries = payload.get("files")
    if not isinstance(entries, list) or not entries:
        raise ValueError("Team 12 live cache manifest has no file bindings")
    seen: set[str] = set()
    for entry in entries:
        if not isinstance(entry, Mapping):
            raise ValueError("Team 12 live cache manifest contains a non-object")
        ...
        if (
            ...
            raise ValueError("Team 12 live cache manifest path is unsafe or duplicated")
        seen.add(relative)
        path = (cache / relative).resolve()
        if not path.is_relative_to(cache) or not path.is_file():
            raise RuntimeError(f"Team 12 live cache file is missing: {relative}")
        if path.stat().st_size != entry.get("size"):
            raise RuntimeError(f"Team 12 live cache size drift: {relative}")
        if _sha256_file(path) != entry.get("sha256"):
            raise RuntimeError(f"Team 12 live cache hash drift: {relative}")
        if _cache_file_rows(path) != entry.get("rows"):
            raise RuntimeError(f"Team 12 live cache row-count drift: {relative}")
    required_paths = {
        "archive-kline-provenance.json",
        "archive-symbols.json",
        "bars.parquet",
    ...
    if len(exchange_paths) != 1 or seen != required_paths | exchange_paths:
        raise RuntimeError(
            "Team 12 live cache manifest does not bind the exact load-bearing set"
        )
    diagnostics = _load_json_mapping(cache / "diagnostics.json")
    if diagnostics.get("boundary") != payload.get("boundary"):
        raise RuntimeError("Team 12 live cache manifest/diagnostic boundary drift")
    return payload
...
def write_live_cache_manifest(
    ...
    *,
    ...
    if not exchange_snapshots:
        raise RuntimeError("Team 12 live cache has no exchangeInfo evidence")
    paths.append(exchange_snapshots[-1])
    missing = [str(path) for path in paths if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Team 12 cannot seal incomplete live cache: {missing}")
    entries = []
...
def _combine_historical(
    ...
    *,
    ...
    if result.duplicated(keys).any():
        raise ValueError(f"combined Team 12 data contains duplicate keys: {keys}")
...
def _read_required_parquet(path: Path) -> pd.DataFrame:
    if not path.is_file():
        raise FileNotFoundError(f"Team 12 live cache is missing: {path}")
