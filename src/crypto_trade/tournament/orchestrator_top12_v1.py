"""Organizer authority for the two-round Top-12 V1 tournament."""
...
CONFIG_PATH = TOP12_V1_LAYOUT.config_path
...
def _result_lock(root: Path) -> Iterator[None]:
    path = _path(root, TOP12_V1_LAYOUT.result_lock_path)
    _ensure_directory(root, path.parent)
...
def _journal_path(root: Path) -> Path:
    return _path(root, TOP12_V1_LAYOUT.journal_path)
...
def _candidate_metadata(
    ...
    TOP12_V1_LAYOUT.require_team(team_id)
    entry = _path(root, entrypoint)
    team_root = _path(root, TOP12_V1_LAYOUT.team_root(team_id))
    try:
...
def _derive_authority(
    root: Path,
    loaded: top12_v1.LoadedV4Config,
    team_id: str,
    entrypoint: str,
    ...
    capture = runner_top12_v1.capture_source_bundle(root, team_id, entrypoint)
    ...
    authority = CandidateAuthority(
        ...
        evaluator_sha256=runner_top12_v1._evaluator_authority_sha256(root),
    )
...
def activate(root: str | Path) -> Mapping[str, Any]:
    ...
    with _result_lock(root_path):
        top12_v1.load_config(root=root_path)
        journal_top12_v1.initialize(_journal_path(root_path))
        return activation_top12_v1.activate(root_path)


def validate(root: str | Path, *, require_activation: bool = True) -> Mapping[str, Any]:
    root_path = _safe_root(root)
    loaded = top12_v1.load_config(root=root_path)
    activation = activation_top12_v1.validate(root_path) if require_activation else None
    journal_path = _journal_path(root_path)
    state = (
        journal_top12_v1.read(journal_path)
        if journal_path.exists()
        else journal_top12_v1.replay_bytes(b"")
    )
    ...
    return {
        "ok": True,
        "tournament": TOP12_V1_LAYOUT.name,
        "config_sha256": loaded.sha256,
...
def status(root: str | Path) -> Mapping[str, Any]:
    root_path = _safe_root(root)
    loaded = top12_v1.load_config(root=root_path)
    activated = _path(root_path, TOP12_V1_LAYOUT.activation_freeze_path).exists()
    journal_path = _journal_path(root_path)
    state = (
        journal_top12_v1.read(journal_path)
        if journal_path.exists()
...
def _run_team(
    ...
    *,
    stage: str,
    output: str,
) -> runner_top12_v1.TeamWindowRunResult:
    return runner_top12_v1.run_team(
        root,
        authority.team_id,
        ...
        DATA_MANIFEST_PATH,
        stage=stage,
        _authorization=runner_top12_v1._ORGANIZER_RUN_AUTHORIZATION,
...
def _close_interrupted_is_requests(root: Path) -> journal_top12_v1.JournalState:
    """Convert requests left pending by process death into consumed failed trials."""

    state = journal_top12_v1.read(_journal_path(root))
    pending = [
    ...
    return journal_top12_v1.read(_journal_path(root))
...
def run_is(
    ...
    *,
    ...
    with _result_lock(root_path):
        ...
        activation_top12_v1.validate(root_path, verify_universe_snapshot=False)
        loaded = top12_v1.load_config(root=root_path)
        ...
        output = f"{TOP12_V1_LAYOUT.reports_root}/is/{team_id}/{run_id}"
        request = journal_top12_v1.append(
        ...
        try:
            ...
            summary = scoring_top12_v1.summarize_run(
                root_path,
                result,
                loaded.raw,
                trial_count=trial,
            ...
            terminal = journal_top12_v1.append(
                _journal_path(root_path),
                "is_succeeded",
                {
                    "team_id": team_id,
...
def _candidate_success(
    state: journal_top12_v1.JournalState, team_id: str, candidate_id: str
...
def _certificate(
    ...
    relative: str,
    *,
    state: journal_top12_v1.JournalState,
    config: Mapping[str, Any],
    team_id: str,
    candidate_id: str,
    nominated_request_hash: str,
) -> tuple[Mapping[str, Any], str, dict[str, Any]]:
    required_prefix = f"{TOP12_V1_LAYOUT.tournament_root}/certificates/{team_id}/"
...
def _write_nomination_registry(root: Path, state: journal_top12_v1.JournalState) -> None:
    registry = {
        "schema_version": "top12-v1-nomination-registry-v1",
    ...
    _write_atomic(
        root,
        TOP12_V1_LAYOUT.nomination_registry_path,
        _pretty(registry),
        replace=True,
    )
...
def nominate(
    ...
    root_path = _safe_root(root)
    with _result_lock(root_path):
        activation_top12_v1.validate(root_path)
        loaded = top12_v1.load_config(root=root_path)
        state = _close_interrupted_is_requests(root_path)
        if state.selection is not None:
            raise OrchestratorError("IS is closed")
        ...
        selection = scoring_top12_v1.assess_is(
            summary,
            loaded.raw,
        ...
        nomination = {
            "schema_version": "top12-v1-nomination-v1",
            "team_id": team_id,
            "candidate_id": candidate_id,
            "trial_count": state.trials_by_team[team_id],
        ...
        nomination_path = (
            f"{TOP12_V1_LAYOUT.tournament_root}/nominations/{team_id}-{candidate_id}.json"
        )
        ...
        record = journal_top12_v1.append(
            _journal_path(root_path),
            "nominated",
            {
                "team_id": team_id,
        ...
        _write_nomination_registry(root_path, journal_top12_v1.read(_journal_path(root_path)))
        return {
            "ok": True,
            "team_id": team_id,
            "candidate_id": candidate_id,
...
def close_is(root: str | Path) -> Mapping[str, Any]:
    root_path = _safe_root(root)
    with _result_lock(root_path):
        activation = activation_top12_v1.validate(root_path)
        loaded = top12_v1.load_config(root=root_path)
        state = _close_interrupted_is_requests(root_path)
        _write_nomination_registry(root_path, state)
        if state.selection is not None:
        ...
        if set(state.nominations) | set(state.retired) != set(TOP12_V1_LAYOUT.team_ids):
            unresolved = sorted(
                set(TOP12_V1_LAYOUT.team_ids) - set(state.nominations) - set(state.retired)
            )
        ...
        population.sort(key=lambda row: scoring_top12_v1.is_ranking_key(row["selection"]))
        ...
        freeze = {
            "schema_version": "top12-v1-selection-freeze-v1",
            "tournament": TOP12_V1_LAYOUT.name,
        ...
        freeze_sha256 = _write_atomic(
            root_path,
            TOP12_V1_LAYOUT.selection_freeze_path,
            freeze_payload,
            replace=False,
        )
        record = journal_top12_v1.append(
...
def _selection_freeze(root: Path, state: journal_top12_v1.JournalState) -> Mapping[str, Any]:
    if state.selection is None:
...
def _historical_summary_for_team(
    root: Path, state: journal_top12_v1.JournalState, team_id: str
...
def _ensemble_release(
    ...
    packet: dict[str, Any] = {
        "schema_version": "top12-v1-historical-oos-ensemble-v1",
...
def _release_bundle(
    ...
    reports_root = _path(root, TOP12_V1_LAYOUT.reports_root)
    _ensure_directory(root, reports_root)
    staging_relative = f"{TOP12_V1_LAYOUT.reports_root}/.historical-oos-staging"
    staging = _path(root, staging_relative)
    release_relative = f"{TOP12_V1_LAYOUT.reports_root}/historical-oos"
    ...
    try:
        ...
        for finalist in selection["advancing"]:
            ...
            if terminal["event_type"] == "historical_succeeded":
                ...
                _write_stage_file(
                    ...
                    _pretty(
                        {
                            "schema_version": "top12-v1-historical-oos-dnf-v1",
                            "team_id": team_id,
        ...
        eligible.sort(
            key=lambda packet: scoring_top12_v1.historical_ranking_key(
                packet["championship"]
            )
        )
        winner = None
        ...
        manifest = {
            "schema_version": "top12-v1-historical-oos-release-v1",
...
def historical_release(root: str | Path) -> Mapping[str, Any]:
    ...
    with _result_lock(root_path):
        ...
        activation_top12_v1.validate(root_path, verify_universe_snapshot=False)
        loaded = top12_v1.load_config(root=root_path)
        state = journal_top12_v1.read(_journal_path(root_path))
        ...
        for finalist in selection["advancing"]:
            ...
            output = f"{TOP12_V1_LAYOUT.tournament_root}/private/historical-oos/{team_id}/{run_id}"
            journal_top12_v1.append(
                _journal_path(root_path),
                "historical_accepted",
                {
        ...
        authorization = journal_top12_v1.append(
            _journal_path(root_path),
            "release_authorized",
            {
                "selection_record_sha256": state.selection_record_sha256,
