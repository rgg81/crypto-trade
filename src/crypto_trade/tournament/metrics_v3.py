"""V3-neutral daily, risk, regime, and bootstrap metrics.

The numerical algorithms intentionally match the tournament's canonical
``metrics.py`` implementation.  V3 owns its result schema, regime authority,
and bootstrap seed here, without importing a lifecycle or runner module.
"""
...
class WindowMetrics:
    """The six scalar metrics frozen by the V3 window-result schema."""
...
def compute_window_metrics(daily_returns: pd.Series) -> WindowMetrics:
    """Compute the exact scalar metrics consumed by the V3 result schema."""
