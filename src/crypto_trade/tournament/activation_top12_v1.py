"""One-time hash freeze for the Top-12 V1 tournament infrastructure."""
...
from crypto_trade.tournament import top12_universe_v1, top12_v1
from crypto_trade.tournament.layout_top12_v1 import TOP12_V1_LAYOUT

SCHEMA_VERSION = "top12-v1-activation-freeze-v1"
TEST_OUTPUT_PATH = "tournament/top12-v1/activation-tests.out"
...
FROZEN_SCOPE = (
    ...
    "pyproject.toml",
    "scripts/build_top12_universe.py",
    "scripts/top12_v1_tournament.py",
    "src/crypto_trade/tournament/_strategy_worker_v4.py",
    "src/crypto_trade/tournament/activation_top12_v1.py",
    ...
    "src/crypto_trade/tournament/journal_top12_v1.py",
    "src/crypto_trade/tournament/layout_top12_v1.py",
    "src/crypto_trade/tournament/metrics_v3.py",
    "src/crypto_trade/tournament/orchestrator_top12_v1.py",
    ...
    "src/crypto_trade/tournament/top12_v1.py",
    "src/crypto_trade/tournament/top12_universe_v1.py",
    "tests/tournament/test_top12_v1.py",
    "tests/tournament/test_top12_v1_team_bundles.py",
    "tournament/top12/data_manifest.json",
    "tournament/top12-v1/ORCHESTRATOR-MECHANISM-REGISTRY.json",
    "tournament/top12-v1/README.md",
    "tournament/top12-v1/ROBUSTNESS-POLICY.md",
    "tournament/top12-v1/TEAM-API.md",
    "tournament/top12-v1/TEAM-ISOLATION.md",
    "tournament/top12-v1/TEAM-PLAYBOOK.md",
    "tournament/top12-v1/config.toml",
    "tournament/top12-v1/universe-build.json",
    *tuple(
        f"tournament/top12-v1/teams/team-{number:02d}/BRIEF.md"
...
class ActivationError(ValueError):
    """The Top-12 V1 activation evidence is missing, unsafe, or no longer valid."""
    ...
    try:
        ...
        if exclusive:
            try:
                ...
                raise ActivationError("Top-12 V1 activation freeze already exists") from exc
        else:
...
def _implementation_commit(root: Path) -> str:
    ...
    if branch != TOP12_V1_LAYOUT.branch:
        raise ActivationError(f"current branch must be {TOP12_V1_LAYOUT.branch}")
...
def _audit_sha256(root: Path, config: Mapping[str, Any]) -> str:
    report = top12_universe_v1.audit_report_bytes(root)
    ...
    if (
        ...
        or classification.get("forbidden_exposure_violations") != 0
    ):
...
def activate(root: str | Path) -> Mapping[str, Any]:
    ...
    freeze_path = root_path / TOP12_V1_LAYOUT.activation_freeze_path
    if freeze_path.exists():
        raise ActivationError("Top-12 V1 activation is one-time and already exists")
    ...
    if exit_code != 0:
        raise ActivationError(
            f"Top-12 V1 focused activation tests failed; inspect {TEST_OUTPUT_PATH}"
        )
    ...
    unsigned: dict[str, Any] = {
        ...
        "tournament_id": TOP12_V1_LAYOUT.name,
        "branch": TOP12_V1_LAYOUT.branch,
...
def validate(root: str | Path, *, verify_universe_snapshot: bool = True) -> Mapping[str, Any]:
    ...
    if (
        ...
        or record["tournament_id"] != TOP12_V1_LAYOUT.name
        or record["branch"] != TOP12_V1_LAYOUT.branch
        or record["config_sha256"] != loaded.sha256
    ):
        raise ActivationError("V4 activation identity differs from the current contract")
    if _git(root_path, "branch", "--show-current") != TOP12_V1_LAYOUT.branch:
        raise ActivationError(f"current branch must be {TOP12_V1_LAYOUT.branch}")
