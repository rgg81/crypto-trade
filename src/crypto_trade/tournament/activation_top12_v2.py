"""One-time hash freeze for the Top-12 V2 tournament infrastructure."""
...
from crypto_trade.tournament import top12_universe_v1, top12_v2
from crypto_trade.tournament.layout_top12_v2 import TOP12_V2_LAYOUT

SCHEMA_VERSION = "top12-v2-activation-freeze-v1"
TEST_OUTPUT_PATH = "tournament/top12-v2/activation-tests.out"
...
FROZEN_SCOPE = (
    ...
    "scripts/top12_v2_tournament.py",
    "src/crypto_trade/tournament/_strategy_worker_v4.py",
    "src/crypto_trade/tournament/activation_top12_v2.py",
    ...
    "src/crypto_trade/tournament/journal_top12_v2.py",
    "src/crypto_trade/tournament/layout_top12_v2.py",
    "src/crypto_trade/tournament/metrics_v3.py",
    "src/crypto_trade/tournament/orchestrator_top12_v2.py",
    ...
    "tests/tournament/test_top12_v2.py",
    "tests/tournament/test_top12_v2_team_bundles.py",
    "tournament/top12/data_manifest.json",
    "tournament/top12-v2/ORCHESTRATOR-MECHANISM-REGISTRY.json",
    "tournament/top12-v2/README.md",
    "tournament/top12-v2/ROBUSTNESS-POLICY.md",
    "tournament/top12-v2/TEAM-API.md",
    "tournament/top12-v2/TEAM-ISOLATION.md",
    "tournament/top12-v2/TEAM-PLAYBOOK.md",
    "tournament/top12-v2/config.toml",
    "tournament/top12-v1/universe-build.json",
    *tuple(
        f"tournament/top12-v2/teams/team-{number:02d}/BRIEF.md"
...
class ActivationError(ValueError):
    """The Top-12 V2 activation evidence is missing, unsafe, or no longer valid."""
    ...
    try:
        ...
        if exclusive:
            try:
                ...
                raise ActivationError("Top-12 V2 activation freeze already exists") from exc
        else:
...
def _implementation_commit(root: Path) -> str:
    ...
    if branch != TOP12_V2_LAYOUT.branch:
        raise ActivationError(f"current branch must be {TOP12_V2_LAYOUT.branch}")
...
def activate(root: str | Path) -> Mapping[str, Any]:
    ...
    freeze_path = root_path / TOP12_V2_LAYOUT.activation_freeze_path
    if freeze_path.exists():
        raise ActivationError("Top-12 V2 activation is one-time and already exists")
    ...
    if exit_code != 0:
        raise ActivationError(
            f"Top-12 V2 focused activation tests failed; inspect {TEST_OUTPUT_PATH}"
        )
    ...
    unsigned: dict[str, Any] = {
        ...
        "tournament_id": TOP12_V2_LAYOUT.name,
        "branch": TOP12_V2_LAYOUT.branch,
...
def validate(root: str | Path, *, verify_universe_snapshot: bool = True) -> Mapping[str, Any]:
    ...
    if (
        ...
        or record["tournament_id"] != TOP12_V2_LAYOUT.name
        or record["branch"] != TOP12_V2_LAYOUT.branch
        or record["config_sha256"] != loaded.sha256
    ):
        raise ActivationError("V4 activation identity differs from the current contract")
    if _git(root_path, "branch", "--show-current") != TOP12_V2_LAYOUT.branch:
        raise ActivationError(f"current branch must be {TOP12_V2_LAYOUT.branch}")
