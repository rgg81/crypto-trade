"""Cutoff-aware, strategy-neutral runner for one isolated Top-40 V3 team."""
...
_EVALUATOR_AUTHORITY_PATHS = (
    TOP40_V3_LAYOUT.contract_source,
...
def _pure_crypto_preflight(root: Path, config: Mapping[str, Any]) -> str:
    ...
    if report_sha256 != authority["audit_report_sha256"]:
        raise ValueError("A6 deterministic audit report hash differs from the V3 config")
    try:
    ...
    if (
        ...
        raise ValueError("A6 deterministic audit did not prove a zero-violation pure-crypto universe")
...
def run_team(
    ...
    *,
    ...
    if _authorization is not _ORGANIZER_RUN_AUTHORIZATION:
        raise PermissionError("full-window V3 evaluation is a trusted organizer operation")
    root_path = Path(root).resolve()
    if not all(
        isinstance(value, str) and value
        for value in (_candidate_id, _source_archive_relative, _source_archive_sha256)
    ):
        raise ValueError("V3 runs require an immutable candidate source archive authority")
    ...
    if not expected_output_path.is_relative_to(root_path):
        raise ValueError("V3 output directory escapes the tournament root")
    ...
    try:
        ...
        if after != before:
            ...
            raise ValueError("A6 deterministic audit authority changed during the V3 run")
    if result is None:
        raise RuntimeError("V3 runner completed without a result")
...
def _remove_just_created_output(
    ...
    *,
    ...
    if validated != expected_output_relative:
        raise RuntimeError("refusing to remove an unexpected V3 output directory")
    ...
    if (
        ...
        raise RuntimeError("refusing to remove an unsafe V3 output directory")
    if not expected_output_path.exists():
        return
    if not expected_output_path.is_dir():
        raise RuntimeError("refusing to remove a non-directory V3 output path")
...
def _run_team_impl(
    ...
    *,
    ...
    _authorization: object | None = None,
    _output_relative: str | None = None,
    _source_archive: source_archive_v3.SourceArchive | None = None,
) -> TeamWindowRunResult:
    ...
    if _authorization is not _ORGANIZER_RUN_AUTHORIZATION:
        raise PermissionError(
            ...
            "use scripts/top40_v3_tournament.py"
        )
    if _source_archive is None:
        raise ValueError("V3 runner requires a verified immutable source archive")
    root_path = Path(root).resolve()
    ...
    seed = int(config["labs"]["strategy_seed"])
    ...
    return TeamWindowRunResult(
        ...
        pure_crypto_policy_sha256=str(
            config["universe"]["a6_authority"]["policy_sha256"]
        ),
        pure_crypto_report_sha256=str(
            config["universe"]["a6_authority"]["audit_report_sha256"]
        ),
...
def _resolve_team_entrypoint(root: Path, team_id: str, entrypoint: str | Path) -> tuple[Path, str]:
    ...
    required_prefix = f"{TOP40_V3_LAYOUT.tournament_root}/teams/{team_id}/"
...
def _resolve_team_risk_policy(root: Path, team_id: str, entrypoint: Path) -> Path:
    ...
    team_root = (root / TOP40_V3_LAYOUT.team_root(team_id)).resolve()
    ...
    path = (candidate_root / "risk_policy.json").resolve()
    if (
...
class SourceBundleCapture:
    ...
    entrypoint: str
    sha256: str
    files: tuple[source_archive_v3.SourceFile, ...]
...
def _team_tree_fingerprint(files: Sequence[_TeamTreeFile]) -> str:
    ...
    return source_archive_v3.bundle_fingerprint(entries)
...
def capture_source_bundle(
    ...
    captured: list[source_archive_v3.SourceFile] = []
    for item in before:
...
def _strategy_worker_command(
    ...
    return [
        ...
        "-u",
        "-m",
        "crypto_trade.tournament._strategy_worker_v3",
        "--root",
        str(root),
        "--repository-parent",
...
def _launch_strategy_worker(
    ...
    sandbox = tempfile.TemporaryDirectory(prefix=f"top40-v3-{team_id}-")
    ...
    entrypoint_path, _entrypoint_relative = _resolve_team_entrypoint(
        root, team_id, entrypoint
    )
    try:
...
def _load_config(path: Path, team_id: str) -> dict[str, Any]:
    ...
    strategy_seed = int(config["labs"]["strategy_seed"])
    if not 0 <= strategy_seed <= 2**32 - 1:
        raise ValueError("labs.strategy_seed must fit PYTHONHASHSEED's integer range")
    return config


def _authorized_window(config: Mapping[str, Any], stage: str) -> AuthorizedWindow:
    splits = config["splits"]
    replay_start = str(splits["train"]["start"])
    if stage == "public":
        score_start = str(splits["train"]["start"])
        end_exclusive = str(splits["validation"]["end_exclusive"])
    elif stage in {"train", "validation", "private", "final_oos"}:
        score_start = str(splits[stage]["start"])
        end_exclusive = str(splits[stage]["end_exclusive"])
    else:
        raise ValueError(
            "runner stage must be train, validation, public, private, or final_oos"
        )
...
def _load_verified_snapshot(root: Path, manifest_path: Path) -> _SnapshotData:
    """Verify every manifest entry directly before loading the five V3 datasets."""
...
def _stage_output_prefix(team_id: str, stage: str) -> str:
    TOP40_V3_LAYOUT.require_team(team_id)
    if stage == "train":
        return f"{TOP40_V3_LAYOUT.reports_root}/labs/{team_id}/"
    if stage in {"validation", "public", "private"}:
        return f"{TOP40_V3_LAYOUT.tournament_root}/private/{stage}/{team_id}/"
    if stage == "final_oos":
        return f"{TOP40_V3_LAYOUT.tournament_root}/private/final-oos/{team_id}/"
    raise ValueError(
        "runner stage must be train, validation, public, private, or final_oos"
    )


def _final_publication_prefix(team_id: str) -> str:
    """Canonical public namespace reserved for a later one-shot final publisher."""

    TOP40_V3_LAYOUT.require_team(team_id)
    return f"{TOP40_V3_LAYOUT.reports_root}/results/final-oos/{team_id}/"
...
def _validated_output_relative(
    ...
    if not isinstance(override, str) or not override:
        raise ValueError("every V3 run requires a unique organizer output path")
    ...
    if (
        ...
        raise ValueError(f"{stage} output must remain under its canonical V3 stage prefix")
