...
from crypto_trade.tournament import metrics_v3, runner_top12_v2
...
def _artifact(root: Path, result: runner_top12_v2.TeamWindowRunResult, name: str) -> bytes:
...
def summarize_run(
    root: str | Path,
    result: runner_top12_v2.TeamWindowRunResult,
    ...
    *,
    ...
    packet: dict[str, Any] = {
        "schema_version": "top12-v2-run-summary-v1",
        "stage": result.stage,
    ...
    elif result.stage == "is_confirmation":
        packet["confirmation"] = assess_is_confirmation(packet, config)
    elif result.stage == "historical_oos":
        packet["championship"] = assess_historical_oos(packet, config)
    else:
        raise ScoringError("V2 summary stage is not authorized")
    return packet
...
def assess_is(
    ...
    *,
    ...
    gates = {
        ...
        "median_fold_sharpe": float(np.median(double_fold_sharpes))
        >= float(floors["minimum_median_fold_sharpe_inclusive"]),
...
def assess_is_confirmation(
    packet: Mapping[str, Any], config: Mapping[str, Any]
) -> dict[str, Any]:
    """Apply the frozen one-year confirmation gates exactly once per nominee."""

    policy = config["is_confirmation"]
    window = packet["scored_window"]
    base = window["base_metrics"]
    double = window["double_cost_metrics"]
    diagnostics = packet["diagnostics"]
    quarters = int(round(float(base["positive_quarter_fraction"]) * 4.0))
    edge = diagnostics["gross_edge_per_turnover_bps"]
    cost_share = diagnostics["base_cost_share_of_positive_gross_pnl"]
    gates = {
        "positive_base_return": float(base["annualized_return"])
        > float(policy["minimum_base_annualized_return_exclusive"]),
        "positive_double_cost_return": float(double["annualized_return"])
        > float(policy["minimum_double_cost_annualized_return_exclusive"]),
        "base_sharpe": float(base["net_sharpe"])
        >= float(policy["minimum_base_sharpe_inclusive"]),
        "double_cost_sharpe": float(double["net_sharpe"])
        >= float(policy["minimum_double_cost_sharpe_inclusive"]),
        "max_drawdown": float(base["max_drawdown"])
        <= float(policy["maximum_drawdown_inclusive"]),
        "positive_quarters": quarters >= int(policy["minimum_positive_quarters"]),
        "annualized_turnover": float(diagnostics["annualized_turnover"])
        <= float(policy["maximum_annualized_turnover_inclusive"]),
        "gross_edge_density": edge is not None
        and float(edge) >= float(policy["minimum_gross_edge_per_turnover_bps_inclusive"]),
        "base_cost_share": cost_share is not None
        and float(cost_share) <= float(policy["maximum_base_cost_share_inclusive"]),
    }
    return {
        "eligible": all(gates.values()),
        "gates": gates,
...
def confirmation_ranking_key(confirmation: Mapping[str, Any]) -> tuple[Any, ...]:
    """Deterministic confirmation-only ordering used after the pass/fail gates."""

    vector = confirmation["ranking_vector"]
...
__all__ = [
    "ScoringError",
    "assess_is_confirmation",
    "assess_historical_oos",
    "assess_is",
    "confirmation_ranking_key",
