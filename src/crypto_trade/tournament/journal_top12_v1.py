"""Strict append-only lifecycle journal for the Top-12 V1 tournament."""
...
SCHEMA_VERSION = "top12-v1-lifecycle-journal-v1"
GENESIS_SHA256 = "0" * 64
...
def _team(value: object) -> str:
    if not isinstance(value, str) or value not in TOP12_V1_LAYOUT.team_ids:
...
def replay_bytes(payload: bytes) -> JournalState:
    ...
    head = GENESIS_SHA256
    trials = {team_id: 0 for team_id in TOP12_V1_LAYOUT.team_ids}
    ...
    for expected_sequence, record in enumerate(records, start=1):
        ...
        if event_type == "is_accepted":
            ...
            if set(nominations) | set(retired) != set(TOP12_V1_LAYOUT.team_ids):
