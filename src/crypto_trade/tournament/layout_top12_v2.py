"""Independent twelve-team path layout for Top-12 V2."""
...
class TournamentLayoutTop12V2:
    """Canonical Top-12 V2 identity and authority paths."""
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.team_ids) != 12 or len(set(self.team_ids)) != 12:
            raise ValueError("a Top-12 V2 layout requires twelve unique teams")
        if any(_TEAM_ID.fullmatch(team_id) is None for team_id in self.team_ids):
            raise ValueError("Top-12 V2 team ids must be team-01 through team-12")
    ...
    @property
    def confirmation_freeze_path(self) -> str:
        return f"{self.tournament_root}/confirmation-freeze.json"

    def require_team(self, team_id: str) -> str:
        if team_id not in self.team_ids:
            raise ValueError(f"unknown Top-12 V2 team: {team_id}")
        return team_id
...
TOP12_V2_LAYOUT = TournamentLayoutTop12V2(
    name="quant-portfolio-blind-top12-v2",
    branch="quant-portfolio-blind-top12-v2",
    tournament_root="tournament/top12-v2",
    reports_root="reports-top12-v2",
    orchestrator_script="scripts/top12_v2_tournament.py",
    contract_source="src/crypto_trade/tournament/top12_v2.py",
    team_ids=tuple(f"team-{number:02d}" for number in range(1, 13)),
)


__all__ = ["TOP12_V2_LAYOUT", "TournamentLayoutTop12V2"]
