"""Independent twelve-team path layout for Top-40 V4."""
...
class TournamentLayoutV4:
    """Canonical V4 identity and authority paths without V3's ten-team base class."""
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.team_ids) != 12 or len(set(self.team_ids)) != 12:
            raise ValueError("a V4 layout requires twelve unique teams")
        if any(_TEAM_ID.fullmatch(team_id) is None for team_id in self.team_ids):
            raise ValueError("V4 team ids must be team-01 through team-12")
    ...
    def require_team(self, team_id: str) -> str:
        if team_id not in self.team_ids:
            raise ValueError(f"unknown V4 team: {team_id}")
        return team_id
...
__all__ = ["TOP40_V4_LAYOUT", "TournamentLayoutV4"]
