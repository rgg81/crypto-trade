"""Cutoff-aware, strategy-neutral runner for one isolated Top-40 V2 team."""
...
import crypto_trade.tournament.top40_v2 as tournament_contract
...
from crypto_trade.tournament.top40_v2 import EvaluationWindow, WindowMetrics
        ...
        return {
            ...
            "confidence_intervals": {
                ...
                "double_cost_sharpe_95": list(
                    self.double_cost_sharpe_confidence_interval
                ),
            },
    ...
    *,
    ...
    if _authorization is not _ORGANIZER_RUN_AUTHORIZATION:
        raise PermissionError(
            ...
            "use scripts/top40_v2_tournament.py"
        )
    ...
    risk_policy_path = _resolve_team_risk_policy(root_path, team_id)
    ...
    expected_manifest_path = (
        root_path / "tournament/top40/data_manifest.json"
    ).resolve()
    ...
    bars = snapshot.bars.loc[
        ...
        < pd.Timestamp(authorized.end_exclusive, tz="UTC")
    ].copy()
    ...
    metrics = _compute_metrics(base_daily, stressed_daily, btc_daily, config, authorized)
    ...
    return TeamWindowRunResult(
        ...
        double_cost_sharpe_confidence_interval=metrics[
            "double_cost_sharpe_confidence_interval"
        ],
...
def _resolve_team_entrypoint(root: Path, team_id: str, entrypoint: str | Path) -> tuple[Path, str]:
    ...
    required_prefix = f"{TOP40_V2_LAYOUT.tournament_root}/teams/{team_id}/"
...
def _resolve_team_risk_policy(root: Path, team_id: str) -> Path:
    team_root = (root / TOP40_V2_LAYOUT.team_root(team_id)).resolve()
    path = team_root / "risk_policy.json"
    if (
        ...
        raise ValueError(f"risk_policy.json is missing or unsafe for {team_id}")
    return path
...
def _strategy_worker_command(
    ...
    return [
        ...
        "-u",
        "-m",
        "crypto_trade.tournament._strategy_worker_v2",
        "--root",
        str(root),
        "--repository-parent",
...
def _launch_strategy_worker(
    ...
    sandbox = tempfile.TemporaryDirectory(prefix=f"top40-v2-{team_id}-")
...
def _load_config(path: Path, team_id: str) -> dict[str, Any]:
    loaded = tournament_contract.load_config(path)
    ...
    strategy_seed = int(config["research_budget"]["strategy_seed"])
...
def _authorized_window(config: Mapping[str, Any], stage: str) -> AuthorizedWindow:
    ...
    replay_start = str(splits["visible_development_start"])
    if stage == "development":
        score_start = replay_start
        score_end = str(splits["visible_development_end_inclusive"])
    elif stage == "private":
        score_start = str(splits["private_qualifier_start"])
        score_end = str(splits["private_qualifier_end_inclusive"])
    elif stage == "final_oos":
        score_start = str(splits["final_oos_start"])
        score_end = str(splits["final_oos_end_inclusive"])
    else:
        raise ValueError("runner stage must be development, private, or final_oos")
    end_exclusive = (pd.Timestamp(score_end) + pd.Timedelta(days=1)).date().isoformat()
...
def _decision_grid(authorized: AuthorizedWindow) -> pd.DatetimeIndex:
    start = pd.Timestamp(authorized.replay_start, tz="UTC")
...
def _phase0_allows_fast_snapshot_verification(
    ...
    """
    return _v2_phase0_allows_fast_snapshot_verification(
        root, manifest_path, manifest, manifest_text
    )


def _v2_phase0_allows_fast_snapshot_verification(
    root: Path,
    manifest_path: Path,
    manifest: Mapping[str, Any],
    manifest_text: str,
) -> bool:
    """Verify a generic V2 frozen-file map before using the snapshot fast path."""
    ...
    if not freeze_path.is_file() or freeze_path.is_symlink():
        raise ValueError("V2 phase0_freeze.json is missing or unsafe")
    state_path = root / TOP40_V2_LAYOUT.state_path
    if not state_path.is_file() or state_path.is_symlink():
        raise ValueError("V2 run_state.json is missing or unsafe")
    try:
        freeze = json.loads(freeze_path.read_text(encoding="utf-8"))
        state = json.loads(state_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"invalid V2 Phase-0 binding: {exc}") from exc
    if not isinstance(freeze, Mapping) or not isinstance(state, Mapping):
        raise ValueError("invalid V2 Phase-0 binding schema")
    required = {
        "schema_version",
        "frozen_at_utc",
        "branch",
    ...
    if set(freeze) != required or freeze.get("schema_version") != 2:
        raise ValueError("V2 phase0_freeze.json has an invalid schema")
    if freeze.get("branch") != TOP40_V2_LAYOUT.branch:
        raise ValueError("V2 Phase-0 branch binding is invalid")
    ...
    if branch.returncode or branch.stdout.strip() != TOP40_V2_LAYOUT.branch:
        raise ValueError(f"current branch must be {TOP40_V2_LAYOUT.branch}")
    config = tournament_contract.load_config(root / TOP40_V2_LAYOUT.config_path)
    tournament_contract.validate_run_state(state, config)
    phase0_record = state.get("phase0")
    freeze_sha = _sha256_file(freeze_path)
    if not isinstance(phase0_record, Mapping) or (
        phase0_record.get("path") != TOP40_V2_LAYOUT.phase0_freeze_path
        or phase0_record.get("sha256") != freeze_sha
    ):
        raise ValueError("V2 run state differs from phase0_freeze.json")
    canonical_manifest_path = root / str(freeze["shared_snapshot_manifest_path"])
    if manifest_path.resolve() != canonical_manifest_path.resolve():
        raise ValueError("V2 runner manifest differs from the Phase-0 shared manifest")
    if freeze["shared_snapshot_manifest_sha256"] != hashlib.sha256(
        manifest_text.encode("utf-8")
    ).hexdigest():
        raise ValueError("V2 shared snapshot manifest changed after Phase 0")
    if freeze["config_sha256"] != config.sha256:
        raise ValueError("V2 config changed after Phase 0")
    ...
    if not isinstance(frozen_files, Mapping) or set(frozen_files) != set(
        tournament_contract.PHASE0_FROZEN_FILES
    ):
        raise ValueError("V2 Phase-0 frozen_files map is incomplete or unexpected")
    common_commit = freeze["common_freeze_commit"]
    if not isinstance(common_commit, str) or not re.fullmatch(r"[0-9a-f]{40}", common_commit):
        raise ValueError("V2 Phase-0 common commit is invalid")
    common_exists = subprocess.run(
        ["git", "cat-file", "-e", f"{common_commit}^{{commit}}"],
        cwd=root,
        check=False,
        capture_output=True,
    )
    if common_exists.returncode:
        raise ValueError("V2 Phase-0 common commit does not exist")
    for relative, expected_sha256 in frozen_files.items():
        if (
            ...
            raise ValueError("V2 Phase-0 frozen_files entry is malformed")
        pure_relative = PurePosixPath(relative)
        if pure_relative.is_absolute() or any(
            part in {"", ".", ".."} for part in pure_relative.parts
        ):
            raise ValueError("V2 Phase-0 frozen_files path is unsafe")
        path = (root / relative).resolve()
        if not path.is_relative_to(root) or not path.is_file() or path.is_symlink():
            raise ValueError(f"V2 Phase-0 frozen file is missing or unsafe: {relative}")
        if _sha256_file(path) != expected_sha256:
            raise ValueError(f"V2 Phase-0 frozen file changed: {relative}")
        ...
        if committed.returncode or hashlib.sha256(committed.stdout).hexdigest() != expected_sha256:
            raise ValueError(f"V2 Phase-0 frozen file differs from common commit: {relative}")
    manifest_at_freeze = subprocess.run(
        [
            "git",
            "show",
            f"{common_commit}:{freeze['shared_snapshot_manifest_path']}",
        ],
        cwd=root,
        check=False,
        capture_output=True,
    )
    if manifest_at_freeze.returncode or hashlib.sha256(manifest_at_freeze.stdout).hexdigest() != (
    ...
    if parents.returncode or parents.stdout.split() != [record_commit, common_commit]:
        raise ValueError("V2 Phase-0 record must be the direct child of the common commit")
    committed_freeze = subprocess.run(
        ["git", "show", f"{record_commit}:{TOP40_V2_LAYOUT.phase0_freeze_path}"],
        cwd=root,
        check=False,
        capture_output=True,
    )
    if committed_freeze.returncode or hashlib.sha256(committed_freeze.stdout).hexdigest() != (
        freeze_sha
    ):
        raise ValueError("V2 phase0_freeze.json differs from its first-add commit")
    ...
    if record_ancestor.returncode:
        raise ValueError("V2 Phase-0 record commit is not an ancestor of HEAD")
    common_ancestor = subprocess.run(
        ["git", "merge-base", "--is-ancestor", common_commit, "HEAD"],
        cwd=root,
        check=False,
        capture_output=True,
    )
    if common_ancestor.returncode:
        raise ValueError("V2 Phase-0 common commit is not an ancestor of HEAD")
    return True
...
def _validate_snapshot_bounds(
    ...
    hard_end_exclusive = pd.Timestamp(config["data"]["hard_end_exclusive"], tz="UTC")
    bars = snapshot.bars
...
def _canonical_daily_returns(
    ...
    expected = pd.date_range(
        authorized.replay_start, authorized.score_end_inclusive, freq="1D", tz="UTC"
    )
...
def _btc_daily_returns(
    ...
    warmup_start = pd.Timestamp(config["data"]["warmup_start"], tz="UTC")
    score_end = pd.Timestamp(authorized.score_end_inclusive, tz="UTC")
...
def _publish_artifacts(
    ...
    *,
    ...
    try:
        ...
        raise
    return {
        name: f"{output_relative}/{filename}"
        for name, filename in files.items()
    }


def _stage_output_relative(team_id: str, stage: str) -> str:
    if stage == "development":
