"""Canonical, content-addressed source archives for Top40 V3 candidates.
...
def bundle_fingerprint(entries: Iterable[Mapping[str, object]]) -> str:
    """Return the V3 source-bundle digest for a stable ordered file manifest."""
...
def _require_identifier(value: object, label: str) -> str:
    if not isinstance(value, str) or _IDENTIFIER.fullmatch(value) is None:
        raise SourceArchiveError(f"{label} must use the lowercase V3 identifier grammar")
    return value
...
def _decode_archive(payload: bytes, *, relative_path: str, sha256: str) -> SourceArchive:
    ...
    if value["schema_version"] != SCHEMA_VERSION:
        raise SourceArchiveError("source archive schema_version is not V3")
    ...
    required_prefix = f"tournament/top40-v3/teams/{team_id}"
    if candidate_root != required_prefix and not candidate_root.startswith(required_prefix + "/"):
        raise SourceArchiveError("candidate_root is outside its V3 team namespace")
    ...
    source_bundle_sha256 = _require_hash(
        value["source_bundle_sha256"], "source_bundle_sha256"
    )
    ...
    for raw_file in raw_files:
        ...
        try:
            ...
            raise SourceArchiveError(f"source archive file is not canonical base64: {path}") from exc
...
def _resolved_archive_path(
    ...
    *,
    ...
    if not relative.startswith(ARCHIVE_NAMESPACE + "/"):
        raise SourceArchiveError("source_archive_path is outside the V3 archive namespace")
