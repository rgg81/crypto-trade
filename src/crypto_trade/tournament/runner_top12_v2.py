"""Cutoff-aware, strategy-neutral runner for one isolated Top-12 V2 team."""
...
import crypto_trade.tournament.top12_v2 as tournament_contract
from crypto_trade.tournament import source_archive_top12_v2, top12_universe_v1
...
_EVALUATOR_AUTHORITY_PATHS = (
    TOP12_V2_LAYOUT.contract_source,
...
def run_team(
    ...
    *,
    ...
    archive = source_archive_top12_v2.read_source_archive(
        root_path,
...
def _run_team_impl(
    ...
    *,
    ...
    _authorization: object | None = None,
    _output_relative: str | None = None,
    _source_archive: source_archive_top12_v2.SourceArchive | None = None,
) -> TeamWindowRunResult:
    ...
    if _authorization is not _ORGANIZER_RUN_AUTHORIZATION:
        raise PermissionError(
            ...
            "use scripts/top12_v2_tournament.py"
        )
    if _source_archive is None:
        raise ValueError("Top-12 V2 runner requires a verified immutable source archive")
    root_path = Path(root).resolve()
...
def _resolve_team_entrypoint(root: Path, team_id: str, entrypoint: str | Path) -> tuple[Path, str]:
    ...
    required_prefix = f"{TOP12_V2_LAYOUT.tournament_root}/teams/{team_id}/"
...
def _resolve_team_risk_policy(root: Path, team_id: str, entrypoint: Path) -> Path:
    ...
    raw_team_root = root / TOP12_V2_LAYOUT.team_root(team_id)
    if _has_symlink_component(root, raw_team_root):
...
def _risk_policy_from_archive(
    archive: source_archive_top12_v2.SourceArchive,
):
...
class SourceBundleCapture:
    ...
    entrypoint: str
    sha256: str
    files: tuple[source_archive_top12_v2.SourceFile, ...]
...
def _team_tree_fingerprint(files: Sequence[_TeamTreeFile]) -> str:
    ...
    return source_archive_top12_v2.bundle_fingerprint(entries)
...
def _candidate_source_boundary(
    ...
    team_root = (root / TOP12_V2_LAYOUT.team_root(team_id)).resolve()
    candidate_root = entrypoint_path.parent
...
def capture_source_bundle(
    ...
    captured: list[source_archive_top12_v2.SourceFile] = []
    for item in before:
...
def _stage_archived_source_bundle(
    destination: Path,
    files: Sequence[source_archive_top12_v2.SourceFile],
    *,
    ...
    fingerprint = source_archive_top12_v2.bundle_fingerprint(manifest)
...
def _launch_strategy_worker(
    ...
    archived_files: Sequence[source_archive_top12_v2.SourceFile] | None = None,
    ...
    sandbox = tempfile.TemporaryDirectory(prefix=f"top12-v2-{team_id}-")
...
def _authorized_window(config: Mapping[str, Any], stage: str) -> AuthorizedWindow:
    ...
    if stage in {"is", "is_confirmation", "historical_oos"}:
        score_start = str(splits[stage]["start"])
        end_exclusive = str(splits[stage]["end_exclusive"])
    else:
        raise ValueError("runner stage must be is, is_confirmation, or historical_oos")
...
def _stage_output_prefix(team_id: str, stage: str) -> str:
    ...
    if stage == "is":
        return f"{TOP12_V2_LAYOUT.reports_root}/is/{team_id}/"
    if stage == "is_confirmation":
        return f"{TOP12_V2_LAYOUT.tournament_root}/private/is-confirmation/{team_id}/"
    if stage == "historical_oos":
        return f"{TOP12_V2_LAYOUT.tournament_root}/private/historical-oos/{team_id}/"
    raise ValueError("runner stage must be is, is_confirmation, or historical_oos")
...
def _historical_publication_prefix(team_id: str) -> str:
    ...
    TOP12_V2_LAYOUT.require_team(team_id)
    return f"{TOP12_V2_LAYOUT.reports_root}/historical-oos/{team_id}/"
