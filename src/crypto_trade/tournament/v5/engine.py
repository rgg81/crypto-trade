"""Strategy-neutral, next-open Binance perpetual portfolio evaluator for Top-40 V5.

Forked from ``engine_v2`` at the R9 tip. V4-R2's completed record binds the hash of that module,
so V5 changes are made here rather than in place: editing the shared evaluator would rewrite bytes
a finished tournament's reproducibility claim depends on.

This module starts byte-for-byte equivalent in behaviour to its parent. Every V5 change is gated
behind an explicitly disabled-by-default flag on :class:`EvaluatorConfig`, and
``test_v5_engine_parity`` asserts that with those flags off the two evaluators produce identical
frames on the parent's own fixtures. That is what makes each subsequent change attributable to
exactly one flag instead of to the fork.
"""
...
class EvaluatorConfig:
    ...
    # A bar in which nothing traded is not executable. Binance publishes a placeholder bar for a
    # dormant contract with prices carried forward and zero volume; on such a bar the frozen close
    # and the still-moving index mark diverge without bound (25x for LUNA at its delisting, 989x
    # for SXP). Sizing quantity on the mark and settling at the close then produces a loss many
...
def generate_targets(
    ...
    *,
    ...
    # Fillability shown to a strategy must match the fillability the evaluator will enforce,
    # otherwise a strategy is offered a symbol it cannot trade and its target is rejected.
    executable = frame[frame["quote_volume"] > 0.0] if require_traded_bar else frame
    symbols_with_open = {
        pd.Timestamp(open_time): frozenset(group["symbol"])
        for open_time, group in executable.groupby("open_time", observed=True, sort=False)
    }
...
def evaluate_targets(
    ...
    *,
    ...
    for index, fill_time in enumerate(evaluation_times):
        ...
        current_volume = quote_volume.loc[fill_time].reindex(symbols)
        executable_now = current_open.notna()
        if cfg.require_traded_bar_to_fill:
            executable_now &= current_volume.fillna(0.0) > 0.0
        fillable = set(current_open[executable_now].index)
        ...
        if risk_policy is not None:
            ...
            policy_current_drawdown = max(0.0, 1.0 - equity_after_boundary_funding / peak_equity)
            policy_annualized_volatility = _past_annualized_volatility(
                return_rows,
                lookback_days=risk_policy.volatility_target.lookback_days,
                interval_hours=cfg.interval_hours,
            )
            position_returns = _position_returns(quantities, entry_prices, current_mark)
            ...
            turnover_budget = (
                ...
                else policy_decision.maximum_one_way_turnover * equity_after_boundary_funding
            )
            ...
            for symbol in policy_target_quantities[policy_target_quantities.ne(quantities)].index:
                ...
                policy_reasons_by_symbol[symbol] = tuple(dict.fromkeys((*prior, *additional)))

            policy_requested_quantity = policy_target_quantities - quantities
            policy_requested_notional = policy_requested_quantity.mul(current_open).fillna(0.0)
        ...
        requested_notional = float(
            policy_requested_notional.abs().sum() + requested_notional_by_symbol.abs().sum()
        )
        ...
        for symbol in policy_filled_notional[policy_filled_notional.ne(0.0)].index:
            ...
            event_rows.append(
                _risk_policy_event(
                    ...
                    fee=(
                        abs(symbol_notional) * cfg.taker_fee_bps_per_side / 10_000 * cost_multiplier
                    ),
                    slippage=(
                        abs(symbol_notional) * cfg.slippage_bps_per_side / 10_000 * cost_multiplier
                    ),
                )
            )
        ...
        current_bar_executed = (
            policy_filled_notional.abs() + filled_notional.abs() + risk_reduction_notional.abs()
        )
...
def _position_returns(
    ...
    for symbol in quantities[quantities.ne(0.0)].index:
        ...
        result[symbol] = math.copysign(1.0, float(quantities[symbol])) * (mark / basis - 1.0)
    return result
...
def _validate_filled_prices(
    ...
    if missing.any():
        raise ValueError(
            f"missing trade price for {label} symbols: {sorted(filled_notional[missing].index)}"
        )
    ...
    for symbol in fill_quantities[fill_quantities.ne(0.0)].index:
        ...
        if old_quantity == 0.0 or math.copysign(1.0, old_quantity) != math.copysign(
            ...
            if not math.isfinite(old_entry) or old_entry <= 0.0:
                raise ValueError(f"missing authoritative entry basis for {symbol}")
            updated_entries[symbol] = (abs(old_quantity) * old_entry + abs(delta) * price) / abs(
                new_quantity
            )
