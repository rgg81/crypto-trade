"""Cutoff-aware, strategy-neutral runner for one isolated Top-12 V1 team."""
...
import crypto_trade.tournament.top12_v1 as tournament_contract
from crypto_trade.tournament import source_archive_top12_v1, top12_universe_v1
...
_EVALUATOR_AUTHORITY_PATHS = (
    ...
    "src/crypto_trade/tournament/scoring_top12_v1.py",
    "src/crypto_trade/tournament/source_archive_top12_v1.py",
...
def _pure_crypto_preflight(root: Path, config: Mapping[str, Any]) -> str:
    ...
    for path_field, hash_field in (
        ...
        ("classification_dependency_path", "classification_dependency_sha256"),
        ("builder_path", "builder_sha256"),
        ("build_record_path", "build_record_sha256"),
    ):
    ...
    report_bytes = top12_universe_v1.audit_report_bytes(root)
    ...
    if (
        ...
        or not isinstance(report.get("classification"), Mapping)
        or report["classification"].get("forbidden_exposure_violations")
        != authority["expected_violations"]
        or report.get("membership_rows") != authority["expected_membership_rows"]
        or report.get("membership_symbols")
        != authority["expected_distinct_membership_symbols"]
        or report.get("weekly_reconstitutions")
        != authority["expected_weekly_reconstitutions"]
    ):
        raise ValueError(
...
def run_team(
    ...
    *,
    ...
    archive = source_archive_top12_v1.read_source_archive(
        root_path,
...
def _run_team_impl(
    ...
    *,
    ...
    _authorization: object | None = None,
    _output_relative: str | None = None,
    _source_archive: source_archive_top12_v1.SourceArchive | None = None,
) -> TeamWindowRunResult:
    ...
    if _authorization is not _ORGANIZER_RUN_AUTHORIZATION:
        raise PermissionError(
            ...
            "use scripts/top12_v1_tournament.py"
        )
    if _source_archive is None:
        raise ValueError("Top-12 V1 runner requires a verified immutable source archive")
    root_path = Path(root).resolve()
    ...
    expected_manifest_path = (root_path / "tournament/top12/data_manifest.json").resolve()
    if canonical_manifest_path != expected_manifest_path:
        raise ValueError(
            "manifest_path must be the shared immutable tournament/top12/data_manifest.json"
        )
...
def _resolve_team_entrypoint(root: Path, team_id: str, entrypoint: str | Path) -> tuple[Path, str]:
    ...
    required_prefix = f"{TOP12_V1_LAYOUT.tournament_root}/teams/{team_id}/"
...
def _resolve_team_risk_policy(root: Path, team_id: str, entrypoint: Path) -> Path:
    ...
    raw_team_root = root / TOP12_V1_LAYOUT.team_root(team_id)
    if _has_symlink_component(root, raw_team_root):
...
def _risk_policy_from_archive(
    archive: source_archive_top12_v1.SourceArchive,
):
...
class SourceBundleCapture:
    ...
    entrypoint: str
    sha256: str
    files: tuple[source_archive_top12_v1.SourceFile, ...]
...
def _team_tree_fingerprint(files: Sequence[_TeamTreeFile]) -> str:
    ...
    return source_archive_top12_v1.bundle_fingerprint(entries)
...
def _candidate_source_boundary(
    ...
    team_root = (root / TOP12_V1_LAYOUT.team_root(team_id)).resolve()
    candidate_root = entrypoint_path.parent
...
def capture_source_bundle(
    ...
    captured: list[source_archive_top12_v1.SourceFile] = []
    for item in before:
...
def _stage_archived_source_bundle(
    destination: Path,
    files: Sequence[source_archive_top12_v1.SourceFile],
    *,
    ...
    fingerprint = source_archive_top12_v1.bundle_fingerprint(manifest)
...
def _launch_strategy_worker(
    ...
    archived_files: Sequence[source_archive_top12_v1.SourceFile] | None = None,
    ...
    sandbox = tempfile.TemporaryDirectory(prefix=f"top12-v1-{team_id}-")
...
def _load_verified_snapshot(root: Path, manifest_path: Path) -> _SnapshotData:
    """Verify every manifest entry directly before loading the five Top-12 datasets."""
...
def _stage_output_prefix(team_id: str, stage: str) -> str:
    ...
    if stage == "is":
        return f"{TOP12_V1_LAYOUT.reports_root}/is/{team_id}/"
    if stage == "historical_oos":
        return f"{TOP12_V1_LAYOUT.tournament_root}/private/historical-oos/{team_id}/"
    raise ValueError("runner stage must be is or historical_oos")
...
def _historical_publication_prefix(team_id: str) -> str:
    ...
    TOP12_V1_LAYOUT.require_team(team_id)
    return f"{TOP12_V1_LAYOUT.reports_root}/historical-oos/{team_id}/"
