"""Organizer authority for the three-stage Top-12 V2 tournament."""
...
CONFIG_PATH = TOP12_V2_LAYOUT.config_path
...
def _result_lock(root: Path) -> Iterator[None]:
    path = _path(root, TOP12_V2_LAYOUT.result_lock_path)
    _ensure_directory(root, path.parent)
...
def _require_cpu_budget(root: Path) -> None:
    """Fail closed when an activated result command is not pinned to at most two CPUs."""

    if not _path(root, TOP12_V2_LAYOUT.activation_freeze_path).exists():
        return
    affinity = os.sched_getaffinity(0) if hasattr(os, "sched_getaffinity") else {0, 1}
    if len(affinity) > 2:
        raise OrchestratorError(
            "activated result commands require CPU affinity capped at two CPUs (use taskset)"
        )


def _journal_path(root: Path) -> Path:
    return _path(root, TOP12_V2_LAYOUT.journal_path)
...
def _candidate_metadata(
    ...
    TOP12_V2_LAYOUT.require_team(team_id)
    entry = _path(root, entrypoint)
    team_root = _path(root, TOP12_V2_LAYOUT.team_root(team_id))
    try:
...
def _derive_authority(
    root: Path,
    loaded: top12_v2.LoadedV4Config,
    team_id: str,
    entrypoint: str,
    ...
    authority = CandidateAuthority(
        ...
        evaluator_sha256=runner_top12_v2._evaluator_authority_sha256(root),
    )
...
def activate(root: str | Path) -> Mapping[str, Any]:
    ...
    with _result_lock(root_path):
        top12_v2.load_config(root=root_path)
        journal_top12_v2.initialize(_journal_path(root_path))
        return activation_top12_v2.activate(root_path)


def validate(root: str | Path, *, require_activation: bool = True) -> Mapping[str, Any]:
    root_path = _safe_root(root)
    loaded = top12_v2.load_config(root=root_path)
    activation = activation_top12_v2.validate(root_path) if require_activation else None
    journal_path = _journal_path(root_path)
    state = (
        journal_top12_v2.read(journal_path)
        if journal_path.exists()
        else journal_top12_v2.replay_bytes(b"")
    )
    visible_head = state.head_sha256
    visible_records = state.record_count
    if state.confirmation is not None and state.confirmation_release is None:
        visible_head = str(state.confirmation_record_sha256)
        visible_records = next(
            int(record["sequence"])
            for record in state.records
            if record["record_sha256"] == state.confirmation_record_sha256
        )
    ...
    return {
        "ok": True,
        "tournament": TOP12_V2_LAYOUT.name,
        "config_sha256": loaded.sha256,
...
def status(root: str | Path) -> Mapping[str, Any]:
    root_path = _safe_root(root)
    loaded = top12_v2.load_config(root=root_path)
    activated = _path(root_path, TOP12_V2_LAYOUT.activation_freeze_path).exists()
    journal_path = _journal_path(root_path)
    state = (
        journal_top12_v2.read(journal_path)
        if journal_path.exists()
    ...
    elif state.confirmation is not None and state.selection is None:
        phase = "is-confirmation-sealed-running"
    ...
    if state.confirmation is None:
        ...
        result["confirmation"] = {
            ...
            "status": "sealed until selection freeze and atomic release",
        }
...
def _run_team(
    ...
    *,
    stage: str,
    output: str,
) -> runner_top12_v2.TeamWindowRunResult:
    return runner_top12_v2.run_team(
        root,
        authority.team_id,
        ...
        DATA_MANIFEST_PATH,
        stage=stage,
        _authorization=runner_top12_v2._ORGANIZER_RUN_AUTHORIZATION,
...
def _close_interrupted_is_requests(root: Path) -> journal_top12_v2.JournalState:
    """Convert requests left pending by process death into consumed failed trials."""

    state = journal_top12_v2.read(_journal_path(root))
    pending = [
    ...
    return journal_top12_v2.read(_journal_path(root))
...
def run_is(
    ...
    *,
    ...
    with _result_lock(root_path):
        ...
        activation_top12_v2.validate(root_path, verify_universe_snapshot=False)
        loaded = top12_v2.load_config(root=root_path)
        ...
        if state.trials_by_team[team_id] >= maximum:
            raise OrchestratorError("team exhausted its sixteen accepted trials")
        ...
        research = loaded.raw["research"]
        tags = set(metadata["tags"])
        first_neighborhood = int(research["neighborhood_first_trial"])
        last_neighborhood = int(research["neighborhood_last_required_trial"])
        if trial < first_neighborhood and "local-neighborhood" in tags:
            raise OrchestratorError(
                "local-neighborhood evidence is reserved for trials 9 through 13"
            )
        if first_neighborhood <= trial <= last_neighborhood:
            if (
                "local-neighborhood" not in tags
                or metadata["neighborhood_id"] is None
                or not metadata["neighborhood_coordinates"]
            ):
                raise OrchestratorError(
                    "trials 9 through 13 must be preregistered neighborhood points"
                )
        if trial > last_neighborhood and research["post_neighborhood_tag"] not in tags:
            raise OrchestratorError("post-neighborhood trials require the frozen repair tag")
        if trial == int(research["core_evidence_deadline_trial"]):
            core_tags = set(research["required_certificate_tags"]) - {"local-neighborhood"}
            observed = set(tags)
            for request in state.is_requests.values():
                if request["payload"]["team_id"] == team_id:
                    observed.update(request["payload"]["metadata"]["tags"])
            missing = sorted(core_tags - observed)
            if missing:
                raise OrchestratorError(
                    f"trial 8 must complete preregistered core evidence: {', '.join(missing)}"
                )
        run_id = f"{team_id.replace('-', '')}-is-{trial:02d}-{authority.source_bundle_sha256[:12]}"
        output = f"{TOP12_V2_LAYOUT.reports_root}/is/{team_id}/{run_id}"
        request = journal_top12_v2.append(
        ...
        try:
            ...
            summary = scoring_top12_v2.summarize_run(
                root_path,
                result,
                loaded.raw,
                trial_count=trial,
            ...
            terminal = journal_top12_v2.append(
                _journal_path(root_path),
                "is_succeeded",
                {
                    "team_id": team_id,
...
def _candidate_success(
    state: journal_top12_v2.JournalState, team_id: str, candidate_id: str
...
def _certificate(
    ...
    relative: str,
    *,
    state: journal_top12_v2.JournalState,
    config: Mapping[str, Any],
    team_id: str,
    candidate_id: str,
    nominated_request_hash: str,
) -> tuple[Mapping[str, Any], str, dict[str, Any]]:
    required_prefix = f"{TOP12_V2_LAYOUT.tournament_root}/certificates/{team_id}/"
    ...
    core_deadline = int(research["core_evidence_deadline_trial"])
    first_neighborhood = int(research["neighborhood_first_trial"])
    last_neighborhood = int(research["neighborhood_last_required_trial"])
    core_tags = set(required_tags) - {"local-neighborhood"}
    early_tags: set[str] = set()
    for request in team_requests.values():
        event = request["payload"]
        trial_number = int(event["trial_number"])
        tags = set(event["metadata"]["tags"])
        if trial_number <= core_deadline:
            early_tags.update(tags)
        if first_neighborhood <= trial_number <= last_neighborhood:
            if "local-neighborhood" not in tags:
                raise OrchestratorError("protected neighborhood trials lost their evidence tag")
        elif "local-neighborhood" in tags:
            raise OrchestratorError("local-neighborhood evidence is outside trials 9 through 13")
        if trial_number > last_neighborhood and research["post_neighborhood_tag"] not in tags:
            raise OrchestratorError("post-neighborhood trial lacks its preregistered repair tag")
    missing_early = sorted(core_tags - early_tags)
    if missing_early:
        raise OrchestratorError(
            f"core evidence was not completed by trial 8: {', '.join(missing_early)}"
        )
    ...
    neighborhood_trials = {
        int(team_requests[request_hash]["payload"]["trial_number"])
        for request_hash in neighborhood_hashes
    }
    required_trials = set(range(first_neighborhood, last_neighborhood + 1))
    if neighborhood_trials != required_trials or len(neighborhood_hashes) != len(required_trials):
        raise OrchestratorError("certificate must use exactly protected trials 9 through 13")
...
def _write_nomination_registry(root: Path, state: journal_top12_v2.JournalState) -> None:
    registry = {
        "schema_version": "top12-v2-nomination-registry-v1",
    ...
    _write_atomic(
        root,
        TOP12_V2_LAYOUT.nomination_registry_path,
        _pretty(registry),
        replace=True,
    )
...
def nominate(
    ...
    root_path = _safe_root(root)
    with _result_lock(root_path):
        activation_top12_v2.validate(root_path)
        loaded = top12_v2.load_config(root=root_path)
        state = _close_interrupted_is_requests(root_path)
        if state.confirmation is not None:
            raise OrchestratorError("IS is closed")
        ...
        if state.trials_by_team.get(team_id, 0) < minimum:
            raise OrchestratorError("team must consume at least thirteen structured trials")
        ...
        selection = scoring_top12_v2.assess_is(
            summary,
            loaded.raw,
        ...
        nomination = {
            "schema_version": "top12-v2-nomination-v1",
            "team_id": team_id,
            "candidate_id": candidate_id,
            "trial_count": state.trials_by_team[team_id],
        ...
        nomination_path = (
            f"{TOP12_V2_LAYOUT.tournament_root}/nominations/{team_id}-{candidate_id}.json"
        )
        ...
        record = journal_top12_v2.append(
            _journal_path(root_path),
            "nominated",
            {
                "team_id": team_id,
        ...
        _write_nomination_registry(root_path, journal_top12_v2.read(_journal_path(root_path)))
        return {
            "ok": True,
            "team_id": team_id,
            "candidate_id": candidate_id,
...
def retire(root: str | Path, team_id: str, *, reason: str) -> Mapping[str, Any]:
    ...
    with _result_lock(root_path):
        ...
        if state.trials_by_team[team_id] < minimum:
            raise OrchestratorError(
                "retirement requires at least thirteen accepted research trials"
            )
...
def _confirmation_freeze(
    root: Path, state: journal_top12_v2.JournalState
) -> Mapping[str, Any]:
    if state.confirmation is None:
        raise OrchestratorError("IS confirmation has not been frozen")
    relative = str(state.confirmation["confirmation_freeze_path"])
    payload = _stable_authority_bytes(_path(root, relative))
    if _sha256(payload) != state.confirmation["confirmation_freeze_sha256"]:
        raise OrchestratorError("confirmation freeze changed after journal binding")
    result = _strict_object_bytes(payload, relative)
    if [row["team_id"] for row in result["nominees"]] != list(
        state.confirmation["nominees"]
    ):
        raise OrchestratorError("confirmation freeze nominee order differs from the journal")
    return result


def _confirmation_selection_key(row: Mapping[str, Any]) -> tuple[Any, ...]:
    confirmation = row["confirmation"]["ranking_vector"]
    development = row["selection"]["ranking_vector"]
    confirmation_double = float(confirmation["double_cost_sharpe"])
    development_median = float(development["median_fold_double_cost_sharpe"])
    return (
        -min(confirmation_double, development_median),
        -confirmation_double,
        -float(development["worst_fold_double_cost_sharpe"]),
        -float(confirmation["base_annualized_return"]),
        str(row["team_id"]),
    )


def close_is(root: str | Path) -> Mapping[str, Any]:
    """Freeze nominees, observe sealed confirmation once, and freeze final OOS entrants."""

    root_path = _safe_root(root)
    with _result_lock(root_path):
        _require_cpu_budget(root_path)
        activation = activation_top12_v2.validate(root_path, verify_universe_snapshot=False)
        loaded = top12_v2.load_config(root=root_path)
        state = _close_interrupted_is_requests(root_path)
        _write_nomination_registry(root_path, state)
        if state.selection is not None:
            freeze = _selection_freeze(root_path, state)
            if state.confirmation_release is None:
                confirmation = _confirmation_freeze(root_path, state)
                staging, release, manifest_sha256 = _confirmation_release_bundle(
                    root_path, confirmation, freeze, state
                )
                ...
                journal_top12_v2.append(
                    ...
                    "confirmation_release_authorized",
                    {
                        ...
                        "terminal_record_sha256s": [
                            state.confirmation_terminals[team_id]["record_sha256"]
                            for team_id in state.confirmation["nominees"]
                        ],
                ...
                state = journal_top12_v2.read(_journal_path(root_path))
            promoted = _promote_authorized(root_path, state.confirmation_release)
            return {
                "ok": True,
                "already_frozen": True,
                "advancing": [str(row["team_id"]) for row in freeze["advancing"]],
                "selection_freeze_path": state.selection["selection_freeze_path"],
                "confirmation_release_path": promoted["release_path"],
            }
        if set(state.nominations) | set(state.retired) != set(TOP12_V2_LAYOUT.team_ids):
            unresolved = sorted(
                set(TOP12_V2_LAYOUT.team_ids) - set(state.nominations) - set(state.retired)
            )
            raise OrchestratorError(f"IS cannot close while teams are unresolved: {unresolved}")

        # Freeze the complete qualified population before any confirmation data can be opened.
        if state.confirmation is None:
            ...
            population.sort(key=lambda row: scoring_top12_v2.is_ranking_key(row["selection"]))
            confirmation = {
                "schema_version": "top12-v2-confirmation-freeze-v1",
                "tournament": TOP12_V2_LAYOUT.name,
                "input_journal_head_sha256": state.head_sha256,
                "config_sha256": loaded.sha256,
                "activation_record_sha256": activation["record_sha256"],
                "window": dict(loaded.raw["splits"]["is_confirmation"]),
                "nominees": [
                    {
            ...
            payload = _pretty(confirmation)
            digest = _write_atomic(
                root_path,
                TOP12_V2_LAYOUT.confirmation_freeze_path,
                payload,
                replace=False,
            )
            journal_top12_v2.append(
                ...
                "confirmation_frozen",
                {
                    ...
                    "nominees": [str(row["team_id"]) for row in confirmation["nominees"]],
                    "confirmation_freeze_path": TOP12_V2_LAYOUT.confirmation_freeze_path,
                    "confirmation_freeze_sha256": digest,
                },
            )
            state = journal_top12_v2.read(_journal_path(root_path))
        confirmation = _confirmation_freeze(root_path, state)

        # Batch-accept every identity before the first sealed snapshot read.
        for nominee in confirmation["nominees"]:
            team_id = str(nominee["team_id"])
            if team_id in state.confirmation_requests:
                continue
            authority = CandidateAuthority(**dict(nominee["authority"]))
            run_id = (
                f"{team_id.replace('-', '')}-confirmation-"
                f"{authority.source_bundle_sha256[:12]}"
            )
            output = (
                f"{TOP12_V2_LAYOUT.tournament_root}/private/is-confirmation/"
                f"{team_id}/{run_id}"
            )
            journal_top12_v2.append(
                _journal_path(root_path),
                "confirmation_accepted",
                {
                    ...
                    "candidate_id": authority.candidate_id,
                    "run_id": run_id,
                    "confirmation_record_sha256": state.confirmation_record_sha256,
                    "output_path": output,
                },
            )
            state = journal_top12_v2.read(_journal_path(root_path))
        expected = {str(row["team_id"]) for row in confirmation["nominees"]}
        if set(state.confirmation_requests) != expected:
            raise OrchestratorError("not every nominee identity was durably accepted")

        # A started observation that lost its process is a DNF, never a retry.
        for team_id in list(state.confirmation_starts):
            if team_id in state.confirmation_terminals:
                continue
            request = state.confirmation_requests[team_id]
            event = request["payload"]
            journal_top12_v2.append(
                _journal_path(root_path),
                "confirmation_failed",
        ...
        for nominee in confirmation["nominees"]:
            ...
            try:
                authority = _validate_authority_current(root_path, loaded, nominee["authority"])
                result = _run_team(
                    root_path,
                    authority,
                    stage="is_confirmation",
                ...
                summary = scoring_top12_v2.summarize_run(
                    ...
                    result,
                    loaded.raw,
                    trial_count=int(nominee["trial_count"]),
                )
                summary["candidate_id"] = authority.candidate_id
                summary["run_id"] = event["run_id"]
                summary_path = f"{event['output_path']}/summary.json"
                summary_sha256 = _write_atomic(
                    root_path, summary_path, _pretty(summary), replace=False
                )
                journal_top12_v2.append(
                    _journal_path(root_path),
        ...
        nominees_by_team = {str(row["team_id"]): row for row in confirmation["nominees"]}
        for team_id in state.confirmation["nominees"]:
            nomination = _verified_nomination(root_path, state.nominations[team_id])
            terminal = state.confirmation_terminals[team_id]
            summary = None
            if terminal["event_type"] == "confirmation_succeeded":
                summary = _verified_summary(root_path, terminal)
                if summary.get("stage") != "is_confirmation":
                    raise OrchestratorError("confirmation terminal summary has the wrong stage")
            row = {
                "team_id": team_id,
                "candidate_id": nomination["candidate_id"],
                "selection": nomination["selection"],
                "confirmation": summary["confirmation"] if summary is not None else None,
                "status": "succeeded" if summary is not None else "DNF",
            }
            population_rows.append(row)
            if summary is not None and summary["confirmation"]["eligible"]:
                eligible.append(row)
        eligible.sort(key=_confirmation_selection_key)
        advance_count = int(loaded.raw["selection"]["ranking"]["advance_count"])
        finalists = eligible[:advance_count]
        advancing = [str(row["team_id"]) for row in finalists]
        finalist_nominations = [
            _verified_nomination(root_path, state.nominations[row["team_id"]])
        ...
        freeze = {
            "schema_version": "top12-v2-selection-freeze-v1",
            "tournament": TOP12_V2_LAYOUT.name,
            "input_journal_head_sha256": state.head_sha256,
            "config_sha256": loaded.sha256,
            "activation_record_sha256": activation["record_sha256"],
            ...
            "advancing": [
                {
                    ...
                    "team_id": row["team_id"],
                    "candidate_id": row["candidate_id"],
                    "authority": nominees_by_team[row["team_id"]]["authority"],
                    "trial_count": nominees_by_team[row["team_id"]]["trial_count"],
                    "nomination_path": state.nominations[row["team_id"]]["payload"][
                        "nomination_path"
                    ],
                    "nomination_sha256": state.nominations[row["team_id"]]["payload"][
                    ...
                    "development_selection": row["selection"],
        ...
        freeze_sha256 = _write_atomic(
            root_path,
            TOP12_V2_LAYOUT.selection_freeze_path,
            freeze_payload,
            replace=False,
        )
        record = journal_top12_v2.append(
        ...
        manifest = _strict_object(root_path, f"{staging}/manifest.json")
        authorization = journal_top12_v2.append(
            _journal_path(root_path),
            "confirmation_release_authorized",
            {
...
def _selection_freeze(root: Path, state: journal_top12_v2.JournalState) -> Mapping[str, Any]:
...
def _historical_summary_for_team(
    root: Path, state: journal_top12_v2.JournalState, team_id: str
...
def _verified_stage_packet(
    ...
    *,
    ...
    elif stage == "is_confirmation":
        request = state.confirmation_requests[team_id]["payload"]
    else:
        raise OrchestratorError("release packet stage is not authorized")
    ...
    if summary_relative != f"{output_relative}/summary.json":
        raise OrchestratorError("sealed summary is outside its accepted output directory")
    source = _path(root, output_relative)
    if not source.is_dir() or source.is_symlink():
        raise OrchestratorError("private sealed artifact directory is missing or unsafe")
    runner = summary.get("runner")
    if (
        not isinstance(runner, Mapping)
        or runner.get("team_id") != team_id
        or runner.get("stage") != stage
        or runner.get("output_dir") != output_relative
    ):
        raise OrchestratorError("sealed summary runner identity is invalid")
    ...
    if (
        ...
        raise OrchestratorError("sealed summary artifact authority is incomplete")
    expected_names = set(_RELEASE_ARTIFACT_FILES.values()) | {"summary.json"}
    actual_names: set[str] = set()
    for path in source.iterdir():
        if path.is_symlink() or not path.is_file():
            raise OrchestratorError("private sealed directory contains an unsafe node")
        actual_names.add(path.name)
    if actual_names != expected_names:
        raise OrchestratorError("private sealed directory differs from the exact allowlist")
    ...
    for logical_name, filename in _RELEASE_ARTIFACT_FILES.items():
        ...
        if (
            ...
            raise OrchestratorError("sealed artifact authority is malformed")
        ...
        if (
            ...
            raise OrchestratorError("sealed artifact changed after terminal success")
...
def _ensemble_release(
    ...
    packet: dict[str, Any] = {
        "schema_version": "top12-v2-historical-oos-ensemble-v1",
...
def _confirmation_release_bundle(
    root: Path,
    confirmation: Mapping[str, Any],
    selection: Mapping[str, Any],
    state: journal_top12_v2.JournalState,
) -> tuple[str, str, str]:
    """Stage every confirmation result together; nothing is public before selection freezes."""

    reports_root = _path(root, TOP12_V2_LAYOUT.reports_root)
    _ensure_directory(root, reports_root)
    staging_relative = f"{TOP12_V2_LAYOUT.reports_root}/.is-confirmation-staging"
    staging = _path(root, staging_relative)
    release_relative = f"{TOP12_V2_LAYOUT.reports_root}/is-confirmation"
    release = _path(root, release_relative)
    if release.exists():
        raise OrchestratorError(
            "IS confirmation release path already exists without durable authorization"
        )
    temporary = Path(tempfile.mkdtemp(prefix=".is-confirmation-build-", dir=reports_root))
    advancing = {str(row["team_id"]) for row in selection["advancing"]}
    try:
        rows: list[dict[str, Any]] = []
        for nominee in confirmation["nominees"]:
            team_id = str(nominee["team_id"])
            terminal = state.confirmation_terminals[team_id]
            destination = temporary / "teams" / team_id
            summary = None
            if terminal["event_type"] == "confirmation_succeeded":
                summary = _verified_summary(root, terminal)
                packet = _verified_stage_packet(
                    root,
                    state,
                    team_id,
                    terminal,
                ...
                _copy_verified_packet(packet, destination)
            else:
                _write_stage_file(
                    destination / "result.json",
                    _pretty(
                        {
                            "schema_version": "top12-v2-is-confirmation-dnf-v1",
                            "team_id": team_id,
            ...
            rows.append(
                {
                    "development_rank": nominee["development_rank"],
                    "team_id": team_id,
                    "candidate_id": nominee["candidate_id"],
                    "status": "succeeded" if summary is not None else "DNF",
                    "confirmation": summary["confirmation"] if summary is not None else None,
                    "advanced_to_oos": team_id in advancing,
                }
            )
        ...
        manifest = {
            "schema_version": "top12-v2-is-confirmation-release-v1",
            "evidence_label": "sealed-selection-confirmation-not-final-oos",
            "window": {
        ...
        if staging.exists() and not staging.is_dir():
            raise OrchestratorError("IS confirmation staging path is not a directory")
        if staging.exists():
...
def _release_bundle(
    ...
    reports_root = _path(root, TOP12_V2_LAYOUT.reports_root)
    _ensure_directory(root, reports_root)
    staging_relative = f"{TOP12_V2_LAYOUT.reports_root}/.historical-oos-staging"
    staging = _path(root, staging_relative)
    release_relative = f"{TOP12_V2_LAYOUT.reports_root}/historical-oos"
    release = _path(root, release_relative)
    if release.exists():
    ...
    try:
        ...
        for finalist in selection["advancing"]:
            ...
            if terminal["event_type"] == "historical_succeeded":
                ...
                _write_stage_file(
                    ...
                    _pretty(
                        {
                            "schema_version": "top12-v2-historical-oos-dnf-v1",
                            "team_id": team_id,
        ...
        eligible.sort(
            key=lambda packet: scoring_top12_v2.historical_ranking_key(
                packet["championship"]
            )
        )
        winner = None
        ...
        bundle_sha256 = _sha256(_canonical({"files": file_entries}))
        manifest = {
            "schema_version": "top12-v2-historical-oos-release-v1",
...
def historical_release(root: str | Path) -> Mapping[str, Any]:
    ...
    with _result_lock(root_path):
        ...
        for finalist in selection["advancing"]:
            ...
            output = f"{TOP12_V2_LAYOUT.tournament_root}/private/historical-oos/{team_id}/{run_id}"
            journal_top12_v2.append(
                _journal_path(root_path),
                "historical_accepted",
                {
