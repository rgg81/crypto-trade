"""Binance wire parsing and append-invariant caching for the CUP-50 v2 paper desks.

Ported wholesale from cup20_desk.live_data rather than imported from it. cup20's desk is
live and monitored; an edition that imports a running desk's code inherits every edit made
to keep that desk alive, which is a coupling no amount of care survives. tournament.snapshot
stays shared because it carries no edition references and is genuinely common infrastructure.
"""
...
# hash-bound; `crypto_trade/cup50v2/` is, and nothing here touches it.
from crypto_trade.cup50v2.universe import MEMBERSHIP_COLUMNS
...
CACHE_SCHEMA_VERSION = "cup50v2-desk-cache-v1"
...
built by ``crypto_trade.cup50v2_desk.snapshot_forward`` out of the other four -- but it is written
...
def _snap_funding_intervals(intervals: pd.Series) -> pd.Series:
    ...
    **Why this exists, because it is not cosmetic.** ``funding_interval_hours`` reaches the sealed
    ...
    ``crypto_trade.cup50v2_desk.snapshot_forward._stack`` refuses ANY disagreement between two
    sources
