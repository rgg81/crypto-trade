...
from crypto_trade.cup50v2.config import IS_START, OOS_END
...
from crypto_trade.cup50v2_desk.snapshot_forward import build_forward_snapshot
from crypto_trade.cup50v2_desk.wire import FrameSchema, append_frame
...
def replay_desk(desk: Desk, snapshot: Any, *, root: Path) -> Any:
    """Replay this desk's frozen lane at its own centre.

    The centre comes from the desk record and is checked against the nomination by verify_lineage,
    so a desk cannot quietly replay a lane at parameters the lane never froze.
    """
    strategy = strategy_from_module(load_strategy_module(desk_bundle(desk, root) / "strategy.py"))
...
def persist_tick(
    ...
    *,
    ...
    paper = paper_root(desk, base)
    deployment = verify_deployment(desk, base)
    ...
    replay = replay_desk(desk, snapshot, root=base)
