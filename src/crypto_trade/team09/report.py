"""All-period reporting for the frozen Team 09 continuous replay."""
...
REPORT_TITLE = (
    "Team 09 — Continuous IS + Historical OOS "
    "(2020-02-03 through 2026-06-30)"
)
...
def load_released_daily_returns(root: str | Path | None = None) -> pd.Series:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"missing Team 09 released daily returns: {path}")
    actual_sha256 = sha256_file(path)
    if actual_sha256 != GOLDEN_DAILY_RETURNS_SHA256:
        raise RuntimeError(
            "Team 09 released daily-return drift: "
            f"{actual_sha256} != {GOLDEN_DAILY_RETURNS_SHA256}"
        )
    frame = pd.read_csv(path)
    if set(frame) != {"date", "net_return"}:
        raise ValueError(f"unexpected Team 09 daily-return schema: {list(frame)}")
    series = pd.Series(
...
def validate_all_period_returns(returns: pd.Series) -> None:
    ...
    if not isinstance(returns.index, pd.DatetimeIndex):
        raise TypeError("Team 09 daily returns require a DatetimeIndex")
    ...
    if not series.index.is_unique:
        raise ValueError("Team 09 daily returns contain duplicate dates")
    if not series.index.equals(expected):
        raise ValueError(
            "Team 09 daily returns must be one gap-free continuous stream from "
            f"{expected[0].date()} through {expected[-1].date()}"
        )
    values = series.to_numpy()
    if not np.isfinite(values).all() or (values <= -1.0).any():
        raise ValueError("Team 09 daily returns contain invalid values")
...
def period_statistics(returns: pd.Series) -> dict[str, object]:
    ...
    windows = {
        ...
        "historical_oos": returns.loc[returns.index >= IS_END_EXCLUSIVE],
    }
...
def generate_all_period_report(
    ...
    *,
    ...
    daily_path = destination / "team09-all-periods-daily-returns.csv"
    metrics_path = destination / "team09-all-periods-statistics.json"
    html_path = destination / "team09-all-periods-quantstats.html"

    daily = pd.DataFrame(
        {
            "date": returns.index.strftime("%Y-%m-%d"),
            "net_return": returns.to_numpy(),
            "period": np.where(returns.index < IS_END_EXCLUSIVE, "IS", "historical OOS"),
        }
    )
    ...
    payload = {
        "schema_version": "team09-all-periods-report-v1",
