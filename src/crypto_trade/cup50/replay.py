"""Continuous causal CUP-50 replay and execution."""
...
from crypto_trade.cup50.common_risk import common_risk_scalars
...
def load_strategy_module(path: str | Path) -> ModuleType:
    ...
    module = ModuleType(f"cup50_candidate_{hashlib.sha256(source.read_bytes()).hexdigest()[:16]}")
...
def apply_strategy_parameters(
    strategy: TargetStrategyV2, parameters: Mapping[str, object]
) -> None:
...
def generate_targets(
    ...
    *,
    ...
    for decision in decisions:
        ...
        if uses_funding:
            funding_start = int(
                funding_times.searchsorted(
                    decision - pd.Timedelta(days=history_days), side="left"
                )
            )
