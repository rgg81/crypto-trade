"""v3 regime features — Hurst, ATR percentile rank, BB width rank, CUSUM reset count.

Track-isolated copy of v2's regime.py. No imports from crypto_trade.features
or crypto_trade.features_v2 permitted in this file.

Also exports ``natr_21_raw`` helper used by the shared labeling code
(``LightGbmStrategy.atr_column``). That column is NOT in V3_FEATURE_COLUMNS.
"""
...
def add_regime_v3_features(df: pd.DataFrame) -> pd.DataFrame:
    """Add v3 regime features to *df*.
    ...
    # Helper column for labeling (NOT a feature)
