...
Stage-1 ranking (locked): net IS Sharpe @1× cost, ties broken by the 2×-stress Sharpe
(cost_mult=2 AND slip_mult=2), then by max drawdown (less negative wins). The Critic PASS
gate is applied by the orchestrator via the team list passed to ``build_stage1``. The
board prints the Sharpe NOISE FLOOR (SE over the window's monthly points) next to the
ranking — gaps inside it are statistical ties that the locked rule still decides.
"""
...
def sharpe_se(sharpe: float, n_months: int) -> float:
    """Standard error of an annualised monthly Sharpe (Lo 2002 iid approx): the noise floor."""
    if not math.isfinite(sharpe) or n_months < 2:
        return float("nan")
    s_m = sharpe / math.sqrt(12)
    return math.sqrt((1 + s_m * s_m / 2) / n_months) * math.sqrt(12)
...
def run_team(
    ...
    ``2x`` doubles BOTH the taker fee and the slippage model (the full cost stress).
    """
    pn, aux, scoring = te.load_is_panels(snapshot_dir, manifest_path)
    mod = tp.load_strategy(team_dir)
    raw = te.conform_raw(mod.build_raw_weights(te.team_view(pn), te.make_team_aux(aux)), pn)
    net1, w1, parts1 = te.net_series(raw, pn, scoring, cost_mult=1.0, slip_mult=1.0)
    net2, w2, parts2 = te.net_series(raw, pn, scoring, cost_mult=2.0, slip_mult=2.0)
    m1 = te.evaluate(net1, w1, parts1, lo=tc.TRN_IS_START, hi=tc.TRN_IS_HI)
...
def stage1_rank(entries: list[dict]) -> list[dict]:
    """Rank by net IS Sharpe @1x; ties: 2x-stress Sharpe, then maxDD (less negative). NaN sinks."""
        ...
        entries.append(
            {
                ...
                "sharpe_1x": m1["sharpe"],
                "sharpe_2x": m2["sharpe"],
                "sharpe_se": sharpe_se(m1["sharpe"], m1["n_months"]),
                "maxdd": m1["maxdd"],
                ...
                "total_funding_pnl": m1["total_funding_pnl"],
                "total_cost": m1["total_cost"],
            }
        )
