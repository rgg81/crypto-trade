"""crypto-cup-01 CLI — the single front door for teams AND the orchestrator.

  uv run python analysis/portfolio_tournament/cli.py <command> [...]

Team-facing:      team-run --team NN | audit --team NN | log-experiment --team NN --json '{}'
Orchestrator:     build-universe | data-report | build-snapshot | verify-snapshot |
                  init-teams | register-family | freeze --team NN --family-id ID |
                  leaderboard --teams a,b,c | run-holdout --team NN --confirm-holdout [...] |
                  final-report
Every state-changing command appends one line to tournament/crypto/journal.jsonl.
"""
...
BOOTSTRAP_TEMPLATE = """\
# {team_id} — crypto-cup-01 boot card
...
def _cmd_build_universe(args) -> int:
    summary = tu.build(Path(args.src), tc.BUILD_DIR)
    tc.journal("universe_built", **{k: v for k, v in summary.items() if k != "grid_lo_ms"})
    print(json.dumps(summary, indent=2))
    return 0


def _cmd_data_report(args) -> int:
    """Coverage gate: per union symbol, kline/funding/OI span vs top-40 membership span."""
    import pandas as pd

    src = Path(args.src)
    window = args.window
    mask = pd.read_csv(tc.BUILD_DIR / "eligibility_full.csv", index_col="open_time")
    hi_ms = tc.IS_END_MS if window == "is" else int(tc.TRN_HOLD_HI.value // 1_000_000) - 1
    m = mask[mask.index <= hi_ms]
    rows, n_fail, n_warn = [], 0, 0
    for sym in sorted(c for c in m.columns if m[c].any()):
        member = m[sym][m[sym] > 0]
        lo_mem, hi_mem = int(member.index.min()), int(member.index.max())
        fp = src / "funding_rates" / f"{sym}.csv"
        status, detail = "OK", ""
        if not fp.exists():
            status, detail = "FAIL", "no funding file"
        else:
            f = pd.read_csv(fp)
            if not len(f):
                status, detail = "FAIL", "empty funding file"
            else:
                flo, fhi = int(f["funding_time"].min()), int(f["funding_time"].max())
                n_in = int(((f["funding_time"] >= lo_mem) & (f["funding_time"] <= hi_mem)).sum())
                expect = max(1, (min(fhi, hi_mem) - max(flo, lo_mem)) // tc.STEP_MS)
                week = 7 * 24 * 3600 * 1000
                if n_in == 0:
                    status, detail = "FAIL", "no funding events inside membership span"
                elif flo > lo_mem + week:
                    status, detail = "WARN", f"funding starts {(flo - lo_mem) // 86400000}d late"
                elif fhi < hi_mem - week:
                    status, detail = "WARN", f"funding ends {(hi_mem - fhi) // 86400000}d early"
                elif n_in / expect < 0.9:
                    status, detail = "WARN", f"sparse funding ({n_in}/{expect} expected)"
        has_oi = (src / "open_interest" / sym / "8h.csv").exists()
        n_fail += status == "FAIL"
        n_warn += status == "WARN"
        rows.append((sym, status, detail, "oi" if has_oi else "-"))
    for sym, status, detail, oi in rows:
        if status != "OK" or args.verbose:
            print(f"{sym:22} {status:5} {oi:3} {detail}")
    print(f"\ndata-report [{window}]: {len(rows)} symbols, {n_fail} FAIL, {n_warn} WARN")
    tc.journal("data_report", window=window, n_symbols=len(rows), n_fail=n_fail, n_warn=n_warn)
...
def _cmd_log_experiment(args) -> int:
    """Evaluator-stamped experiment-ledger append (tradfi Critic recommendation #2)."""
    ...
    entry = {
        "ts": datetime.now(UTC).isoformat(timespec="seconds"),  # evaluator clock, not team's
        "team_id": args.team,
        **payload,
    }
    with (td / "experiments.jsonl").open("a") as f:
        f.write(json.dumps(entry, sort_keys=True) + "\n")
    print(f"{args.team}: experiment logged ({entry['ts']})")
    return 0
...
def _cmd_team_run(args) -> int:
    ...
    print(
        f"\n{args.team}: IS Sharpe {m1['sharpe']:+.3f} @1x / {m2['sharpe']:+.3f} @2x-stress  "
...
def _cmd_freeze(args) -> int:
    td = tc.team_dir(args.team)
    tp.require_approved_family(args.team, args.family_id)  # registry gate (Critic rec #1)
...
def _cmd_leaderboard(args) -> int:
    ...
    lines = [
        "# crypto-cup-01 — Stage-1 leaderboard (net IS Sharpe @1x, critic-gated)",
        "",
        "| rank | team | IS Sharpe 1x | ±SE | IS Sharpe 2x-stress | maxDD | ann. turnover "
        "| breadth L/S | finalist |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for e in board["ranked"]:
        lines.append(
            f"| {e['rank']} | {e['team_id']} | {e['sharpe_1x']:+.3f} | {e['sharpe_se']:.2f} "
            f"| {e['sharpe_2x']:+.3f} | {e['maxdd'] * 100:.1f}% | {e['ann_turnover']:.1f} "
            f"| {e['median_names_long']:.0f}/{e['median_names_short']:.0f} "
            f"| {'YES' if e['finalist'] else ''} |"
        )
    if board["ranked"]:
        se = board["ranked"][0]["sharpe_se"]
        lines.append(
            f"\nNOISE FLOOR: Sharpe SE ≈ ±{se:.2f} over this window — rank gaps smaller than "
            "that are statistical ties; the locked metric still decides."
        )
...
def _cmd_run_holdout(args) -> int:
    ...
    if label is None:
        ...
        if args.slip_mult != 1.0:
            parts.append(f"slip{args.slip_mult:g}x")
...
def _cmd_final_report(_args) -> int:
    ...
    n_months = 24
    lines = [
        "# crypto-cup-01 — Stage-2 sealed holdout (2024-07-01 .. 2026-06-30, funding on)",
        "",
    ...
    if rows:
        se = tlb.sharpe_se(rows[0]["metrics"]["sharpe"], n_months)
        lines.append(
            f"\nNOISE FLOOR: {n_months} monthly points ⇒ Sharpe SE ≈ ±{se:.2f} — gaps inside "
            "it are statistical ties; the locked rule (best net holdout Sharpe) decides."
        )
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="crypto-cup-01")
    sub = ap.add_subparsers(dest="cmd", required=True)

    p = sub.add_parser("build-universe", help="full-pool weekly top-40 mask + union lists")
    p.add_argument("--src", default=str(tc.MAIN_DATA_DIR))
    p.set_defaults(fn=_cmd_build_universe)

    p = sub.add_parser("data-report", help="funding/OI coverage vs top-40 membership spans")
    p.add_argument("--src", default=str(tc.MAIN_DATA_DIR))
    p.add_argument("--window", choices=["is", "full"], default="is")
    p.add_argument("--verbose", action="store_true")
    p.set_defaults(fn=_cmd_data_report)

    p = sub.add_parser("build-snapshot", help="freeze the IS snapshot from the full data store")
    p.add_argument("--src", default=str(tc.MAIN_DATA_DIR))
    ...
    p = sub.add_parser("log-experiment", help="evaluator-stamped experiments.jsonl append")
    p.add_argument("--team", required=True)
    p.add_argument(
        "--json", required=True, help="one experiment as a JSON object, e.g. '{\"h\": 1}'"
    )
    p.set_defaults(fn=_cmd_log_experiment)

    p = sub.add_parser("team-run", help="IS run @1x and @2x-stress cost; writes out/ artifacts")
    p.add_argument("--team", required=True)
    ...
    p = sub.add_parser("freeze", help="registry gate + full audit + SHA-bind the submission")
    p.add_argument("--team", required=True)
    p.add_argument("--family-id", required=True)
    ...
    p.add_argument("--slip-mult", type=float, default=1.0)
    p.add_argument("--src", default=str(tc.MAIN_DATA_DIR))
