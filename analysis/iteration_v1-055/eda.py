"""
EDA for iter-v1/055 — Pre-launch ADF stationarity + IC orthogonality check.

Feature under test: eth_vs_btc_ret_ratio_30
Axis: feature-family — ETH-only specialist cross-asset idiosyncratic return signal
      (cycle-6 EXPLORATION 10/10 FINAL)

eth_vs_btc_ret_ratio_30 is the z-scored ratio of ETHUSDT's 30-bar rolling return
vs BTCUSDT's concurrent return, capturing ETH idiosyncratic momentum vs market leader.
It is ETH-only (NaN for all other symbols) and is a direct algebraic mirror of
dot_vs_btc_ret_ratio_30 from iter-v1/050.
...
Checks:
  ADF        -- stationarity on ETHUSDT IS window (close_time < OOS_CUTOFF_MS)
  IC_pearson -- Pearson correlation of eth_vs_btc_ret_ratio_30 vs each of the 47
               other V1_FEATURE_COLUMNS_PRUNED features on ETHUSDT IS data
  dist_stats -- distribution statistics on ETHUSDT IS window

Parquet source: data/features/ETHUSDT_8h_features.parquet
IS barrier: close_time < OOS_CUTOFF_MS (1742774400000 = 2025-03-24 00:00:00 UTC)

Outputs:
  analysis/iteration_v1-055/eda.csv         -- one row per check result
  analysis/iteration_v1-055/eda_summary.md  -- markdown table summary
"""
...
TARGET_FEATURE: str = "eth_vs_btc_ret_ratio_30"

# Output paths
OUT_DIR: Path = Path("analysis/iteration_v1-055")
...
# The 47 peer features (V1_FEATURE_COLUMNS_PRUNED at iter-v1/055 state = 48 cols,
# minus the new feature eth_vs_btc_ret_ratio_30 itself = 47 peers).
# SOURCE: src/crypto_trade/features_v1/__init__.py V1_FEATURE_COLUMNS_PRUNED
PEER_FEATURES_47: tuple[str, ...] = (
...
assert len(PEER_FEATURES_47) == 47, f"Expected 47 peer features, got {len(PEER_FEATURES_47)}"


# ---------------------------------------------------------------------------
# Data loading — IS-only (ETHUSDT only)
# ---------------------------------------------------------------------------


def load_eth_is_data() -> pd.DataFrame:
    """Load ETHUSDT IS-only parquet. Applies close_time < OOS_CUTOFF_MS barrier.

    Returns DataFrame with eth_vs_btc_ret_ratio_30 + 47 peer columns.
    """
    ...
    missing_peers = [f for f in PEER_FEATURES_47 if f not in df.columns]
    if missing_peers:
...
def run_adf(df: pd.DataFrame) -> dict[str, float]:
    """Run ADF on eth_vs_btc_ret_ratio_30 for ETHUSDT IS window.
    ...
    print(f"  Symbol: {TARGET_SYMBOL} (ETH-only specialist feature)")
...
# ---------------------------------------------------------------------------
# IC orthogonality (Pearson) — ETHUSDT IS data only
# ---------------------------------------------------------------------------


def run_ic_orthogonality(df: pd.DataFrame) -> dict[str, float]:
    """Compute Pearson IC of eth_vs_btc_ret_ratio_30 vs all 47 peer features.

    Uses ETHUSDT IS data only (ETH-only feature — NaN for all other symbols).
    Returns dict feature_name -> |Pearson IC|.

    NOTE: values are INFORMATIONAL (per rule bf2c812).
    All values reported regardless of magnitude — no ABORT trigger.
    """
    print("\n=== IC Orthogonality (Pearson) — ETHUSDT IS ===")
    print(f"  Target feature: {TARGET_FEATURE}")
    print(f"  Peers: {len(PEER_FEATURES_47)} features")
    print(
    ...
    print(f"  ETHUSDT IS rows: {n_is} | valid {TARGET_FEATURE}: {n_target_valid}")

    ic_abs: dict[str, float] = {}

    for feat in PEER_FEATURES_47:
...
# ---------------------------------------------------------------------------
# Distribution stats — ETHUSDT IS
# ---------------------------------------------------------------------------


def distribution_stats(df: pd.DataFrame) -> dict:
    """Compute IS distribution stats for eth_vs_btc_ret_ratio_30 on ETHUSDT."""
...
def write_eda_summary_md(
    ...
    lines: list[str] = []
    lines.append("# EDA — iter-v1/055 Pre-launch Checks")
    lines.append("")
    lines.append(f"Feature under test: `{TARGET_FEATURE}`")
    lines.append(
        "Axis: feature-family — ETH-only specialist cross-asset idiosyncratic return "
        "vs BTC 30d z-scored (cycle-6 EXPLORATION 10/10 FINAL)"
    )
    lines.append(f"IS cutoff: `OOS_CUTOFF_MS = {OOS_CUTOFF_MS}` (2025-03-24)")
    lines.append(f"Symbol scope: `{TARGET_SYMBOL}` only (ETH-only specialist feature)")
    lines.append("")
    lines.append(
    ...
    # IC table (all 47 peers, top 15 shown in detail)
    lines.append(
        f"## IC Orthogonality — `{TARGET_FEATURE}` vs 47 peer features ({TARGET_SYMBOL} IS)"
    )
    lines.append("")
    lines.append(
...
def main() -> None:
    print("=" * 70)
    print("iter-v1/055 Pre-launch EDA: ADF + IC Orthogonality (Informational)")
    print(f"  Feature under test: {TARGET_FEATURE}")
    print(f"  Symbol scope: {TARGET_SYMBOL} only (ETH-only specialist feature)")
