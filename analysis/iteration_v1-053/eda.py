"""
EDA for iter-v1/053 — btc_funding_rate_8h_impulse + btc_funding_spread_30_90.

Multi-seed validation iteration. Features are UNCHANGED from iter-v1/052.
This script is an idempotent check: it recomputes ADF + IC + dist stats on
the same IS window and verifies the values match iter-v1/052's eda.csv within
floating-point tolerance.

Purpose: confirm the parquet has not been regenerated with different values
since /052's EDA was produced. Any deviation would indicate a data-pipeline
change that needs investigation before the multi-seed run.
...
Outputs:
  analysis/iteration_v1-053/eda.csv         — one row per check result (same schema as /052)
  analysis/iteration_v1-053/eda_summary.md  — short note confirming idempotent match
"""
...
# Two features under test — unchanged from /052
...
# Output paths
OUT_DIR: Path = Path("analysis/iteration_v1-053")
EDA_CSV: Path = OUT_DIR / "eda.csv"
EDA_SUMMARY_MD: Path = OUT_DIR / "eda_summary.md"

# Reference /052 EDA CSV for idempotent check
REF_CSV: Path = Path("analysis/iteration_v1-052/eda.csv")

# Informational thresholds — no ABORT on any result
F5_PASS_THRESHOLD: float = 0.30
F5_DOCUMENT_THRESHOLD: float = 0.60

# The 46 peer features (V1_FEATURE_COLUMNS_PRUNED at iter-v1/051 state).
...
# ---------------------------------------------------------------------------
# Idempotent check vs /052
# ---------------------------------------------------------------------------


def verify_matches_052(df_new: pd.DataFrame) -> None:
    """Load /052 eda.csv and verify ADF + dist stat values match within tolerance.

    This is a sanity check confirming the parquet has not changed since /052.
    Prints MATCH or MISMATCH for each ADF row and each dist_stats row.
    No ABORT — differences are flagged for QR inspection.
    """
    if not REF_CSV.exists():
        print(f"\n[IDEMPOTENT CHECK] Reference CSV not found: {REF_CSV} — SKIPPING CHECK")
        return

    print(f"\n=== Idempotent Check vs iter-v1/052 ({REF_CSV}) ===")
    ref_df = pd.read_csv(REF_CSV)

    # ADF check
    adf_ref = ref_df[(ref_df["check"] == "ADF") & (ref_df["metric"] == "p_value")]
    for _, row in adf_ref.iterrows():
        feat = row["feature"]
        ref_val = float(row["value"])
        # Recompute for comparison
        if feat in df_new.columns:
            series = df_new[feat].dropna()
            if len(series) >= 50:
                adf_result = adfuller(series, regression="c", maxlag=10)
                new_val = float(adf_result[1])
                match = abs(new_val - ref_val) < 1e-8
                label = "MATCH" if match else f"MISMATCH (ref={ref_val:.6e}, new={new_val:.6e})"
                print(f"  ADF {feat}: {label}")
            else:
                print(f"  ADF {feat}: insufficient data — SKIP comparison")
        else:
            print(f"  ADF {feat}: NOT FOUND in parquet — SKIP comparison")

    # Dist stats: compare n (row count) and std as quick proxies
    dist_ref = ref_df[ref_df["check"] == "dist_stats"]
    for feat in NEW_FEATURES:
        for metric in ("n", "std"):
            mask = (dist_ref["feature"] == feat) & (dist_ref["metric"] == metric)
            if not mask.any():
                continue
            ref_val = float(dist_ref.loc[mask, "value"].iloc[0])
            if feat in df_new.columns:
                series = df_new[feat].dropna()
                if metric == "n":
                    new_val = float(len(series))
                    match = new_val == ref_val
                else:  # std
                    new_val = float(series.std())
                    match = abs(new_val - ref_val) / (ref_val + 1e-12) < 1e-6
                label = "MATCH" if match else f"MISMATCH (ref={ref_val:.6g}, new={new_val:.6g})"
                print(f"  dist_stats [{feat}] {metric}: {label}")

    print("[IDEMPOTENT CHECK] Complete.")
...
def write_eda_csv(
    ...
    # Markdown summary — short note for multi-seed validation iter
    with EDA_SUMMARY_MD.open("w") as f:
        f.write("# EDA Summary — iter-v1/053\n\n")
        f.write("Multi-seed validation iter; EDA same as /052 by construction.\n\n")
        f.write(
...
def main() -> None:
    print("=" * 70)
    print("EDA iter-v1/053: btc_funding_rate_8h_impulse + btc_funding_spread_30_90")
    print("  Multi-seed validation — EDA is idempotent check vs iter-v1/052.")
    ...
    print(
        "\n[EDA] Complete. iter-v1/053 EDA artifact committed for Phase 5.5 gate "
        "(idempotent vs /052)."
    )
