"""iter-v1/076 — AAVEUSDT NEW SYMBOL specialist EDA (IS-only).

PURPOSE
-------
This is the load-bearing numerical EDA for iter-v1/076, the second NEW SYMBOL
universe-extension SPECIALIST EXPLORATION under the 2026-06-06 autopilot mining
directive. iter-v1/075 (ATOMUSDT) was the first; /076 (AAVEUSDT) is the first
DeFi-lending narrative specialist candidate in the v1 roster.

AAVEUSDT promoted past higher-composite ICP/FTM/EGLD candidates on the unique
NEGATIVE-baseline gate — AAVE is the ONLY eligible NEW SYMBOL with a negative
TS-mom IS Sharpe at the mine-phase formula (mine-phase cited -0.311; this
script's mom_5/fwd_3 replication is -0.124 — same SIGN, weaker magnitude).
NEGATIVE-pooled-baseline is the necessary precondition for specialist edge
extraction per the DOT/063 precedent (LINK/066 + LTC/067 ELIMINATED on the
positive-baseline trap).
...
- The ONLY single-bit deviations vs /063 dispatch: SYMBOLS=("AAVEUSDT",),
  ITERATION_LABEL="v1-076", ATR pair shifted /063's (3.5, 1.75) → /064's
  (2.9, 1.45) cell (Model A wrapper match — same as /064 /065 /075).

OUTPUTS (writes ALL to analysis/v1-076/ as CSV tables consumed by Section 2)
---------------------------------------------------------------------------
...
- table_05_cross_asset_corr.csv          Rolling 90d return corr with BTC / ETH / DOT / LTC / LINK
...
- table_10_cross_cohort_corr_pyramid.csv AAVE-vs-{BTC,ETH,DOT,LTC,LINK} IS

USAGE
-----
    uv run python analysis/v1-076/eda.py

The script is IDEMPOTENT and SAFE TO COMMIT — it reads ONLY IS-window data and
writes deterministic CSV outputs to analysis/v1-076/.
"""
...
OUT_DIR: Path = REPO_ROOT / "analysis" / "v1-076"
OUT_DIR.mkdir(parents=True, exist_ok=True)

SYMBOL: str = "AAVEUSDT"
ANCHOR_SYMBOLS: tuple[str, ...] = ("BTCUSDT", "ETHUSDT", "DOTUSDT", "LTCUSDT", "LINKUSDT")

# ATR pair LOCKED per Phase 4.5 LM Master (ETH/064 cell — Model A match).
...
def _assert_is_only_path(p: Path | str) -> None:
    ...
    assert "out_of_sample" not in sp, (
        f"[v1-076 EDA] IS-FIREWALL VIOLATION: path '{sp}' contains 'out_of_sample'. "
        "EDA scripts MUST NOT open OOS files. Abort."
    )


# ---------------------------------------------------------------------------
# KLINE LOAD — restricts to FULL csv rows but filters to IS-only at usage
# ---------------------------------------------------------------------------


def load_klines(symbol: str) -> pd.DataFrame:
    """Load 8h klines for `symbol` and return a tidy DataFrame with `open_time` ms
    plus typed OHLCV columns. The full csv is loaded — the IS firewall is
...
def _hurst_simple(log_returns: np.ndarray) -> float:
    ...
    NOTE: this rescaled-range-on-log-returns variant is biased toward 0 on noisy
    financial returns; values in [-0.10, +0.15] are NOT Hurst-interpretable.
    The LM Master Phase 4.5 advisor uses a separate computation; this EDA's
    table_03 is reproducibility-only and NOT cited as Hurst evidence."""
...
# ---------------------------------------------------------------------------
# TABLE 05 — CROSS-ASSET ROLLING 90D RETURN CORR (BTC / ETH / DOT / LTC / LINK)
# ---------------------------------------------------------------------------


def table_05_cross_asset_corr() -> pd.DataFrame:
    df_aave = load_klines(SYMBOL)
    df_aave_is = is_only(df_aave).copy()
    df_aave_is["ret"] = df_aave_is["close"].pct_change()
...
def table_09_feature_nan_audit() -> pd.DataFrame:
    """Read the AAVEUSDT features parquet and audit NaN coverage on the 48-col
    ...
    `data/features/AAVEUSDT_8h_features.parquet` — no IS/OOS subpath)."""
...
def main() -> None:
    print("=" * 70)
    print("iter-v1/076 AAVEUSDT NEW SYMBOL specialist EDA (IS-only)")
    ...
    print("[v1-076 EDA] Table 01 - Data extent ...")
    print(table_01_data_extent().to_string(index=False))
    print()
    print("[v1-076 EDA] Table 02 - Realized vol regime by year ...")
    print(table_02_realized_vol_regime().to_string())
    print()
    print("[v1-076 EDA] Table 03 - Rolling 100-bar Hurst exponent IS ...")
    print(table_03_hurst_by_window().to_string(index=False))
    print()
    print("[v1-076 EDA] Table 04 - Regime mix (50-bar return bull/bear/chop) ...")
    print(table_04_regime_mix().to_string(index=False))
    print()
    print("[v1-076 EDA] Table 05 - Cross-asset rolling-90 return corr ...")
    print(table_05_cross_asset_corr().to_string(index=False))
    print()
    print("[v1-076 EDA] Table 06 - TS-mom IS Sharpe grid ...")
    print(table_06_ts_mom_baseline().to_string(index=False))
    print()
    print("[v1-076 EDA] Table 07 - Triple-barrier label distribution ATR(2.9, 1.45) ...")
    print(table_07_triple_barrier_label_dist().to_string(index=False))
    print()
    print("[v1-076 EDA] Table 08 - Per-direction propensity by year ...")
    print(table_08_per_direction_audit().to_string(index=False))
    print()
    print("[v1-076 EDA] Table 09 - 48-col feature NaN audit IS+OOS ...")
    out9 = table_09_feature_nan_audit()
    print(out9.head(15).to_string(index=False))
    print(f"  ... ({len(out9)} features total)")
    print()
    print("[v1-076 EDA] Table 10 - Cross-cohort return-corr pyramid ...")
    print(table_10_cross_cohort_corr_pyramid().to_string(index=False))
    print()
    print("=" * 70)
    print("[v1-076 EDA] DONE.  All tables persisted to analysis/v1-076/*.csv")
