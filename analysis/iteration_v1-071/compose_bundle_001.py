"""
iter-v1/071 — BUNDLE-001 ASSEMBLY composition script.

Reads the 3 specialists' trades.csv files (IS + OOS) for:
  - /063 DOT specialist (R1+R2+R3, atr 3.5/1.75)
  - /064 ETH specialist (R3 only, atr 2.9/1.45)
  - /065 BTC specialist (R3 only, atr 2.9/1.45)

Writes full bundle output to reports-v1/iteration_v1-071/:
...
BUNDLE_DIR = REPORTS / "iteration_v1-071"
...
# ---------------------------------------------------------------------------
# Helper: portfolio metrics from a list of net_pnl_pct values + close_times
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
def main() -> None:
    print("=== BUNDLE-001 ASSEMBLY — iter-v1/071 ===")
    print()
    ...
    for r in is_per_sym:
        sym_line = (
            f"    {r['symbol']:<10}  trades={r['trades']:>3}"
    ...
    # Step 7: compute headline metrics
    print("[7] Computing headline metrics ...")
    ...
    # Step 8: write comparison.csv
    print("[8] Writing comparison.csv ...")
    write_comparison(is_metrics, oos_metrics, oos_per_sym, BUNDLE_DIR / "comparison.csv")
    print(f"  Wrote: {BUNDLE_DIR / 'comparison.csv'}")
    print()

    # Step 9: per-specialist summary cross-check
    print("[9] Per-specialist cross-check (n_trades) ...")
