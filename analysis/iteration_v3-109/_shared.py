"""iter-v3/109 gating EDA — shared IS-only data loader + /059-faithful labeling.

The axis under test: does a higher-capacity / different-class model (a neural
model — MLP, and a temporal-window MLP as a TCN proxy) extract MORE signal from
the /059 14-feature stack than the depth-3-5 LightGBM that v3 has used for 109
iterations? This module builds the labeled IS dataset ONCE; the two EDA scripts
import it.
...
is sign(better-of long_pnl / short_pnl) — i.e. the directional target the v3
primary model is trained to predict. Adverse-first intra-bar tie-break (SL
checked before TP), exactly as the production loop short-circuits.
"""
...
# the ONE clean variable is the model class.
V3_FEATURE_COLUMNS = [
    "max_dd_window_50",
    "ema_spread_atr_20",
    "ret_kurt_50",
...
def make_walk_forward_folds(
    ...
    Mirrors the /059 walk-forward discipline: the last ``n_folds`` equal-size
    blocks are held-out test folds; the training set for fold k is every candle
    strictly BEFORE the fold start MINUS an ``embargo`` of 22 candles (the
    /059 ``compute_embargo_candles`` value) so no label window straddles the
    train/test boundary. Returns (train_idx, test_idx) positional arrays.
    """
    ...
    for k in range(n_folds):
        ...
        if train_end < fold_size:  # need a usable minimum training set
            continue
