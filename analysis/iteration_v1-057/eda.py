"""
EDA for iter-v1/057 — Pre-launch ADF stationarity + IC orthogonality check.

Feature under test: ltc_vs_btc_ret_ratio_30
Axis: feature-family — LTC-only specialist cross-asset idiosyncratic return signal
      (cycle-7 EXPLORATION 1/N)

ltc_vs_btc_ret_ratio_30 is the z-scored ratio of LTCUSDT's 30-bar rolling return
vs BTCUSDT's concurrent return, capturing LTC idiosyncratic momentum vs market leader.
It is LTC-only (NaN for all other symbols) and is a direct algebraic mirror of
eth_vs_btc_ret_ratio_30 from iter-v1/055 (which itself mirrors dot_vs_btc_ret_ratio_30
from iter-v1/050).

NOTE (per rule bf2c812): EDA values are INFORMATIONAL only.
No ABORT trigger on max |IC| — artifact existence is what Phase 5.5 requires.
All values reported regardless of magnitude.

Checks:
  ADF        -- stationarity on LTCUSDT IS window (close_time < OOS_CUTOFF_MS)
  IC_pearson -- Pearson correlation of ltc_vs_btc_ret_ratio_30 vs each of the 48
               other V1_FEATURE_COLUMNS_PRUNED features on LTCUSDT IS data
  dist_stats -- distribution statistics on LTCUSDT IS window

Parquet source: data/features/LTCUSDT_8h_features.parquet
IS barrier: close_time < OOS_CUTOFF_MS (1742774400000 = 2025-03-24 00:00:00 UTC)

Outputs:
  analysis/iteration_v1-057/eda.csv         -- one row per check result
  analysis/iteration_v1-057/eda_summary.md  -- markdown table summary
"""
...
INTERVAL: str = "8h"
TARGET_SYMBOL: str = "LTCUSDT"
TARGET_FEATURE: str = "ltc_vs_btc_ret_ratio_30"

# Output paths
OUT_DIR: Path = Path("analysis/iteration_v1-057")
...
# The 48 peer features (V1_FEATURE_COLUMNS_PRUNED at iter-v1/057 state = 49 cols,
# minus the new feature ltc_vs_btc_ret_ratio_30 itself = 48 peers).
# SOURCE: src/crypto_trade/features_v1/__init__.py V1_FEATURE_COLUMNS_PRUNED
PEER_FEATURES_48: tuple[str, ...] = (
...
assert len(PEER_FEATURES_48) == 48, f"Expected 48 peer features, got {len(PEER_FEATURES_48)}"


# ---------------------------------------------------------------------------
# Data loading — IS-only (LTCUSDT only)
# ---------------------------------------------------------------------------


def load_ltc_is_data() -> pd.DataFrame:
    """Load LTCUSDT IS-only parquet. Applies close_time < OOS_CUTOFF_MS barrier.

    Returns DataFrame with ltc_vs_btc_ret_ratio_30 + 48 peer columns.
    """
    ...
    missing_peers = [f for f in PEER_FEATURES_48 if f not in df.columns]
    if missing_peers:
...
def run_adf(df: pd.DataFrame) -> dict[str, float]:
    """Run ADF on ltc_vs_btc_ret_ratio_30 for LTCUSDT IS window.
    ...
    print(f"  Symbol: {TARGET_SYMBOL} (LTC-only specialist feature)")
...
# ---------------------------------------------------------------------------
# IC orthogonality (Pearson) — LTCUSDT IS data only
# ---------------------------------------------------------------------------


def run_ic_orthogonality(df: pd.DataFrame) -> dict[str, float]:
    """Compute Pearson IC of ltc_vs_btc_ret_ratio_30 vs all 48 peer features.

    Uses LTCUSDT IS data only (LTC-only feature — NaN for all other symbols).
    Returns dict feature_name -> |Pearson IC|.

    NOTE: values are INFORMATIONAL (per rule bf2c812).
    All values reported regardless of magnitude — no ABORT trigger.
    """
    print("\n=== IC Orthogonality (Pearson) — LTCUSDT IS ===")
    print(f"  Target feature: {TARGET_FEATURE}")
    print(f"  Peers: {len(PEER_FEATURES_48)} features")
    print(
    ...
    print(f"  LTCUSDT IS rows: {n_is} | valid {TARGET_FEATURE}: {n_target_valid}")

    ic_abs: dict[str, float] = {}

    for feat in PEER_FEATURES_48:
...
# ---------------------------------------------------------------------------
# Distribution stats — LTCUSDT IS
# ---------------------------------------------------------------------------


def distribution_stats(df: pd.DataFrame) -> dict:
    """Compute IS distribution stats for ltc_vs_btc_ret_ratio_30 on LTCUSDT."""
    ...
    # Clip events check (LM Master Rec 1 — informational)
    raw_target = df[TARGET_FEATURE].dropna()
    # The feature is already z-scored/clipped in parquet; check for near-boundary values
    clip_events = int((raw_target.abs() >= 9.9).sum())
    ...
    print(f"  Clip events (|zscore| >= 9.9): {clip_events} (LM Master Rec 1 check — informational)")
    std_flag = "PASS" if st["std"] > 0.5 else "FLAG (std <= 0.5 — feature may be flat)"
    print(f"  Std check (> 0.5 for regime conditioning): {st['std']:.4f} [{std_flag}]")
        ...
        rows.append(
            {
                ...
                "note": (
                    ...
                    f"clip_events={dist_st['clip_events']}"
                ),
            }
        )
...
def write_eda_summary_md(
    ...
    lines: list[str] = []
    lines.append("# EDA — iter-v1/057 Pre-launch Checks")
    lines.append("")
    lines.append(f"Feature under test: `{TARGET_FEATURE}`")
    lines.append(
        "Axis: feature-family — LTC-only specialist cross-asset idiosyncratic return "
        "vs BTC 30d z-scored (cycle-7 EXPLORATION 1/N)"
    )
    lines.append(f"IS cutoff: `OOS_CUTOFF_MS = {OOS_CUTOFF_MS}` (2025-03-24)")
    lines.append(f"Symbol scope: `{TARGET_SYMBOL}` only (LTC-only specialist feature)")
    lines.append("")
    lines.append(
    ...
    if dist_st:
        std_flag = "ok" if dist_st["std"] > 0.5 else "flag (std<=0.5 — may be flat)"
        lines.append(
            f"| Std check (>0.5) | {std_flag} | "
            f"{dist_st['std']:.4f} | LM Master Rec 1 — informational |"
        )
        lines.append(
            f"| Clip events (|z|>=9.9) | informational | "
            f"{dist_st['clip_events']} | LM Master Rec 1 load-bearing check |"
        )
    lines.append("")
    ...
    # IC table (all 48 peers, top 15 shown in detail)
    lines.append(
        f"## IC Orthogonality — `{TARGET_FEATURE}` vs 48 peer features ({TARGET_SYMBOL} IS)"
    )
    lines.append("")
    lines.append(
...
def main() -> None:
    print("=" * 70)
    print("iter-v1/057 Pre-launch EDA: ADF + IC Orthogonality (Informational)")
    print(f"  Feature under test: {TARGET_FEATURE}")
    print(f"  Symbol scope: {TARGET_SYMBOL} only (LTC-only specialist feature)")
    ...
    print(f"  ADF p-value ({TARGET_SYMBOL}):  {adf_pval:.6e}")
    print(f"  Max |IC|:                   {max_ic_val:.4f} vs '{max_ic_feat}'")
    if dist_st:
        print(f"  Std (>0.5 check):           {dist_st['std']:.4f}")
        print(f"  Clip events (|z|>=9.9):     {dist_st['clip_events']}")
    print(f"  EDA CSV rows written:       {n_rows}")
    print("=" * 70)
