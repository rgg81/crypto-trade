"""Iter 148: Apply per-symbol VT (iter 147 config) to A+C+D+DOGE portfolio.

Tests whether per-symbol vol targeting unlocks DOGE addition (iter 143/146 failed
with portfolio-wide and no sizing).
"""
...
def main() -> None:
    ...
    print(f"Loaded {len(all_raw)} trades (A+C+D+DOGE)")

    # Apply iter 147's IS-tuned per-symbol config
    target_vol = 0.5
    lookback = 30
    print(f"Config: target={target_vol}, lookback={lookback} (per-symbol, iter 147 params)")
    ...
    # Per-symbol avg scales
    per_sym_scales = defaultdict(list)
    for t in all_raw:
        key = (int(t["open_time"]), t["symbol"], int(t["direction"]))
        per_sym_scales[t["symbol"]].append(scales.get(key, 1.0))
    print("\nPer-symbol avg scale:")
    for sym, sc in sorted(per_sym_scales.items()):
        print(f"  {sym}: {sum(sc)/len(sc):.2f}")

    report_dir = generate_iteration_reports(
        trades=scaled, iteration=148,
