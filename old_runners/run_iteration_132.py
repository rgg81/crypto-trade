"""Iter 132: Model D screening — BNB standalone (iter 126 config template).

Screen BNB as a potential Model D candidate.
...
def main() -> None:
    print(f"Iter {ITERATION}: MODEL D SCREENING — BNB STANDALONE")
    print("=" * 60)

    config = BacktestConfig(
        symbols=("BNBUSDT",), interval="8h", max_amount_usd=1000.0,
    ...
    print(f"\nBNB complete: {len(results)} trades in {elapsed:.0f}s")
