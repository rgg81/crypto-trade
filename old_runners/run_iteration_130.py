"""Iter 130: Model D screening — ADA standalone (iter 126 config template).

Screen ADA (Cardano) as a potential Model D candidate.
...
def main() -> None:
    print(f"Iter {ITERATION}: MODEL D SCREENING — ADA STANDALONE")
    print("=" * 60)

    config = BacktestConfig(
        symbols=("ADAUSDT",), interval="8h", max_amount_usd=1000.0,
    ...
    print(f"\nADA complete: {len(results)} trades in {elapsed:.0f}s")
