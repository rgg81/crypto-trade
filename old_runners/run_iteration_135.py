"""Iter 135: Model E screening — SOL standalone (iter 126 config template).

Screen SOL as a potential Model E candidate.
...
def main() -> None:
    print(f"Iter {ITERATION}: MODEL E SCREENING — SOL STANDALONE")
    print("=" * 60)

    config = BacktestConfig(
        symbols=("SOLUSDT",), interval="8h", max_amount_usd=1000.0,
    ...
    print(f"\nSOL complete: {len(results)} trades in {elapsed:.0f}s")
