"""Iter 133: Model D/E screening — DOT standalone (iter 126 config template).

Screen DOT (Polkadot) as a potential Model D/E candidate.
Config: ATR labeling 3.5x/1.75x, 185 auto-discovery features, single symbol.
"""
...
def main() -> None:
    print(f"Iter {ITERATION}: MODEL D/E SCREENING — DOT STANDALONE")
    print("=" * 60)

    config = BacktestConfig(
        symbols=("DOTUSDT",), interval="8h", max_amount_usd=1000.0,
    ...
    print(f"\nDOT complete: {len(results)} trades in {elapsed:.0f}s")
