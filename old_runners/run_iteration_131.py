"""Iter 131: Model D screening — XRP standalone (iter 126 config template).

Screen XRP as a potential Model D candidate.
...
def main() -> None:
    print(f"Iter {ITERATION}: MODEL D SCREENING — XRP STANDALONE")
    print("=" * 60)

    config = BacktestConfig(
        symbols=("XRPUSDT",), interval="8h", max_amount_usd=1000.0,
    ...
    print(f"\nXRP complete: {len(results)} trades in {elapsed:.0f}s")
