"""Iter 058: BTC+ETH, 24mo, TP=8%/SL=4%, timeout=7d, slow features (global intersection).

Same as iter 057 but with global intersection (unchanged discovery code).
All ~760 symbols regenerated so slow features appear in intersection.
Expected: ~137 features (106 baseline + 31 slow).
"""
import sys, time
...
def main() -> None:
    print(f"Iter 058: BTC+ETH, 24mo, TP=8%/SL=4%, timeout=7d, slow features (global)")
    ...
    strategy = LightGbmStrategy(
        ...
        label_tp_pct=8.0, label_sl_pct=4.0,
        label_timeout_minutes=10080,  # 7 days
