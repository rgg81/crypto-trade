"""Iter 136: BTC standalone screening — ATR labeling 3.5x/1.75x.

Test BTC as standalone model matching LINK/BNB template (iter 126 config).
Hypothesis: ATR labeling improves BTC's weak signal vs static 8%/4%.
"""
...
def main() -> None:
    print(f"Iter {ITERATION}: BTC STANDALONE — ATR LABELING 3.5x/1.75x")
    print("=" * 60)

    config = BacktestConfig(
        symbols=("BTCUSDT",), interval="8h", max_amount_usd=1000.0,
    ...
    print(f"\nBTC complete: {len(results)} trades in {elapsed:.0f}s")
