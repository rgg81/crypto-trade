"""iter-v1/045 — CSV-replay bundle aggregator (CONFIRMATION-MERGE-PORTFOLIO).

Design: NO fresh Optuna, NO LightGBM fit.  Reads pre-existing per-coin
trade rosters from source iterations, applies 0.2 weight, concatenates,
sorts, and emits bundle-level reports.

Usage:
  uv run python run_iteration_045.py [options]

  --bundle-config  "C-BTC:0.2,C-ETH:0.2,C-LINK:0.2,C-LTC:0.2,C-DOT:0.2"
  --weights-csv    analysis/iteration_v1-045/bundle_weights.csv
  --out            reports-v1/iteration_v1-045/
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="iter-v1/045 CSV-replay bundle aggregator")
    p.add_argument(
        "--bundle-config",
    ...
    p.add_argument(
        "--weights-csv",
        default="analysis/iteration_v1-045/bundle_weights.csv",
        help="Pre-registered bundle_weights.csv path (Check 17 byte-match assertion).",
    )
    p.add_argument(
        "--out",
        default="reports-v1/iteration_v1-045/",
...
def main() -> None:
    ...
    print("[iter-v1/045] CONFIRMATION-MERGE-PORTFOLIO ACTIVE")
    ...
    print()
    print("[iter-v1/045] DONE — OVERALL=READY-FOR-CRITIC")
