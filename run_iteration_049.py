"""iter-v1/049 — long_short_zscore_30 feature-family EXPLORATION runner.

This is a thin dispatch wrapper around ``run_baseline_v1.main()``.  It hardwires
the EXPLORATION-mode arguments for iter-v1/049 and adds the ``--features-base-hash``
guard recommended by LM Master Phase 4.5 Rec #1.

Feature axis:
    ADD ``long_short_zscore_30`` (rolling-30bar z-score of
    ``sum_toptrader_long_short_ratio`` from ``data/open_interest/<SYMBOL>/8h.csv``,
    a top-trader account-NOTIONAL positioning sentinel already cached at 8h cadence
    by the ``fetch-oi`` subcommand — NO new fetcher required) to
    ``V1_FEATURE_COLUMNS_PRUNED``: 44 → 45 columns.

    Module: src/crypto_trade/features_v1/longshort_v1.py (NEW at iter-v1/049)
    Source column: ``sum_toptrader_long_short_ratio`` in OI CSV (NOT a kline field)
    Non-kline defense: account-aggregated NOTIONAL positioning is structurally
    orthogonal to OHLCV/funding/OI/calendar at the primitive class level.
...
Invocation:
    ...
    uv run python run_iteration_049.py --n-trials 35

    # Verify features-base-hash only (dry-run):
    uv run python run_iteration_049.py --check-hash

Output paths:
    reports-v1/iteration_v1-049/in_sample/
    reports-v1/iteration_v1-049/out_of_sample/
    reports-v1/iteration_v1-049/comparison.csv
    reports-v1/iteration_v1-049/feature_importance.csv
    reports-v1/iteration_v1-049/run.log

LM Master Phase 4.5 Rec #1 (ADOPTED):
    Pre-registered SHA-256 hash of V1_FEATURE_COLUMNS_PRUNED (45 cols after /049 change).
...
# after the iter-v1/049 source change.  If this value changes, the Optuna search
...
#   "
FEATURES_BASE_HASH_EXPECTED: str = (
    "a2e36f6ab3790b95e9555a0699e72bf13180063f9ec74cb406644a18b02fa93e"
)
...
ITERATION_LABEL: str = "v1-049"
ITERATION_NUMBER: int = 49
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="iter-v1/049 EXPLORATION runner: long_short_zscore_30 feature axis"
    )
    p.add_argument(
        "--n-trials",
        type=int,
...
def _verify_features_hash(expected: str) -> None:
    ...
    if actual != expected:
        print(
            f"[iter-v1/049] FEATURES-BASE-HASH MISMATCH\n"
            f"  expected : {expected}\n"
            f"  actual   : {actual}\n"
        ...
        sys.exit(
            "[iter-v1/049] ABORT: feature column set does not match pre-registered hash.  "
            ...
            " and update FEATURES_BASE_HASH_EXPECTED in run_iteration_049.py."
        )
    print(
        f"[iter-v1/049] features-base-hash PASS: {actual[:16]}... "
        f"(45 columns, long_short_zscore_30 included)"
    )
...
def main() -> None:
    ...
    if args.check_hash:
        print(
            f"[iter-v1/049] --check-hash mode.  "
            f"Hash = {FEATURES_BASE_HASH_EXPECTED}.  "
            f"V1_FEATURE_COLUMNS_PRUNED len = {len(V1_FEATURE_COLUMNS_PRUNED)}.  "
            "No backtest launched."
        )
        print(
            "  long_short_zscore_30 in pruned:",
            "long_short_zscore_30" in V1_FEATURE_COLUMNS_PRUNED,
        )
        print("  Column list:")
    ...
    assert "long_short_zscore_30" in V1_FEATURE_COLUMNS_PRUNED, (
        "[iter-v1/049] PRE-FLIGHT FAIL: long_short_zscore_30 not in V1_FEATURE_COLUMNS_PRUNED.  "
    ...
    assert len(V1_FEATURE_COLUMNS_PRUNED) == 45, (
        f"[iter-v1/049] PRE-FLIGHT FAIL: expected 45 features, "
        f"got {len(V1_FEATURE_COLUMNS_PRUNED)}."
    )

    print("=" * 60)
    print("iter-v1/049 — long_short_zscore_30 EXPLORATION")
    ...
    lsr_idx = list(V1_FEATURE_COLUMNS_PRUNED).index("long_short_zscore_30")
    print(f"  long_short_zscore_30   : index {lsr_idx} of {n_cols}")
    print("  source column          : sum_toptrader_long_short_ratio (OI CSV, non-kline)")
    ...
    # EXPLORATION arguments for iter-v1/049.  This is the standard dispatch
