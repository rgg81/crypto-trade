...
from crypto_trade.tournament import (
    activation_top12_v1,
    journal_top12_v1,
    orchestrator_top12_v1,
    runner_top12_v1,
    scoring_top12_v1,
    source_archive_top12_v1,
    top12_universe_v1,
    top12_v1,
)
...
def _accepted(team_id: str, trial: int) -> dict[str, object]:
    return {
        ...
        "output_path": f"reports-top12-v1/is/{team_id}/run-{trial:02d}",
    }
...
def _success(request: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "summary_path": f"reports-top12-v1/is/{payload['team_id']}/summary.json",
        "summary_sha256": "2" * 64,
    }
...
def _candidate_authority(team_id: str) -> dict[str, str]:
    ...
    candidate_root = f"tournament/top12-v1/teams/{team_id}/candidates/{candidate_id}"
    ...
    return {
        ...
        "source_archive_path": (
            f"tournament/top12-v1/source-archives/{team_id}/{candidate_id}.json"
        ),
...
def _historical_request(team_id: str, sequence: int) -> dict[str, object]:
    ...
    return {
        ...
        "payload": {
            ...
            "output_path": (
                f"tournament/top12-v1/private/historical-oos/{team_id}/{run_id}"
            ),
        },
    }
...
def test_v4_contract_has_twelve_teams_and_only_two_historical_stages() -> None:
    loaded = top12_v1.load_config(root=ROOT)
    assert loaded.raw["teams"] == list(TOP12_V1_LAYOUT.team_ids)
    assert TOP12_V1_LAYOUT.team_ids[-1] == "team-12"
    assert (
        runner_top12_v1._authorized_window(loaded.raw, "is").score_start
        == "2020-08-03T00:00:00Z"
    )
    historical = runner_top12_v1._authorized_window(loaded.raw, "historical_oos")
    ...
    with pytest.raises(ValueError, match="is or historical_oos"):
        runner_top12_v1._authorized_window(loaded.raw, "validation")
...
def test_metric_window_normalizes_utc_start_and_date_only_end() -> None:
    ...
    authorized = runner_top12_v1.AuthorizedWindow(
        stage="is",
    ...
    packet = runner_top12_v1._compute_metrics(
        daily,
        daily,
        daily,
        daily,
...
def test_config_rejects_a_softened_gate() -> None:
    ...
    with pytest.raises(ValueError, match="frozen contract"):
        top12_v1.validate_config(changed)
...
def test_config_freezes_strategy_blindness_and_disallows_team_collisions() -> None:
    ...
    assert isolation["rediscovery_of_prior_mechanism_allowed"] is True
...
def test_journal_is_hash_chained_and_an_accepted_failure_consumes_trial(
    ...
    journal_top12_v1.initialize(path)
    request = dict(journal_top12_v1.append(path, "is_accepted", _accepted("team-01", 1)))
    journal_top12_v1.append(
        path,
        "is_failed",
        {
            "team_id": "team-01",
    ...
    state = journal_top12_v1.read(path)
    assert state.trials_by_team["team-01"] == 1
    assert request["record_sha256"] in state.is_terminals
    with pytest.raises(journal_top12_v1.JournalError, match="numbering"):
        journal_top12_v1.append(path, "is_accepted", _accepted("team-01", 3))


def test_journal_rejects_early_nomination_and_detects_tampering(tmp_path: Path) -> None:
    path = tmp_path / "journal.jsonl"
    request = dict(journal_top12_v1.append(path, "is_accepted", _accepted("team-02", 1)))
    success = dict(journal_top12_v1.append(path, "is_succeeded", _success(request)))
    with pytest.raises(journal_top12_v1.JournalError, match="eight"):
        journal_top12_v1.append(
            path,
            "nominated",
            {
    ...
    path.write_text("\n".join(rows) + "\n", encoding="ascii")
    with pytest.raises(journal_top12_v1.JournalError, match="hash"):
        journal_top12_v1.read(path)


def test_journal_recovers_only_an_unterminated_crash_tail(tmp_path: Path) -> None:
    path = tmp_path / "journal.jsonl"
    journal_top12_v1.append(path, "is_accepted", _accepted("team-01", 1))
    ...
    with path.open("ab") as handle:
        handle.write(b"{}\n")
    with pytest.raises(journal_top12_v1.JournalError, match="schema"):
        journal_top12_v1.read(path)
...
def test_journal_rejects_duplicate_run_ids_across_teams(tmp_path: Path) -> None:
    ...
    journal_top12_v1.append(path, "is_accepted", first)
    duplicate = _accepted("team-02", 1)
    duplicate["run_id"] = first["run_id"]
    with pytest.raises(journal_top12_v1.JournalError, match="duplicate tournament run_id"):
        journal_top12_v1.append(path, "is_accepted", duplicate)
...
def test_journal_allows_an_empty_bracket_but_never_lowers_the_floors(
    ...
    path = tmp_path / "journal.jsonl"
    for team_id in TOP12_V1_LAYOUT.team_ids:
        for trial in range(1, 9):
            request = dict(journal_top12_v1.append(path, "is_accepted", _accepted(team_id, trial)))
            journal_top12_v1.append(
                path,
                "is_failed",
                {
    ...
    before = journal_top12_v1.read(path)
    selection = dict(
        journal_top12_v1.append(
            path,
            "selection_frozen",
            {
                ...
                "advancing": [],
                "selection_freeze_path": "tournament/top12-v1/selection-freeze.json",
    ...
    journal_top12_v1.append(
        ...
        "release_authorized",
        {
            ...
            "staging_path": "reports-top12-v1/.historical-oos-staging",
            "release_path": "reports-top12-v1/historical-oos",
    ...
    with pytest.raises(journal_top12_v1.JournalError, match="selection freeze"):
        journal_top12_v1.append(path, "is_accepted", _accepted("team-01", 9))
...
def test_is_selection_is_conjunctive_and_includes_neighborhood_gate() -> None:
    config = top12_v1.load_config(root=ROOT).raw
    packet = _passing_is_packet()
    failed = scoring_top12_v1.assess_is(packet, config, trial_count=8)
    assert failed["eligible"] is False
    assert failed["gates"]["neighborhood_stability"] is False
    passed = scoring_top12_v1.assess_is(packet, config, trial_count=8, neighborhood_passed=True)
    assert passed["eligible"] is True
    packet["diagnostics"]["annualized_turnover"] = 20.0001
    result = scoring_top12_v1.assess_is(packet, config, trial_count=8, neighborhood_passed=True)
...
def test_neighborhood_coordinate_identity_normalizes_signed_zero() -> None:
    positive = orchestrator_top12_v1._coordinate_vector_key({"lookback": 0.0, "buffer": 1})
    negative = orchestrator_top12_v1._coordinate_vector_key({"lookback": -0.0, "buffer": 1.0})
...
def test_close_is_selects_exact_top_five_and_never_backfills(
    ...
    state = SimpleNamespace(
        ...
        retired={
            team_id: {} for team_id in TOP12_V1_LAYOUT.team_ids if team_id not in nominations
        },
        head_sha256="a" * 64,
    )
    ...
    monkeypatch.setattr(top12_v1, "load_config", lambda *_args, **_kwargs: loaded)
    monkeypatch.setattr(
        orchestrator_top12_v1,
        "_close_interrupted_is_requests",
        lambda _root: state,
    )
    monkeypatch.setattr(orchestrator_top12_v1, "_write_nomination_registry", lambda *_args: None)
    monkeypatch.setattr(
        orchestrator_top12_v1, "_verified_nomination", lambda _root, record: record["nomination"]
    )
    monkeypatch.setattr(orchestrator_top12_v1, "_capped_inverse_vol_weights", weights)
    monkeypatch.setattr(journal_top12_v1, "append", append)

    result = orchestrator_top12_v1.close_is(tmp_path)

    assert result["advancing"] == expected
    assert captured["advancing"] == expected
    freeze = json.loads((tmp_path / TOP12_V1_LAYOUT.selection_freeze_path).read_text())
...
def test_nomination_retry_replaces_only_an_unjournaled_orphan(
    ...
    team_id = "team-01"
    authority = orchestrator_top12_v1.CandidateAuthority(**_candidate_authority(team_id))
    candidate_id = authority.candidate_id
    state = SimpleNamespace(
        selection=None,
        nominations={},
    ...
    monkeypatch.setattr(activation_top12_v1, "validate", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(top12_v1, "load_config", lambda *_args, **_kwargs: loaded)
    monkeypatch.setattr(
        orchestrator_top12_v1,
        "_close_interrupted_is_requests",
    ...
    monkeypatch.setattr(
        orchestrator_top12_v1, "_validate_authority_current", lambda *_args: authority
    )
    monkeypatch.setattr(orchestrator_top12_v1, "_verified_summary", lambda *_args: {})
    monkeypatch.setattr(
        orchestrator_top12_v1,
        "_certificate",
    ...
    monkeypatch.setattr(orchestrator_top12_v1, "_write_nomination_registry", lambda *_args: None)
    monkeypatch.setattr(journal_top12_v1, "append", append)
    monkeypatch.setattr(journal_top12_v1, "read", lambda _path: state)

    certificate = "tournament/top12-v1/certificates/team-01/research.json"
    with pytest.raises(OSError, match="simulated crash"):
        orchestrator_top12_v1.nominate(tmp_path, team_id, candidate_id, certificate)
    nomination_path = (
        tmp_path / f"tournament/top12-v1/nominations/{team_id}-{candidate_id}.json"
    )
    assert json.loads(nomination_path.read_text())["trial_count"] == 8

    state.trials_by_team[team_id] = 9
    result = orchestrator_top12_v1.nominate(tmp_path, team_id, candidate_id, certificate)
...
def test_close_is_idempotency_verifies_the_journal_bound_freeze(
    ...
    state = SimpleNamespace(
        selection={
            "advancing": ["team-01"],
            "selection_freeze_path": TOP12_V1_LAYOUT.selection_freeze_path,
        },
        nominations={},
        retired={},
    )
    ...
    def reject_corrupt_freeze(_root: Path, observed: SimpleNamespace):
        ...
        raise orchestrator_top12_v1.OrchestratorError(
    ...
    monkeypatch.setattr(orchestrator_top12_v1, "_write_nomination_registry", lambda *_args: None)
    monkeypatch.setattr(orchestrator_top12_v1, "_selection_freeze", reject_corrupt_freeze)

    with pytest.raises(orchestrator_top12_v1.OrchestratorError, match="journal binding"):
        orchestrator_top12_v1.close_is(tmp_path)
...
def test_historical_batch_accepts_every_finalist_before_first_snapshot_read(
    ...
    monkeypatch.setattr(activation_top12_v1, "validate", validate)
    monkeypatch.setattr(
        top12_v1,
        "load_config",
    ...
    monkeypatch.setattr(journal_top12_v1, "read", lambda _path: state)
    monkeypatch.setattr(journal_top12_v1, "append", _historical_append_stub(state, events))
    monkeypatch.setattr(orchestrator_top12_v1, "_selection_freeze", lambda *_args: selection)

    with pytest.raises(RuntimeError, match="first snapshot read sentinel"):
        orchestrator_top12_v1.historical_release(tmp_path)
...
def test_historical_base_exception_stops_after_recording_only_the_started_dnf(
    ...
    monkeypatch.setattr(
        ...
        lambda _root, _loaded, expected: orchestrator_top12_v1.CandidateAuthority(**dict(expected)),
    )
    monkeypatch.setattr(
        orchestrator_top12_v1,
        "_run_team",
...
def test_authorized_release_rebuilds_missing_staging_before_promotion(
    ...
    def promote(_root: Path, observed: dict[str, object]) -> dict[str, object]:
        ...
        if promotion_attempts == 1:
            raise orchestrator_top12_v1.OrchestratorError("authorized staging is missing")
    ...
    monkeypatch.setattr(orchestrator_top12_v1, "_promote_authorized", promote)
    monkeypatch.setattr(orchestrator_top12_v1, "_release_bundle", rebuild)
    monkeypatch.setattr(
        orchestrator_top12_v1,
        "_strict_object",
        lambda *_args: {"bundle_sha256": release["bundle_sha256"]},
    )
    monkeypatch.setattr(orchestrator_top12_v1, "_validate_staged_release", validate)

    result = orchestrator_top12_v1._recover_authorized_release(tmp_path, {}, state)
...
def test_inverse_volatility_cap_leaves_cash_when_bracket_is_too_small(
    ...
    for index, team_id in enumerate(("team-01", "team-02"), start=1):
        ...
        frame = pd.DataFrame(
            {
                    "date": pd.date_range("2020-08-03", "2024-07-01", inclusive="left", tz="UTC"),
            }
        )
    ...
    weights, cash = orchestrator_top12_v1._capped_inverse_vol_weights(tmp_path, nominees, 0.30)
...
def test_source_archive_accepts_team_12_namespace(tmp_path: Path) -> None:
    files = (
        source_archive_top12_v1.SourceFile(
            path="strategy.py",
            size=4,
    ...
    bundle = source_archive_top12_v1.bundle_fingerprint(item.manifest_entry for item in files)
    archive = source_archive_top12_v1.write_source_archive(
        tmp_path,
        team_id="team-12",
        candidate_id="team12-test-v1",
        candidate_root="tournament/top12-v1/teams/team-12/candidates/team12-test-v1",
        entrypoint="strategy.py",
        source_bundle_sha256=bundle,
        files=files,
    )
    loaded = source_archive_top12_v1.read_source_archive(
...
def test_worker_bundle_is_materialized_from_archive_bytes(tmp_path: Path) -> None:
    ...
    bundle = source_archive_top12_v1.bundle_fingerprint(manifest)
    destination = tmp_path / "bundle"
    assert (
        runner_top12_v1._stage_archived_source_bundle(
            destination,
            files,
...
def test_artifact_publisher_keeps_independent_base_double_and_triple_cost_runs(
    ...
    artifacts = runner_top12_v1._publish_artifacts(
        tmp_path,
        "team-12",
        stage="is",
        output_relative="reports-top12-v1/is/team-12/triple-test",
...
def test_authority_path_helpers_reject_symlink_components(tmp_path: Path) -> None:
    ...
    with pytest.raises(orchestrator_top12_v1.OrchestratorError, match="symlink"):
        orchestrator_top12_v1._path(tmp_path, "alias/authority.py")
    with pytest.raises(activation_top12_v1.ActivationError, match="symlink"):
        activation_top12_v1._file(tmp_path, "alias/authority.py")


def test_activation_scope_is_explicit_and_complete() -> None:
    assert len(activation_top12_v1.FROZEN_SCOPE) == len(set(activation_top12_v1.FROZEN_SCOPE))
    missing = [
        relative for relative in activation_top12_v1.FROZEN_SCOPE if not (ROOT / relative).is_file()
    ]
    assert missing == []
