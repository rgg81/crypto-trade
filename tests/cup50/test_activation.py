...
from crypto_trade.cup50.activation import build_activation_record, verify_activation


def test_activation_binds_relative_artifacts_and_transitive_imports(tmp_path) -> None:
    package = tmp_path / "src" / "crypto_trade" / "cup50"
    package.mkdir(parents=True)
    (package / "entry.py").write_text("from crypto_trade.cup50 import helper\n")
    (package / "helper.py").write_text("VALUE = 1\n")
    (tmp_path / "config.toml").write_text("name='cup50'\n")
    ...
    build_activation_record(
        ...
        evaluator_entries=["src/crypto_trade/cup50/entry.py"],
        git_commit="a" * 40,
        git_clean=True,
        sandbox_image="cup50",
    ...
    assert "src/crypto_trade/cup50/helper.py" in record["artifacts"]
