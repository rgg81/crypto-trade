...
def test_protocol_export_contains_only_the_strategy_interface(tmp_path) -> None:
    ...
    assert manifest["namespace"] == "cup50-protocol-export"
    ...
    assert files == [
        ...
        "crypto_trade/cup50",
        "crypto_trade/cup50/__init__.py",
        "crypto_trade/cup50/protocol.py",
        "manifest.json",
    ]
...
def test_evaluator_export_rejects_imports_from_earlier_namespaces(tmp_path) -> None:
    source = tmp_path / "cup50"
    source.mkdir()
    (source / "__init__.py").write_text("")
    (source / "entry.py").write_text("from crypto_trade.cup50 import helper\n")
    (source / "helper.py").write_text("VALUE = 1\n")
    destination = tmp_path / "evaluator"

    manifest = export_evaluator_bundle(source, destination)

    assert manifest["namespace"] == "cup50-evaluator-export"
    assert set(manifest["files"]) == {
        "crypto_trade/cup50/__init__.py",
        "crypto_trade/cup50/entry.py",
        "crypto_trade/cup50/helper.py",
    }
    contaminated = tmp_path / "cup50-contaminated"
    contaminated.mkdir()
    (contaminated / "bad.py").write_text("from crypto_trade.cup20 import replay\n")
    with pytest.raises(ValueError, match="source must be the CUP-50"):
        export_evaluator_bundle(contaminated, tmp_path / "bad-export")

    (source / "bad.py").write_text("from crypto_trade.cup20 import replay\n")
    with pytest.raises(ValueError, match="outside CUP-50"):
...
def test_sandbox_image_tag_must_resolve_to_bound_digest(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "crypto_trade.cup50.isolation.subprocess.run", lambda *args, **kwargs: Result()
    )
    assert require_image_digest("cup50", "a" * 64) == "sha256:" + "a" * 64
    with pytest.raises(RuntimeError, match="does not resolve"):
        require_image_digest("cup50", "b" * 64)
