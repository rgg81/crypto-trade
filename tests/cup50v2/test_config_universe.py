...
from crypto_trade.cup50v2.availability import load_unavailability_audit, unavailable_symbols
from crypto_trade.cup50v2.config import IS_START, OOS_END, OOS_START, load_config
...
def test_frozen_unavailability_audit_is_half_open_and_canonical() -> None:
    windows = load_unavailability_audit("tournament/cup50v2/historical-unavailability.json")
    # The organizer audit is the union of every verified cessation, including the three that a
    # prior edition discovered only after roster exit; a missing window fails readiness later.
    ...
    assert unavailable_symbols(windows, pd.Timestamp("2022-05-13T08:00:00Z")) == {"LUNAUSDT"}
...
def test_unavailability_audit_accepts_hash_bound_organizer_recovery_phase() -> None:
    payload = json.loads(Path("tournament/cup50v2/historical-unavailability.json").read_text())
    ...
    assert load_unavailability_audit(payload) == load_unavailability_audit(
        "tournament/cup50v2/historical-unavailability.json"
    )


def test_team_visible_is_unavailability_audit_has_no_oos_window() -> None:
    windows = load_unavailability_audit("tournament/cup50v2/is-unavailability.json")
    assert [window.symbol for window in windows] == [
        "ICPUSDT",
        "LUNAUSDT",
        "TLMUSDT",
