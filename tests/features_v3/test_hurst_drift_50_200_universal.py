...
# iter-v3/083 UNIVERSE EXPANSION 3 -> 4 (FILUSDT added; denominator expansion).
_V3_MODELS_ITER_053 = ("BCHUSDT", "LDOUSDT", "TRXUSDT", "FILUSDT")
...
def test_hurst_drift_50_200_stationary_per_symbol() -> None:
    ...
    # the incumbent symbols whose v3 parquets already exist; FILUSDT's parquet is
    # generated by the QE in Phase 6 (iter-v3/083 universe expansion) and is not
    # required for this math check.
...
# ---------------------------------------------------------------------------
# Test 5 — V3_MODELS is 4-symbol universe at iter-v3/083
# ---------------------------------------------------------------------------


def test_v3_models_is_4_symbol_at_iter_v3_083() -> None:
    """V3 universe must be (BCHUSDT, LDOUSDT, TRXUSDT, FILUSDT) at iter-v3/083.

    iter-v3/083 CYCLE 3 EXPLORATION #2 — symbol-universe EXPANSION: V3_MODELS
    grows 3 -> 4 by ADDING FILUSDT (denominator expansion; the structural fix
    for the /082 BCH-concentration finding). ALGOUSDT was REVERTED at the system
    level per iter-v3/051; ADAUSDT is CLOSED (/078 SUSPICIOUS-OOS-DOMINANT).

    iter-v3/083: feature count = 14 (the BASELINE_V3 /059 anchor; /082's funding
    family reverted). All 4 symbols return the 14-feature universal fallback.
    REQUIRED_GAP = 88 = (21+1)*4 (4-symbol universe).
    """
    from crypto_trade.features_v3 import features_for_symbol  # noqa: PLC0415

    expected_universe = ("BCHUSDT", "LDOUSDT", "TRXUSDT", "FILUSDT")
    ...
    for sym in expected_universe:
        ...
        assert len(feats) == 14, (
            ...
            "(the BASELINE_V3 /059 anchor stack). iter-v3/083 REVERTS /082's "
            "funding family; hurst_drift_50_200 stays PARKED-ABSENT."
        )
