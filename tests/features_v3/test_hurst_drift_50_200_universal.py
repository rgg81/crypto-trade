...
def test_v3_models_is_3_symbol_at_iter_v3_084() -> None:
    ...
    iter-v3/084: feature count = 14 (the BASELINE_V3 /059 anchor; /082's funding
    family stays reverted). All 3 symbols return the 14-feature universal
    fallback. REQUIRED_GAP = 66 = (21+1)*3 (3-symbol universe).
    """
    ...
    for sym in expected_universe:
        ...
        assert len(feats) == 14, (
            ...
            "(the BASELINE_V3 /059 anchor stack). iter-v3/084 keeps the "
            "14-feature anchor UNCHANGED; hurst_drift_50_200 stays PARKED-ABSENT."
        )
