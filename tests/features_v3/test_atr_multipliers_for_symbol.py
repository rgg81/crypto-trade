"""Adversarial tests for per-symbol ATR multipliers — iter-v3/047 (UPDATED iter-v3/066).

iter-v3/066 state (EXPLORATION — cycle 1 #7 — NON-FEATURE PIVOT CONTINUATION; RISK PRIMITIVE axis
                   Path E0.8: RiskV2Config.vol_scale_ceiling 1.0 → 0.8 (universal)):
  - DEFAULT_ATR_MULTIPLIERS REVERTED to (2.0, 1.0).
    /065 set (2.0, 1.5); /066 REVERTS for single-axis attribution (Path E0.8 is the one change).
    EDA SHA `1d75cb0`; analysis/iteration_v3-066/risk_primitive_eda.py Section 2.7 T6.
    /065 SL widening reserved for /069 CONFIRMATION bundle alongside /066 ceiling=0.8.
  ...
  - V3_MODELS = (BCHUSDT, LDOUSDT, TRXUSDT) — 3 symbols (ALGO REVERTED at /051).
  - Single-axis change (this iter): vol_scale_ceiling=0.8 in RiskV2Config ONLY.
...
History of V3_ATR_MULTIPLIERS_PER_SYMBOL:
  ...
  iter-v3/065: still EMPTY (carry-forward; universal DEFAULT change only).
  iter-v3/066: still EMPTY (carry-forward; /066 axis is risk primitive, not labeling).

Validates (iter-v3/066 state):
  1. DEFAULT_ATR_MULTIPLIERS == (2.0, 1.0) at iter-v3/066 (axis isolation: /065 reverted).
...
def test_atr_multipliers_default():
    """DEFAULT_ATR_MULTIPLIERS == (2.0, 1.0) at iter-v3/066 (axis isolation: /065 reverted).

    iter-v3/066: /065's SL widening (2.0, 1.5) reverted to (2.0, 1.0). The single varied
    axis at /066 is RiskV2Config.vol_scale_ceiling=0.8 (Path E0.8). V3_ATR_MULTIPLIERS_PER_SYMBOL
    is EMPTY (SYSTEM-LEVEL REVERT carry-forward from /051). All 3 symbols (BCH/LDO/TRX)
    fall back to DEFAULT_ATR_MULTIPLIERS = (2.0, 1.0).
    """
    ...
    assert DEFAULT_ATR_MULTIPLIERS == (2.0, 1.0), (
        ...
        "iter-v3/066 axis isolation: /065's (2.0, 1.5) reverted to (2.0, 1.0). "
        "Verify DEFAULT_ATR_MULTIPLIERS = (2.0, 1.0) in features_v3/__init__.py."
    )
    # iter-v3/066: TRX uses DEFAULT fallback (V3_ATR_MULTIPLIERS_PER_SYMBOL empty).
    assert atr_multipliers_for_symbol("TRXUSDT") == (2.0, 1.0)
    # iter-v3/066: ALGOUSDT falls back to DEFAULT (V3_ATR_MULTIPLIERS_PER_SYMBOL empty).
    assert atr_multipliers_for_symbol("ALGOUSDT") == (2.0, 1.0), (
        "ALGOUSDT must return (2.0, 1.0) at iter-v3/066 — DEFAULT fallback. "
        "V3_ATR_MULTIPLIERS_PER_SYMBOL must be empty; ALGO falls back to DEFAULT."
    )
    # iter-v3/066: LDOUSDT falls back to DEFAULT (V3_ATR_MULTIPLIERS_PER_SYMBOL empty).
    assert atr_multipliers_for_symbol("LDOUSDT") == (2.0, 1.0), (
        "LDOUSDT must return (2.0, 1.0) at iter-v3/066 — DEFAULT fallback. "
        "V3_ATR_MULTIPLIERS_PER_SYMBOL must be empty; LDO falls back to DEFAULT."
    )
    # iter-v3/066: BCHUSDT returns DEFAULT (2.0, 1.0) — no per-symbol entry.
    assert atr_multipliers_for_symbol("BCHUSDT") == (2.0, 1.0), (
        "BCHUSDT must return (2.0, 1.0) at iter-v3/066 (DEFAULT fallback; "
...
def test_atr_multipliers_bch_default_fallback():
    """BCH must return (2.0, 1.0) — via DEFAULT fallback — at iter-v3/066.
    ...
    iter-v3/066: DEFAULT reverted to (2.0, 1.0); BCH still uses DEFAULT fallback.
    """
    ...
    assert "BCHUSDT" not in V3_ATR_MULTIPLIERS_PER_SYMBOL, (
        ...
        f"iter-v3/066. V3_ATR_MULTIPLIERS_PER_SYMBOL must be EMPTY (carry-forward). "
    ...
    assert bch_result == (2.0, 1.0), (
        ...
        f"iter-v3/066: BCH uses DEFAULT_ATR_MULTIPLIERS = (2.0, 1.0) via fallback "
        f"(V3_ATR_MULTIPLIERS_PER_SYMBOL empty)."
    )


def test_atr_multipliers_trx_default_fallback():
    """TRX must return (2.0, 1.0) via DEFAULT fallback at iter-v3/066.

    TRX has no per-symbol ATR entry (V3_ATR_MULTIPLIERS_PER_SYMBOL is empty at /066).
    """
    from crypto_trade.features_v3 import V3_ATR_MULTIPLIERS_PER_SYMBOL, atr_multipliers_for_symbol

    assert "TRXUSDT" not in V3_ATR_MULTIPLIERS_PER_SYMBOL, (
        f"V3_ATR_MULTIPLIERS_PER_SYMBOL has 'TRXUSDT' key — must be ABSENT at iter-v3/066. "
    ...
    assert trx_result == (2.0, 1.0), (
        ...
        f"iter-v3/066: DEFAULT_ATR_MULTIPLIERS = (2.0, 1.0); TRX uses DEFAULT fallback."
    )
...
def test_atr_multipliers_runner_dispatch():
    """The runner's _build_v3_model dispatches (2.0, 1.0) for ALL 3 symbols at iter-v3/066.
    ...
    iter-v3/066 axis isolation: DEFAULT reverted from /065's (2.0, 1.5) to (2.0, 1.0).
    ...
    # iter-v3/066 axis isolation: all 3 symbols use DEFAULT (2.0, 1.0) — /065 reverted.
    for sym, strat in [("BCHUSDT", strat_bch), ("LDOUSDT", strat_ldo), ("TRXUSDT", strat_trx)]:
        assert strat.inner.atr_tp_multiplier == 2.0, (
            ...
            "iter-v3/066: TP multiplier unchanged at 2.0×ATR for all symbols."
        )
        assert strat.inner.atr_sl_multiplier == 1.0, (
            f"{sym} atr_sl_multiplier: expected 1.0, got {strat.inner.atr_sl_multiplier}. "
            "iter-v3/066 axis isolation: /065 SL widening (1.5) reverted to 1.0. "
