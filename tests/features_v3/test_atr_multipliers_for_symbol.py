"""Adversarial tests for per-symbol ATR multipliers — iter-v3/047 (UPDATED iter-v3/065).

iter-v3/065 state (EXPLORATION — cycle 1 #6 — NON-FEATURE PIVOT; UNIVERSAL LABELING AXIS
                   Path D: DEFAULT_ATR_MULTIPLIERS (2.0, 1.0) → (2.0, 1.5)):
  - DEFAULT_ATR_MULTIPLIERS CHANGED to (2.0, 1.5).
    SL multiplier widened from 1.0×ATR to 1.5×ATR (TP unchanged at 2.0×ATR).
    EDA SHA `662659c`; analysis/iteration_v3-065/labeling_parameter_eda.py.
    Path D selected for highest LDO TP-hit-rate lift (+8.6pp: 30.8% → 39.4%)
    and largest LDO SL reduction (-9.6pp: 68.7% → 59.0%) of any labeling path.
  ...
  - Single-axis change (this iter): DEFAULT_ATR_MULTIPLIERS ONLY.
...
Validates (iter-v3/065 state):
  1. DEFAULT_ATR_MULTIPLIERS == (2.0, 1.5) at iter-v3/065 (widened SL).
...
def test_atr_multipliers_default():
    """DEFAULT_ATR_MULTIPLIERS == (2.0, 1.5) at iter-v3/065 (Path D universal SL widening).

    iter-v3/065: SL multiplier widened 1.0 → 1.5 universally. V3_ATR_MULTIPLIERS_PER_SYMBOL
    is EMPTY (SYSTEM-LEVEL REVERT carry-forward from /051). All 3 symbols (BCH/LDO/TRX)
    fall back to DEFAULT_ATR_MULTIPLIERS = (2.0, 1.5).
    """
    ...
    # iter-v3/065: TRX uses DEFAULT fallback (V3_ATR_MULTIPLIERS_PER_SYMBOL empty).
    assert atr_multipliers_for_symbol("TRXUSDT") == (2.0, 1.5)
    # iter-v3/065: ALGOUSDT falls back to DEFAULT (V3_ATR_MULTIPLIERS_PER_SYMBOL empty).
    assert atr_multipliers_for_symbol("ALGOUSDT") == (2.0, 1.5), (
        "ALGOUSDT must return (2.0, 1.5) at iter-v3/065 — DEFAULT fallback. "
        "V3_ATR_MULTIPLIERS_PER_SYMBOL must be empty; ALGO falls back to DEFAULT."
    )
    # iter-v3/065: LDOUSDT falls back to DEFAULT (V3_ATR_MULTIPLIERS_PER_SYMBOL empty).
    assert atr_multipliers_for_symbol("LDOUSDT") == (2.0, 1.5), (
        "LDOUSDT must return (2.0, 1.5) at iter-v3/065 — DEFAULT fallback. "
        "V3_ATR_MULTIPLIERS_PER_SYMBOL must be empty; LDO falls back to DEFAULT."
    )
    # iter-v3/065: BCHUSDT returns DEFAULT (2.0, 1.5) — no per-symbol entry.
    assert atr_multipliers_for_symbol("BCHUSDT") == (2.0, 1.5), (
        "BCHUSDT must return (2.0, 1.5) at iter-v3/065 (DEFAULT fallback; "
...
def test_atr_multipliers_bch_default_fallback():
    """BCH must return (2.0, 1.5) — via DEFAULT fallback — at iter-v3/065.
    ...
    iter-v3/065: DEFAULT changed to (2.0, 1.5); BCH still uses DEFAULT fallback.
    """
    ...
    assert "BCHUSDT" not in V3_ATR_MULTIPLIERS_PER_SYMBOL, (
        ...
        f"iter-v3/065. V3_ATR_MULTIPLIERS_PER_SYMBOL must be EMPTY (carry-forward). "
    ...
    assert bch_result == (2.0, 1.5), (
        ...
        f"iter-v3/065: BCH uses DEFAULT_ATR_MULTIPLIERS = (2.0, 1.5) via fallback "
        f"(V3_ATR_MULTIPLIERS_PER_SYMBOL empty)."
    )


def test_atr_multipliers_trx_default_fallback():
    """TRX must return (2.0, 1.5) via DEFAULT fallback at iter-v3/065.

    TRX has no per-symbol ATR entry (V3_ATR_MULTIPLIERS_PER_SYMBOL is empty at /065).
    """
    from crypto_trade.features_v3 import V3_ATR_MULTIPLIERS_PER_SYMBOL, atr_multipliers_for_symbol

    assert "TRXUSDT" not in V3_ATR_MULTIPLIERS_PER_SYMBOL, (
        f"V3_ATR_MULTIPLIERS_PER_SYMBOL has 'TRXUSDT' key — must be ABSENT at iter-v3/065. "
    ...
    assert trx_result == (2.0, 1.5), (
        ...
        f"iter-v3/065: DEFAULT_ATR_MULTIPLIERS = (2.0, 1.5); TRX uses DEFAULT fallback."
    )
...
def test_v3_atr_multipliers_per_symbol_is_empty():
    ...
    iter-v3/065 is a UNIVERSAL labeling change only — no per-symbol entries added.
    ...
    History:
      ...
      iter-v3/065: still EMPTY (universal DEFAULT change; no per-symbol overrides).
    """
    ...
    assert len(V3_ATR_MULTIPLIERS_PER_SYMBOL) == 0, (
        ...
        f"iter-v3/065 Path D is a universal DEFAULT change only — no per-symbol overrides."
    )


def test_atr_multipliers_runner_dispatch():
    """The runner's _build_v3_model dispatches (2.0, 1.5) for ALL 3 symbols at iter-v3/065.
    ...
    iter-v3/065 Path D: SL multiplier 1.0 → 1.5; TP multiplier unchanged at 2.0.
    """
    import sys
    ...
    # iter-v3/065 Path D: all 3 symbols use DEFAULT (2.0, 1.5).
    for sym, strat in [("BCHUSDT", strat_bch), ("LDOUSDT", strat_ldo), ("TRXUSDT", strat_trx)]:
        assert strat.inner.atr_tp_multiplier == 2.0, (
            ...
            "iter-v3/065 Path D: TP multiplier unchanged at 2.0×ATR for all symbols."
        )
        assert strat.inner.atr_sl_multiplier == 1.5, (
            f"{sym} atr_sl_multiplier: expected 1.5, got {strat.inner.atr_sl_multiplier}. "
            "iter-v3/065 Path D: universal SL widening from 1.0 → 1.5×ATR. "
