...
def test_required_gap_matches_formula() -> None:
    ...
    assert REQUIRED_GAP == 88, f"REQUIRED_GAP should be 88 for v3 (21+1)*4=88, got {REQUIRED_GAP}"
...
def test_gap_removes_boundary_samples() -> None:
    """With gap=88, boundary samples adjacent to test blocks are purged from train."""
    n = 200
    gap = 20
    splits = combinatorial_purged_cv(
        n_samples=n,
