...
from crypto_trade.team12 import backtest
...
from crypto_trade.team12.live import (
...
from crypto_trade.team12.report import (
...
def test_frozen_team12_authority_is_exact() -> None:
    ...
    assert historical_terminal_held_symbols() == (
        "1000PEPEUSDT",
        "ADAUSDT",
        "BNBUSDT",
        "BTCUSDT",
...
def test_forward_gate_uses_daily_metrics_but_keeps_bar_count() -> None:
    timestamps = pd.date_range("2026-08-03", periods=6, freq="8h", tz="UTC")
...
def test_absent_contract_lifecycle_uses_current_horizon_not_replay_boundary(
    ...
    monkeypatch.setattr(
        "crypto_trade.team12.live_data.current_boundary",
        lambda: horizon,
    )
    full = pd.DataFrame(
        {
...
def test_released_all_period_statistics_match_tournament() -> None:
    ...
    assert statistics["is"]["observations"] == 1062
    assert statistics["is_confirmation"]["observations"] == 366
    assert statistics["historical_oos"]["observations"] == 730
    assert statistics["all_periods"]["observations"] == 2158
    assert statistics["is"]["cumulative_return"] == pytest.approx(
        0.8498699295103118, abs=1e-15
    )
    assert statistics["is_confirmation"]["cumulative_return"] == pytest.approx(
        0.08764074192530047, abs=1e-15
    )
    assert statistics["historical_oos"]["cumulative_return"] == pytest.approx(
        0.4186496421196686, abs=1e-15
    )
    assert statistics["all_periods"]["cumulative_return"] == pytest.approx(
        1.8543144298674585, abs=1e-15
    )
    naive = load_released_daily_returns()
    naive.index = naive.index.tz_localize(None)
    assert period_statistics(naive)["all_periods"]["observations"] == 2158
