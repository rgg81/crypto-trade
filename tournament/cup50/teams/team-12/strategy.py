    ...
    def target_weights(self, context: DecisionContextV2, *, seed: int):
        ...
        for symbol in context.eligible_symbols:
            ...
            if history is not None and len(history) > self.trend_bars:
                ...
                trend[symbol] = float(close.iloc[-1] / close.iloc[-1 - self.trend_bars] - 1.0)
                recent[symbol] = float(close.iloc[-1] / close.iloc[-1 - self.reversal_bars] - 1.0)
            rates = context.funding.loc[
                context.funding["symbol"].astype(str) == symbol, "funding_rate"
            ].astype(float)
            if len(rates) >= self.funding_events:
