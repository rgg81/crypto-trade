...
class StrategyParameters:
    ...
    unwind_minimum_abs_funding_z: float = 0.75
    chop_minimum_abs_funding_z: float = 1.50
...
def _market_state(
    ...
    *,
    ...
    if frame is None:
        return "chop"
    required_bars = parameters.market_days * (24 // parameters.interval_hours) + 1
    history = _complete_history(
        frame,
        decision_time=_utc(context.decision_time),
        required_bars=required_bars,
    ...
    market_return = float(math.log(closes[-1] / closes[0]))
...
class FundingCrowdingReversal:
    ...
    def target_weights(
        ...
        *,
        ...
        eligible = {str(value) for value in context.eligible_symbols}
        state = _market_state(context, parameters=self.parameters)
        if state == "chop":
            target = _aggregate(
                self._extreme_vintages,
