"""Shrunk three-slot Monday seasonality held over one full calendar week."""
...
class StrategyParameters:
    ...
    minimum_weeks: int = 39
    center_cell_weight: float = 0.75
    adjacent_cell_weight: float = 0.25
...
def _weekly_returns(
    ...
    *,
    ...
    ].tail(parameters.lookback_weeks + 1)
    if len(data) < parameters.minimum_weeks + 1:
        return pd.Series(dtype=float)
    ...
    return result.tail(parameters.lookback_weeks)
...
def _cell_residuals(
    ...
    *,
    ...
    for symbol in sorted({str(value) for value in context.eligible_symbols}):
        ...
        if len(history) >= parameters.minimum_weeks:
            histories[symbol] = history
    if len(histories) < parameters.minimum_valid_symbols:
        return pd.DataFrame()
    returns = pd.DataFrame(histories, dtype=float).tail(parameters.lookback_weeks)
    returns = returns.dropna(axis=1, thresh=parameters.minimum_weeks)
...
def _cell_statistics(
    ...
    *,
    ...
    for symbol in sorted(residuals.columns):
        ...
        if len(raw) < parameters.minimum_weeks:
            continue
        values = _winsorized(raw[-parameters.lookback_weeks :], parameters.winsor_mad_multiplier)
...
def _seasonal_features(
    ...
    *,
    ...
    for hour in hours:
        residuals = _cell_residuals(context, hour_utc=hour, parameters=parameters)
        if residuals.empty:
            return {}
        statistics[hour] = _cell_statistics(residuals, parameters=parameters)
...
def _portfolio(
    ...
    *,
    ...
    shorts = [symbol for symbol in ordered if features[symbol].expected_return < 0.0][
        :side_count
    ]
    longs = [symbol for symbol in reversed(ordered) if features[symbol].expected_return > 0.0][
        :side_count
    ]
    if (
...
class MondayThreeSlotSeasonality:
    ...
    def target_weights(
        ...
        *,
        ...
        if not features:
            return {}
        return _portfolio(features, parameters=self.parameters)


def build_strategy() -> TargetStrategy:
    return MondayThreeSlotSeasonality()
