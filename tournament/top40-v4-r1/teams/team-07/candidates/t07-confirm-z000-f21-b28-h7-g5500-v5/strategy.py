"""Funding carry with three-day price confirmation at z=0.00."""
...
class StrategyParameters:
    ...
    confirmation_return_bars: int = 9
    confirmation_z: float = 0.00
    market_days: int = 28
    ...
    funding_pool_fraction_per_side: float = 0.50
...
def _price_states(
    ...
    *,
    ...
    for symbol in sorted(residuals.columns):
        ...
        state = float(
            np.sum(values[-parameters.confirmation_return_bars :])
            / (scale * math.sqrt(parameters.confirmation_return_bars))
        )
...
def _cohort(
    ...
    *,
    ...
    pool_count = max(
        ...
        int(math.floor(parameters.funding_pool_fraction_per_side * len(common))),
    )
    pool_count = min(pool_count, len(common) // 2)
    desired = max(
        parameters.minimum_positions_per_side,
        int(math.floor(parameters.selected_fraction_per_side * len(common))),
    )
    desired = min(desired, pool_count)
    long_pool = ordered[:pool_count]
    short_pool = list(reversed(ordered[-pool_count:]))
    longs = [
        symbol for symbol in long_pool if price[symbol] >= parameters.confirmation_z
    ][:desired]
    shorts = [
        symbol for symbol in short_pool if price[symbol] <= -parameters.confirmation_z
    ][:desired]
    ...
    long_weight = min(
        ...
        long_budget / len(longs) if longs else 0.0,
    )
    short_weight = min(
        parameters.maximum_symbol_weight,
        short_budget / len(shorts) if shorts else 0.0,
    )
    ...
    if net > parameters.maximum_abs_net and long_gross > 0.0:
        ...
        result = {
            symbol: (value * scale if value > 0.0 else value)
    ...
    if (
        ...
        or any(
            abs(value) > parameters.maximum_symbol_weight + 1e-12
