"""Team 12 center ensemble with dual absolute-trend confirmation."""
...
class StrategyParameters:
    ...
    minimum_confirmed_long_positions: int = 4
    minimum_confirmed_short_positions: int = 4
...
def _confirmed_long_symbols(
    ...
    *,
    ...
    if raw_returns.empty:
        return frozenset()
    result: set[str] = set()
    for symbol in sorted(raw_returns.columns):
        values = raw_returns[symbol].to_numpy(dtype=float)
        if len(values) < parameters.long_confirmation_bars:
            continue
        absolute_momentum = float(
            np.expm1(np.sum(values[-parameters.long_confirmation_bars :]))
        )
        if math.isfinite(absolute_momentum) and absolute_momentum > 0.0:
            result.add(symbol)
    return frozenset(result)


def _confirmed_short_symbols(
    raw_returns: pd.DataFrame,
    *,
    regime: str,
    parameters: StrategyParameters,
) -> frozenset[str]:
    if regime != "bear" or raw_returns.empty:
        return frozenset()
    result: set[str] = set()
    for symbol in sorted(raw_returns.columns):
        values = raw_returns[symbol].to_numpy(dtype=float)
        if len(values) < parameters.momentum_bars:
            continue
        absolute_momentum = float(np.expm1(np.sum(values[-parameters.momentum_bars :])))
        if math.isfinite(absolute_momentum) and absolute_momentum < 0.0:
            result.add(symbol)
    return frozenset(result)
...
def _select_sides(
    ...
    *,
    confirmed_long_symbols: frozenset[str],
    confirmed_short_symbols: frozenset[str],
    ...
    long_pool = [symbol for symbol in reversed(ordered) if symbol in confirmed_long_symbols]
    long_target_count = min(side_count, len(long_pool))
    if long_target_count < parameters.minimum_confirmed_long_positions:
        long_pool = []
        long_target_count = 0
    long_outer_count = min(
        len(long_pool),
        max(long_target_count, outer_count),
    )
    short_pool = [symbol for symbol in ordered if symbol in confirmed_short_symbols]
    short_target_count = min(side_count, len(short_pool))
    if short_target_count < parameters.minimum_confirmed_short_positions:
        short_pool = []
        short_target_count = 0
    short_outer_count = min(
        len(short_pool),
        max(short_target_count, outer_count),
    )
    short_outer = set(short_pool[:short_outer_count])
    long_outer = set(long_pool[:long_outer_count])
    longs = [
        symbol for symbol in sorted(previous_longs) if symbol in long_outer
    ][:long_target_count]
    shorts = [
        symbol for symbol in sorted(previous_shorts) if symbol in short_outer
    ][:short_target_count]
    for symbol in long_pool:
        if len(longs) >= long_target_count:
            break
        if symbol not in longs and symbol not in shorts:
            longs.append(symbol)
    for symbol in short_pool:
        if len(shorts) >= short_target_count:
            break
...
def _portfolio(
    ...
    *,
    ...
    if not longs and not shorts:
        return {}
    if longs and len(longs) < parameters.minimum_confirmed_long_positions:
        return {}
    if shorts and len(shorts) < parameters.minimum_confirmed_short_positions:
        return {}
    total_gross = _gross_for_regime(regime, parameters)
    long_side_gross = min(
        total_gross / 2.0,
        parameters.maximum_symbol_weight * len(longs),
    )
    short_side_gross = min(
        total_gross / 2.0,
        parameters.maximum_symbol_weight * len(shorts),
    )
    long_weight = long_side_gross / len(longs) if longs else 0.0
    short_weight = short_side_gross / len(shorts) if shorts else 0.0
...
class WeeklyRegimeEnsemble:
    ...
    def target_weights(
        ...
        *,
        ...
        confirmed_longs = _confirmed_long_symbols(
            raw_returns,
            parameters=self.parameters,
        )
        confirmed_shorts = _confirmed_short_symbols(
            raw_returns,
            regime=regime,
            parameters=self.parameters,
        )
        longs, shorts = _select_sides(
            scores,
            confirmed_long_symbols=confirmed_longs,
            confirmed_short_symbols=confirmed_shorts,
