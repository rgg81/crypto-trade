"""Team 12 high-coordinate regime ensemble with bear-only short confirmation."""
...
class StrategyParameters:
    ...
    lookback_return_bars: int = 336
    momentum_bars: int = 84
    reversal_bars: int = 6
    downside_bars: int = 126
    regime_lag_bars: int = 3
    btc_direction_bars: int = 240
...
def _is_scheduled(decision_time: pd.Timestamp, parameters: StrategyParameters) -> bool:
    anchor = pd.Timestamp("1970-01-05T00:00:00Z")
    day_offset = int((decision_time.normalize() - anchor).days)
    return bool(
        parameters.rebalance_days > 0
        and day_offset % parameters.rebalance_days == 0
...
def _portfolio(
    ...
    *,
    ...
    long_side_gross = total_gross / 2.0
    short_side_gross = min(
        total_gross / 2.0,
        parameters.maximum_symbol_weight * len(shorts),
    )
    long_weight = long_side_gross / len(longs)
