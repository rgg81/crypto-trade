"""Team 10 dynamic relative value with a causal Engle-Granger screen."""
...
class StrategyParameters:
    ...
    formation_bars: int = 378
    correlation_return_bars: int = 189
...
def _candidate_graph(
    ...
    *,
    ...
    required = parameters.correlation_return_bars + 1
    for left_index, left in enumerate(symbols):
        left_returns = np.diff(histories[left].to_numpy(dtype=float)[-required:])
        if float(np.std(left_returns)) <= 1e-12:
            continue
        for right in symbols[left_index + 1 :]:
            right_returns = np.diff(histories[right].to_numpy(dtype=float)[-required:])
    ...
    for symbol in symbols:
        ...
        for other in symbols:
            ...
            if correlation is not None and correlation >= parameters.minimum_correlation:
                neighbors.append((correlation, other))
        neighbors.sort(key=lambda item: (-item[0], item[1]))
        for _correlation, other in neighbors[: parameters.neighbors_per_symbol]:
...
def _passes_cointegration_screen(
    y: np.ndarray,
    x: np.ndarray,
    *,
    parameters: StrategyParameters,
    ...
    try:
        _statistic, pvalue, _critical_values = coint(
            y,
            x,
            trend="c",
            method="aeg",
            maxlag=parameters.cointegration_maxlag,
            autolag=None,
        )
    except (FloatingPointError, ValueError, np.linalg.LinAlgError):
        return False
    return bool(
        math.isfinite(float(pvalue))
        and float(pvalue) <= parameters.maximum_cointegration_pvalue
    )
...
def _pair_signal(
    ...
    *,
    ...
    if len(left) != parameters.formation_bars or len(right) != parameters.formation_bars:
        return None
    if not _passes_cointegration_screen(left[:-1], right[:-1], parameters=parameters):
        return None
