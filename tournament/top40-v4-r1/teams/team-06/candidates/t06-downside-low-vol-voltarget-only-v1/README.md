# Team 06 volatility-target-only ablation

This mandatory individual-control ablation estimates each eligible coin's beta to the contemporaneous median-crypto
...
gross exposure are behaviorally identical to the parent. The adjacent policy enables only the
30-day, 25%-annualized downward volatility target. Drawdown brakes, position and time stops, and the
turnover throttle remain disabled. Hard strategy and protocol limits remain in force. This is an
individual-control ablation, not a nomination candidate.
