"""Crypto-breadth hysteresis weekly fast breakout for Team 02."""
...
class StrategyParameters:
    ...
    breadth_days: int = 63
    breadth_bear_threshold: float = 0.45
...
def _scores(
    ...
    *,
    ...
    required_bars = max(channel_bars, confirmation_bars, breadth_bars) + 1
    interval = pd.Timedelta(hours=parameters.interval_hours)
    scores: dict[str, float] = {}
    breadth_returns: dict[str, float] = {}
    ...
    for symbol in sorted({str(value) for value in context.eligible_symbols}):
        ...
        breadth_return = float(np.log(closes[-1] / closes[-breadth_bars - 1]))
        ...
        if math.isfinite(score):
            ...
            breadth_returns[symbol] = breadth_return

    if len(breadth_returns) < parameters.minimum_valid_symbols:
        return {}, None
    positive_breadth = float(
        np.mean([value > 0.0 for value in breadth_returns.values()])
    )
    ...
    def target_weights(
        ...
        *,
        ...
        if not _scheduled(decision_time, self.parameters):
            return None
        scores, positive_breadth = _scores(context, parameters=self.parameters)
        if positive_breadth is None:
            return {}
        if positive_breadth >= self.parameters.breadth_bull_threshold:
            self._regime_side = 1
        elif positive_breadth <= self.parameters.breadth_bear_threshold:
            self._regime_side = -1
        elif self._regime_side == 0:
            self._regime_side = 1 if positive_breadth >= 0.5 else -1
        scores = {
            symbol: score if score * self._regime_side > 0.0 else 0.0
