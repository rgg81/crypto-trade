"""State-gated volume momentum with seven-day price persistence for longs."""
...
def _scores(
    ...
    *,
    ...
    for symbol in sorted({str(value) for value in context.eligible_symbols}):
        ...
        if direction > 0.0:
            confirmation_bars_price = (
                parameters.long_price_confirmation_days * bars_per_day
            )
            recent_price_return = float(np.sum(trend_returns[-confirmation_bars_price:]))
            if recent_price_return <= 0.0:
                continue
