"""Volume-confirmed momentum with persistent longs and inverse-volatility sleeves."""
...
class StrategyParameters:
    ...
    long_price_confirmation_days: int = 7
    ...
    for symbol in sorted({str(value) for value in context.eligible_symbols}):
        ...
        if direction > 0.0:
            price_confirmation_bars = (
                parameters.long_price_confirmation_days * bars_per_day
            )
            if float(np.sum(trend_returns[-price_confirmation_bars:])) <= 0.0:
                continue
        ...
        if positive_recent.size == 0 or volume_sum <= 0.0:
            continue
        support = float(
            np.sum(recent_volumes[recent_returns * direction > 0.0]) / volume_sum
        )
        ...
        if math.isfinite(score):
            ...
            realized_volatility[symbol] = realized
    return scores, realized_volatility
...
def _inverse_volatility_weights(
    ...
    *,
    ...
    if not symbols or gross_budget <= 0.0:
        return {}
    remaining = list(sorted(symbols))
    remaining_gross = gross_budget
    weights: dict[str, float] = {}
    while remaining and remaining_gross > 1e-12:
        inverse = {symbol: 1.0 / realized_volatility[symbol] for symbol in remaining}
        inverse_sum = math.fsum(inverse.values())
        if not math.isfinite(inverse_sum) or inverse_sum <= 0.0:
            return {}
        provisional = {
            symbol: remaining_gross * inverse[symbol] / inverse_sum
            for symbol in remaining
        }
        capped = [
            symbol
            for symbol in remaining
            if provisional[symbol] >= maximum_symbol_weight - 1e-12
        ]
        if not capped:
            weights.update(provisional)
            break
        for symbol in capped:
            weights[symbol] = maximum_symbol_weight
        capped_set = set(capped)
        remaining = [symbol for symbol in remaining if symbol not in capped_set]
        remaining_gross = gross_budget - math.fsum(weights.values())
    return weights
...
def _portfolio(
    ...
    *,
    ...
    if not shorts:
        short_gross = 0.0
    long_weights = _inverse_volatility_weights(
        longs,
        realized_volatility,
        gross_budget=long_gross,
        maximum_symbol_weight=parameters.maximum_symbol_weight,
    )
    short_weights = _inverse_volatility_weights(
        shorts,
        realized_volatility,
        gross_budget=short_gross,
        maximum_symbol_weight=parameters.maximum_symbol_weight,
    )
    targets = {symbol: -weight for symbol, weight in short_weights.items()}
    targets.update(long_weights)
    ...
    def target_weights(
        ...
        *,
        ...
        if not _scheduled(decision_time, self.parameters):
            return None
        scores, realized_volatility = _scores(context, parameters=self.parameters)
