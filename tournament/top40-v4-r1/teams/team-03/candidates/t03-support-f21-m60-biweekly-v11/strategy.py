"""Biweekly version of the Team 03 state-gated volume-momentum center."""
...
def _scheduled(decision_time: pd.Timestamp, parameters: StrategyParameters) -> bool:
    ...
    if not is_boundary:
        return False
    anchor = pd.Timestamp("2020-01-06T00:00:00Z")
    elapsed_days = int((decision_time - anchor) / pd.Timedelta(days=1))
    return elapsed_days >= 0 and elapsed_days % parameters.rebalance_period_days == 0
