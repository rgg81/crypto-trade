"""Countertrend-confirmed fast-market-state momentum for Team 01."""
...
def _portfolio(
    ...
    *,
    ...
    if market_return is not None and market_return > parameters.market_threshold:
        negative_symbols = [symbol for symbol in negative_symbols if symbol in confirmed_symbols]
    elif market_return is not None and market_return < -parameters.market_threshold:
        positive_symbols = [symbol for symbol in positive_symbols if symbol in confirmed_symbols]
    longs = sorted(
        positive_symbols,
    ...
    def target_weights(
        ...
        *,
        ...
        if not _scheduled(decision_time, self.parameters):
            return None
        scores, confirmed_symbols = _scores(context, parameters=self.parameters)
