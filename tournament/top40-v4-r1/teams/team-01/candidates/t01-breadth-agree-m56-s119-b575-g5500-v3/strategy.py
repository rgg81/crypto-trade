"""Breadth-aware, horizon-confirmed slow per-coin momentum for Team 01."""
...
class StrategyParameters:
    ...
    breadth_threshold: float = 0.575
    dominant_side_fraction: float = 0.72
    total_gross: float = 0.55
    maximum_symbol_weight: float = 0.08
...
def _portfolio(
    ...
    *,
    ...
    positive_symbols = [symbol for symbol, score in scores.items() if score > 0.0]
    negative_symbols = [symbol for symbol, score in scores.items() if score < 0.0]
    longs = sorted(
        positive_symbols,
    ...
    positive_fraction = len(positive_symbols) / (len(positive_symbols) + len(negative_symbols))
    if positive_fraction >= parameters.breadth_threshold:
        long_fraction = parameters.dominant_side_fraction
    elif positive_fraction <= 1.0 - parameters.breadth_threshold:
