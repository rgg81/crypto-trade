"""Team 04 ABLATION -- the same book ranked on RAW trailing return.
...
"""Fraction of the eligible cross-section taken into each sleeve (thirds, per the charter)."""

MINIMUM_MEMBERS = 9
"""Below this many usable names the cross-section is too thin to rank; the book holds."""

MARKET_REMOVAL = 0.0
"""How much of the estimated market component is removed. 1.0 is the lane, 0.0 is the ablation."""
...
def _score(context: DecisionContext) -> dict[str, float]:
    ...
    formation_y = window_y[-FORMATION_BARS:]
    formation_x = window_x[-FORMATION_BARS:]
    if formation_y.shape[0] < FORMATION_BARS:
        return {}
    residual = formation_y.sum(axis=0) - MARKET_REMOVAL * beta * float(formation_x.sum())
    return {
        symbol: float(value)
        for symbol, value in zip(symbols, residual, strict=True)
...
class MarketResidualThirds:
    ...
    def target_weights(
        ...
        width = max(1, int(round(len(ordered) * SLEEVE_FRACTION)))
        shorts = ordered[:width]
        longs = ordered[-width:]
