"""Team 08 CONTROL -- the funding LEVEL driven through the nominee's identical machinery.
...
DISPERSION_GATE = 0.40  # trailing percentile of cross-sectional dispersion required to open
...
def _term_spreads(context: DecisionContext) -> dict[str, float]:
    ...
    for symbol in sorted(context.eligible_symbols):
        ...
        # CONTROL: the LEVEL, not the term spread. Everything else in this file is the
        # nominee's, byte for byte, so any difference in the result is the score and nothing else.
        value = float(np.mean(regime))
