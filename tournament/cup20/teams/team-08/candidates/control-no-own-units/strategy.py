"""Team 08 CONTROL -- own-units normalisation REMOVED, dispersion gate kept.
...
def _term_spreads(context: DecisionContext) -> dict[str, float]:
    ...
    for symbol in sorted(context.eligible_symbols):
        ...
        # CONTROL: the raw funding CHANGE, with the own-units denominator removed. Everything
        # else is the nominee's byte for byte, so the difference isolates the normalisation.
        value = float(np.mean(own[-SHORT_WINDOW:])) - float(np.mean(regime))
