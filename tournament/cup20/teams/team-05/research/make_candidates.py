"""Generate team-05's candidate directories from one template.

Every candidate is a standalone ``strategy.py`` -- no shared import, no package -- because the
neighbourhood sweep materialises each point by rewriting literals in that one file and then proves
the rest of it is byte-identical. A candidate that imported its logic from elsewhere would put the
logic outside the file the sweep can see.

The ablations differ from the nominee ONLY in the constants named at the top. Every other byte is
the same template, so an ablation's result is attributable to the control it removes rather than to
an incidental rewrite. Removing a control is expressed as a value that disables it:

  FLOW_SPIKE   = 0.0   -- every bar's trade count clears it, so the forced-flow signature is off
  MIN_BREADTH  = 1     -- one shocked name is a cascade, so the breadth condition is off
  ENTRY_DELAY  = 0     -- buy the bar straight after the shock instead of letting it finish
"""
...
BASE = dict(shock_z="-2.0", flow_spike="2.0", basket_names="8", min_breadth="2",
            lookback_bars="60", entry_delay="1", hold_bars="3")
...
CANDIDATES = {
    ...
    "baseline-plain-reversal": (
        "TRANSPARENT BASELINE / CONTROLS OFF. Buy the deepest fallers after any single-name down\n"
        "move beyond its own scale. No forced-flow signature, no breadth condition. This is the\n"
        "plain reversal the lane must beat to have said anything.",
        {"flow_spike": "0.0", "min_breadth": "1"},
    ),
    ...
    "ablate-breadth": (
        "INDIVIDUAL CONTROL. Forced-flow signature on, breadth OFF. Isolates what requiring the\n"
        "signature in several names at once contributes over requiring it in one.",
        {"min_breadth": "1"},
    ),
    "formation-30": (
...
def main() -> None:
    for cid, (headline, overrides) in CANDIDATES.items():
        ...
        (directory / "strategy.py").write_text(TEMPLATE.format(cid=cid, headline=headline, **values))
        ...
        (directory / "README.md").write_text(
            ...
            + "\nRisk policy: everything off, `volatility_target.enabled = false`"
            " (charter section 6, amendment A3).\n"
        )
