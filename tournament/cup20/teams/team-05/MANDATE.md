...
Lane id: `short-horizon-liquidity-shock-reversal`
Playbook: `tournament/cup20/TEAM-PLAYBOOK.md` (read it before you write any code)
Workspace: `tournament/cup20/teams/team-05/`
Data root: `data/cup20/is/`

## Mechanism intent

Liquidation cascades and forced unwinds push price further than information does, and the overshoot
is mechanical — it happens when a move outruns the liquidity available to absorb it. Your lane is
the snap-back: short-horizon reversal conditioned on a shock *signature* (range relative to its own
recent scale, volume or trade-count spike, funding dislocation) rather than on return magnitude
alone. Reversal at 8h cadence is inherently high-turnover, so the turnover and cost-density floors
bind on you earlier and harder than on any other lane.
