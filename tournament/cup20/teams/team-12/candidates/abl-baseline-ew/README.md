# team-12 / abl-baseline-ew

Control (ablation): the transparent baseline.

Equal-weight long the whole eligible top-20 cross-section, on the same rebalance cadence and
phase as the nominee. Every sleeve is still computed and then discarded, so the only thing that
differs from the nominee is the book itself. Journaled as an ablation and therefore never
nominable.
