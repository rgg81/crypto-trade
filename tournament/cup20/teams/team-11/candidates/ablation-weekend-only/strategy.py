"""Team 11 -- ABLATION (weekly half of the cell only). Journaled with --kind ablation: a control, never a nominee.

INDIVIDUAL CONTROL. The contender's cell is (weekend OR the 00:00-08:00 UTC settlement session).
This variant keeps only the weekend, so the difference between this run and the contender is
exactly the contribution of the intraday settlement-clock cell.
...
def _is_desk_closed(open_time: pd.Timestamp) -> bool:
    """Weekly cell only: Saturday and Sunday UTC."""
    return open_time.dayofweek in _WEEKEND_DAYS
