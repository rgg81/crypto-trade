"""Team 11 -- ABLATION (intraday half of the cell only). Journaled with --kind ablation: a control, never a nominee.

INDIVIDUAL CONTROL. The contender's cell is (weekend OR the 00:00-08:00 UTC settlement session).
This variant keeps only the settlement session, so the difference between this run and the
contender is exactly the contribution of the working-week cell.
...
def _is_desk_closed(open_time: pd.Timestamp) -> bool:
    """Intraday cell only: the 00:00-08:00 UTC settlement session."""
