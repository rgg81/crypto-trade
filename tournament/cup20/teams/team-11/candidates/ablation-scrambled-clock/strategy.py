"""Team 11 -- ABLATION (the clock removed). Journaled with --kind ablation: a control, never a nominee.

The contender's cell is chosen by the clock: the 00:00-08:00 UTC settlement session and the
weekend. Here the cell is chosen by a deterministic scramble of identical shape -- each calendar
day independently permutes its OWN three settlement slots (so every pseudo cell still receives
exactly one bar per day) and each week independently nominates two of its OWN seven days as the
pseudo weekend (so the pseudo weekend is always exactly two days). Cell size, coin mix and
volatility mix are held exactly; only the alignment with the real settlement and working-week
clock is destroyed. The permutation is a fixed integer hash of the bar index, so it is
deterministic and uses nothing the contender does not already see.

If the contender's edge is really the clock's, this book should have none.

Everything else is byte-identical in behaviour to the contender ``desk-closed-share``: same
activity quantity (quote volume), same windows, same volatility projection, same weekly cadence,
same phase, same rank weighting, same eligibility rules.
"""
...
_SCRAMBLE_SALT = 2654435761
_WEEK_SALT = 40503
_SLOT_PERMUTATIONS = (
    (0, 1, 2), (0, 2, 1), (1, 0, 2), (1, 2, 0), (2, 0, 1), (2, 1, 0),
)
_DAY_PAIRS = tuple((a, b) for a in range(7) for b in range(a + 1, 7))


def _mix(value: int) -> int:
    """A fixed integer hash of the bar index. Deterministic and past-only."""
    value = (value ^ 61) ^ (value >> 16)
    value = (value + (value << 3)) & 0xFFFFFFFF
    value ^= value >> 4
    value = (value * _SCRAMBLE_SALT) & 0xFFFFFFFF
    value ^= value >> 15
    return value


def _is_desk_closed(open_time: pd.Timestamp) -> bool:
    """Shape-matched pseudo cell: one slot per day plus two days per week, chosen by hash."""
    index = int((open_time - _MONDAY_EPOCH) // pd.Timedelta(hours=8))
    day_index = index // 3
    if _SLOT_PERMUTATIONS[_mix(day_index) % 6][index % 3] == _DESK_CLOSED_SLOT:
        return True
    pair = _DAY_PAIRS[_mix((day_index // 7) * _WEEK_SALT + 7) % len(_DAY_PAIRS)]
    return (day_index % 7) in pair
