"""Team 11 -- ABLATION (volatility projection removed). Journaled with --kind ablation: a control, never a nominee.

CONTROLS-OFF. The contender projects realised volatility out of the calendar measure, both to keep
the edge inside this lane and to remove the short high-beta tilt the raw measure carries. Here the
projection is removed and the raw desk-closed share is traded directly. The difference between
this run and the contender is exactly what the neutralisation costs and buys.
...
class DeskClosedShare:
    ...
    def target_weights(self, context, *, seed: int) -> Mapping[str, float] | None:
        ...
        _ = volatility_rank  # measured, deliberately not projected out in this ablation
        return _rank_weights(share_rank)
