"""Team 11 -- ABLATION (three-day rebalance instead of weekly). Journaled with --kind ablation: a control, never a nominee.

SECOND REBALANCE HORIZON. The contender rebalances every 21 bars (one week). This variant
rebalances every 9 bars (three days) at the same phase, so the difference between this run and the
contender is exactly the holding horizon.
