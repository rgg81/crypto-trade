...
Construction, per coin, from that coin's own closed bars and nothing else:
    ...
    z(l)       sum(r over the last l bars) / (sigma * sqrt(l)), a trend t-statistic,
               for l over a two-rung ladder at 1 and 2 times FORMATION_BARS
...
*The ladder* averages two estimates of one latent quantity -- how strongly this coin is trending
...
Two rungs rather than four, at two and four weeks rather than starting at five days: a rung much
faster than the anchor contributes most of the book's weight churn and least of its edge, and a
strategy that cannot be traded inside a 25x annual turnover budget is not a strategy.
...
class SlowTimeSeriesMomentum:
    ...
    def target_weights(
        ...
        legs = [
            max(3, formation * rung)
            for rung in (LADDER_RUNG_NEAR, LADDER_RUNG_FAR)
        ]
