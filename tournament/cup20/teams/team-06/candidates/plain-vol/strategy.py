"""Team 06 ABLATION -- the identical book selecting on TOTAL VOLATILITY instead.
...
class TotalVolatilityTercile:
    """Long the lowest-volatility third of the point-in-time top-20, short the highest."""
    ...
    def target_weights(
        ...
        # THE ABLATION. The downside/asymmetry content is removed and nothing else changes:
        # the same universe, formation window, breadth, cadence, phase, beta tilt and weights,
        # selecting on the width of the whole distribution rather than on its left side.
        score = _zscore(matrix.std(axis=1))
...
def build_strategy() -> TotalVolatilityTercile:
    return TotalVolatilityTercile()


__all__: Sequence[str] = ("build_strategy", "TotalVolatilityTercile")
