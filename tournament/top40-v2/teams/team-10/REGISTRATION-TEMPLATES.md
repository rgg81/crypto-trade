...
`scripts/top40_v2_tournament_runtime_preload_v7.py`.
...
risk hash. Do not register a control unless the completed core has passed every numeric activation
threshold and official non-neighbor gate in `qualification_thresholds.json`. For any later policy,
first copy its immutable template byte-for-byte to root,
...
those artifacts exist. All 12 predeclared neighbors must be registered before the first neighbor
result, and qualification remains withheld until their complete aggregation passes. `families.jsonl`
and `experiments.jsonl` are organizer projections and must never be hand edited.
