"""Iter 183: screen XLM with R1+R2 active.

XLM is the lowest-correlation candidate found so far (ρ=0.47 with BTC).
Mechanical diversifier if it passes Gate 3.
"""
...
def main() -> None:
    print("=" * 60)
    print(f"ITER {ITERATION} — {SYMBOL} with R1+R2 active")
    print("=" * 60)
    ...
    report_dir = generate_iteration_reports(
        trades=results,
        iteration=f"{ITERATION}_{SYMBOL.replace('USDT','').lower()}",
        features_dir="data/features",
        reports_dir="reports",
        interval="8h",
        n_trials=ITERATION,
