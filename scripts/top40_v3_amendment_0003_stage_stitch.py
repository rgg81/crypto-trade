#!/usr/bin/env python3
"""Closed command surface for the Amendment 0003 scored-stage addendum."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Top40 V3 scored-stage concatenation")
    parser.add_argument("--root", default=".", help="repository root")
    commands = parser.add_subparsers(dest="command", required=True)
    freeze = commands.add_parser("freeze", help="freeze the prospective stage-stitch addendum")
    freeze.add_argument("--implementation-commit", required=True)
    commands.add_parser("validate", help="verify the frozen addendum")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "freeze":
            provisional = stage_stitch.freeze_activation(
                arguments.root, implementation_commit=arguments.implementation_commit
            )
            result = {
                "addendum_id": stage_stitch.ADDENDUM_ID,
            ...
            result = stage_stitch.validate(arguments.root)
        elif arguments.command == "assess":
            result = stage_stitch.assess(arguments.root)
        elif arguments.command == "report":
            result = stage_stitch.report(arguments.root)
        else:  # pragma: no cover
            raise stage_stitch.StageStitchAddendumError("unknown command")
