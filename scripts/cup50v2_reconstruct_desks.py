...
def _daily(returns: pd.Series) -> pd.Series:
    """8h bar returns compounded into UTC days, the tournament's own convention."""
    frame = pd.to_numeric(returns, errors="raise").astype(float)
    grouped = (1.0 + frame).groupby(frame.index.floor("1D")).prod() - 1.0
...
def main() -> int:
    ...
    snapshot = stitch_snapshots(
        ...
        load_snapshot(root / "data" / "cup50v2" / "sealed"),
    )
    ...
    for desk_id in desk_ids:
        ...
        score = float(score_point(replay.costs).score)
        expected = published.get(desk.team_id)
        parity = True
        if expected is not None:
            parity = math.isclose(score, expected, rel_tol=0.0, abs_tol=1e-9)
            if not parity:
                raise SystemExit(
                    f"{desk_id}: replay does not reproduce the published centre score; "
                    f"{score!r} != {expected!r}"
                )
        ...
        daily = _daily(realistic.returns)
        ledger = paper / "ledger"
