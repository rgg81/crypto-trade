#!/usr/bin/env python3
"""Canonical organizer CLI for the Top-12 V1 tournament."""
...
from crypto_trade.tournament import orchestrator_top12_v1
...
def _parser() -> argparse.ArgumentParser:
    ...
    commands.add_parser("close-is", help="rank eligible nominees and freeze at most five finalists")
    commands.add_parser(
        "historical-release",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "validate":
            result = orchestrator_top12_v1.validate(
                root,
                require_activation=not arguments.pre_activation,
            )
        elif arguments.command == "activate":
            result = orchestrator_top12_v1.activate(root)
        elif arguments.command == "status":
            result = orchestrator_top12_v1.status(root)
        elif arguments.command == "is-run":
            result = orchestrator_top12_v1.run_is(
                root,
                arguments.team_id,
            ...
            result = orchestrator_top12_v1.nominate(
                root,
                arguments.team_id,
            ...
            result = orchestrator_top12_v1.retire(root, arguments.team_id, reason=arguments.reason)
        elif arguments.command == "close-is":
            result = orchestrator_top12_v1.close_is(root)
        elif arguments.command == "historical-release":
            result = orchestrator_top12_v1.historical_release(root)
        else:  # pragma: no cover - argparse owns this invariant.
            raise AssertionError(arguments.command)
    except (OSError, TypeError, ValueError, RuntimeError) as exc:
        sys.stderr.write(f"top12-v1: {exc}\n")
        return 2
