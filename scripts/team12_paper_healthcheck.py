#!/usr/bin/env python3
"""Read-only integrity and liveness check for the Team 12 paper desk."""
...
from crypto_trade.team12.backtest import LIVE_FORWARD_START
from crypto_trade.team12.live import _gate_frame
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Team 12 paper-desk healthcheck")
    parser.add_argument(
        "--paper-dir",
        type=Path,
        default=ROOT / "paper-team12",
    )
    parser.add_argument(
        "--skip-process",
        action="store_true",
...
def _check_process(paper: Path, alerts: list[str], notes: list[str]) -> None:
    ...
    try:
        ...
        return
    if "run_team12_paper.py" not in cmdline or cwd != ROOT:
        alerts.append(
...
def main() -> int:
    ...
    try:
        ...
        if len(members) != 12 or len(set(members)) != 12:
            alerts.append("MEMBERSHIP BAD: current universe is not exactly 12 names")
...
def _print_result(alerts: list[str], notes: list[str]) -> None:
    print(f"STATUS {'ALERT' if alerts else 'OK'} — Team 12 exact-replay paper desk")
    for alert in alerts:
        print(f"  ALERT: {alert}")
    for note in notes:
        print(f"  {note}")
