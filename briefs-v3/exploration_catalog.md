...
> **BANNER (updated 2026-05-06 at iter-v3/013 Phase 8): MKR-drop rule EXECUTED CLEANLY at iter-v3/013 (Critic FINAL SHA `1ee0213`, OVERALL=EXPLORATION-PROMISING-MECHANICAL — new subtype). iter-v3/014 MANDATORY AXIS: ADX threshold (currently 20; 18 looser OR 25 tighter) per Critic FINAL Recommendation 1.** ADX threshold is structurally orthogonal to all 5 prior axes (features×2, labeling×1, gate-zscore×1, gate-btc-trend×1, universe×1) and NOT subject to mechanical-accretion artifact. iter-v3/014 first commit MUST fix stale `= 88` docstrings at `run_baseline_v3.py:206, 211, 218` AND saturation predictor falsifier MUST derive `falsifier_threshold = ceil(1.2 × counterfactual_n_trades)` per Critic FINAL Recommendation 3. ±25% BTC band looser-direction test deferred to iter-v3/015+.
...
**6** of **10 required**. iter-v3/014 onwards needs **4 more EXPLORATIONS** before any CONFIRMATION can launch.

Axis coverage after iter-v3/013: **features × 2** (007, 009), **labeling × 1** (010), **gate-zscore × 1** (011), **gate-btc-trend × 1** (012), **universe × 1** (013) = 5 unique axis representations. Per Critic FINAL Recommendation 1 of iter-v3/013 review (SHA `1ee0213`), **iter-v3/014 axis is ADX threshold (currently 20) — 18 (looser, more trades) OR 25 (tighter, more selective)**. ADX threshold is structurally orthogonal to all 5 prior axes and NOT subject to mechanical-accretion artifact. After iter-v3/014 the catalog will have axis coverage features × 2 + labeling × 1 + gate-zscore × 1 + gate-btc-trend × 1 + universe × 1 + gate-adx × 1 = 6 unique axis representations. Alternative axes deferred to iter-v3/015+: ±25% BTC band looser direction (deferred since iter-v3/012), low-vol filter floor (0.33), vol-scaling clip range ([0.3, 1.0]).

iter-v3/014 first commit MUST also fix stale `= 88` docstrings + comments at `run_baseline_v3.py:206, 211, 218` (parametrize against `REQUIRED_GAP`, e.g., `f"= {REQUIRED_GAP}"`) AND saturation predictor falsifier in iter-v3/014 brief Section 8 MUST derive `falsifier_threshold = ceil(1.2 × counterfactual_n_trades)` not hardcoded — per Critic FINAL Recommendation 3.
