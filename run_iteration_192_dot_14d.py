"""Iter 192: DOT with 14-day label/backtest timeout.

EDA finding: 30% of DOT trades hit the 7-day timeout, and those trades
have WR 78-86% with mean PnL +2.9-4.7%. These are trades that worked
but slowly. Extending timeout to 14 days may:
  - Convert timeout trades into TP hits (bigger wins)
  - Or convert some into SL losses (risk)

Tests: does DOT signal benefit from a longer holding horizon?
"""
...
ITERATION = 192
SYMBOL = "DOTUSDT"
TIMEOUT_MIN = 14 * 24 * 60  # 14 days = 20160 minutes
...
def main() -> None:
    print("=" * 60)
    print(f"ITER {ITERATION} — {SYMBOL} with 14-day timeout "
          f"(label_timeout_minutes={TIMEOUT_MIN}, timeout_minutes={TIMEOUT_MIN})")
    print("=" * 60)
    ...
    strategy = LightGbmStrategy(
        ...
        label_tp_pct=8.0,
        label_sl_pct=4.0,
        label_timeout_minutes=TIMEOUT_MIN,
        fee_pct=0.1,
    ...
    report_dir = generate_iteration_reports(
        trades=results,
        iteration=f"{ITERATION}_dot_14d",
        features_dir="data/features",
        reports_dir="reports",
        interval="8h",
        n_trials=ITERATION,
