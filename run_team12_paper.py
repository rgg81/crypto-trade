#!/usr/bin/env python3
"""Team 12 exact-replay paper desk. This runner never places exchange orders."""
...
from crypto_trade.team12.backtest import load_frozen_snapshot
from crypto_trade.team12.live import persist_paper_tick, run_live_replay
from crypto_trade.team12.live_data import (
    BRIDGE_START,
    LiveMarketData,
...
def _assert_process_deployment(expected: DeploymentAuthority) -> None:
    ...
    if current != expected:
        raise DeploymentChangedError(
            "Team 12 deployment changed while this process was running; restart required"
        )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Team 12 paper-only exact replay (no exchange orders)"
    )
    ...
    parser.add_argument(
        ...
        help="paper output directory (default: paper-team12)",
    )
    parser.add_argument(
        "--no-refresh",
        action="store_true",
    ...
    *,
    ...
    if generation.exists():
        ...
        if sha256_file(generation / "cache-manifest.json") != sha256_file(
            ...
            raise RuntimeError(
                f"Team 12 cache generation collision: {generation}"
            )
...
def _paper_accounting_admissions(
    ...
    if not isinstance(payload, dict) or payload.get("status") != "PASS":
        raise RuntimeError("Team 12 prior paper integrity is not a PASS record")
    data = payload.get("data")
    if not isinstance(data, dict):
        raise RuntimeError("Team 12 prior paper diagnostics are missing")
    admissions = data.get("accounting_admission_boundaries")
    if admissions is not None:
        if not isinstance(admissions, dict):
            raise RuntimeError(
                "Team 12 prior accounting admissions are malformed"
            )
    ...
    if not isinstance(legacy_symbols, list) or not all(
        ...
        raise RuntimeError(
            "Team 12 legacy accounting-symbol evidence is malformed"
        )
    return {
...
def _integrity_bound_cache_generation(
    ...
    *,
    ...
    if not integrity_path.is_file():
        if required:
            raise FileNotFoundError(
                f"Team 12 paper integrity is missing: {integrity_path}"
            )
        return None
    payload = json.loads(integrity_path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict) or payload.get("status") != "PASS":
        raise RuntimeError("Team 12 paper integrity is not a PASS record")
    artifacts = payload.get("artifacts")
    binding = artifacts.get("cache_manifest") if isinstance(artifacts, dict) else None
    if not isinstance(binding, dict):
        raise RuntimeError("Team 12 integrity cache binding is malformed")
    ...
    if (
        ...
        raise RuntimeError("Team 12 integrity cache binding is unsafe")
    ...
    if (
        ...
        raise RuntimeError(
            "Team 12 integrity does not bind a cache generation manifest"
        )
    if not manifest.is_file():
        if required:
            raise FileNotFoundError(
                f"Team 12 integrity-bound cache is missing: {relative}"
            )
        return None
    if (
        ...
        raise RuntimeError("Team 12 integrity-bound cache manifest drift")
...
def _publish_committed_cache(paper_dir: Path, generation: Path) -> None:
    ...
    if bound is None or bound != generation.resolve():
        raise RuntimeError(
            "Team 12 paper integrity does not bind the prepared cache generation"
        )
    _write_text_atomic(
    ...
    if current != bound:
        raise RuntimeError(
            "Team 12 cache CURRENT does not match committed paper integrity"
        )
    _prune_directories(
...
def _reconcile_current_from_integrity(paper_dir: Path) -> None:
    ...
    if _current_cache_generation(cache_root, required=True) != bound:
        raise RuntimeError(
            "Team 12 could not reconcile CURRENT from paper integrity"
        )
...
def _current_cache_generation(
    ...
    *,
    ...
    if not pointer.is_file():
        if required:
            raise FileNotFoundError(f"Team 12 cache pointer is missing: {pointer}")
        return None
    ...
    if (
        ...
        raise RuntimeError("Team 12 cache CURRENT pointer is unsafe")
    generation = (cache_root / relative).resolve()
    if not generation.is_relative_to(cache_root.resolve()) or not generation.is_dir():
        raise RuntimeError("Team 12 cache CURRENT generation is missing")
    verify_live_cache_manifest(generation)
    return generation
...
def single_engine_lock(paper_dir: Path):
    ...
    with lock_path.open("w", encoding="utf-8") as handle:
        try:
            ...
            raise RuntimeError("another Team 12 paper engine is already running") from None
...
def _run_one(
    ...
    print(
        f"[team12-paper] boundary={boundary.isoformat()} refresh={not args.no_refresh} "
...
def _write_attempt(
    ...
    *,
    ...
    payload = {
        "schema_version": "team12-paper-attempt-v1",
        "status": status,
        "boundary": boundary.isoformat(),
