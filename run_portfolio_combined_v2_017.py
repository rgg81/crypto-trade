"""Combined v1+v2 portfolio analysis with BRAKED v2 (iter-v2/018).

Re-run of iter-v2/011's combined portfolio analysis, using the
iter-v2/017 baseline (hit-rate gate productionized) as the v2 input
instead of iter-v2/005.

Key question: does the braked v2 unlock a 50/50 or 60/40 blend that
beats iter-v2/011's original 70/30 recommendation? At iter-v2/011,
v2's 45% MaxDD made 50/50 too risky. iter-v2/017's braked v2 has
MaxDD 24.39% (similar to v1's 20.0%) — much better risk profile.

Inputs:
  - /home/roberto/crypto-trade/reports/iteration_152_min33_max200/
    (v1 iter-152 canonical baseline, unchanged)
  - reports-v2/iteration_v2-017/ (NEW: braked v2 baseline)

Usage:
    uv run python run_portfolio_combined_v2_017.py
"""
...
# iter-v2/018: point at the NEW braked v2 baseline from iter-v2/017
V2_REPORT = Path("reports-v2/iteration_v2-017")
OUT_DIR = Path("reports-v2/iteration_v2-018_combined_braked")
...
def main() -> None:
    ...
    summary = {
        ...
        "v2_oos": _summarize_trades(
            v2_oos, "v2 OOS (iter-v2/017 BRAKED, DOGE+SOL+XRP+NEAR, seed 42)"
        ),
        "combined_oos": _summarize_trades(
            combined_oos, "v1+v2 combined OOS (8 symbols, naive concat, braked v2)"
        ),
    ...
    print("\nv2 (iter-v2/017 braked) per-symbol OOS:")
