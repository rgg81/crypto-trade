...
|---|---|---|
| **3a — DSR/PSR per-regime + bundle** | BUNDLE: INFORMATIONAL. **SPECIALIST: SKIP** | flag-only; never BLOCK |
| **3b — PBO bundle-level** | BUNDLE: INFORMATIONAL. **SPECIALIST: SKIP** | flag-only; never BLOCK |
| **3c — Regime Attribution Clarity (NEW)** | **SPECIALIST + BUNDLE: MANDATORY** | reads §Item 0 table; PASS if regime decomposition is internally consistent (per-regime PnL sums to bundle PnL ±2%) AND bundle-role implications are non-vacuous; BLOCK-PENDING-FIX if table missing or rows incoherent |
| **3d — BUNDLE-level per-regime Pareto-dominance vs BASELINE_V1 (NEW)** | **BUNDLE-only**. SPECIALISTs EXEMPT | PASS iff: every tagged regime has `sharpe_R(cand) ≥ sharpe_R(base) − σ_R` AND `max_dd_R(cand) ≤ max_dd_R(base) + σ_dd_R` AND `trade_count_R(cand) ≥ 0.5 × trade_count_R(base)` AND ≥1 regime is strictly better. σ_R / σ_dd_R sourced from `baseline_seed_regime_matrix.csv`. Otherwise BLOCK |
...
For **SPECIALIST** iterations, Critic emits ONE band, decided by the following decision tree applied **to the regime decomposition** (NOT to bundle OOS Sharpe alone):
...
5. Bundle Sharpe Δ ≥ +σ_R, regime attribution not yet decomposable (rare; <1 month per regime) → **SPECIALIST-PROMISING**
6. Δ ≤ −σ_R in target regime(s) AND Δ < 0 in ≥half non-target → **TRUE-NEG** (== SPECIALIST-NEGATIVE)
...
For **BUNDLE** iterations: additionally evaluate Check 3d. If PASS → `BUNDLE-MERGE` or `BUNDLE-MERGE-ASSEMBLY`; if FAIL → `BUNDLE-BLOCK`.

Path Forward section MANDATORY on every BLOCK verdict (SPECIALIST-NEGATIVE / TRUE-NEG / NEGATIVE-no-effect / LEARNED-NEG / WALK-FORWARD-LEAKAGE / BUNDLE-BLOCK / BLOCK-PENDING-FIX / BLOCK-FINAL).
...
- [ ] Document the iteration's `/044` (or next BUNDLE) bundle composition role candidacy under each plausible regime — even if the verdict is REGIME-SPECIALIST-IS or TAIL-CONTROL, the diary records WHICH bundle slot this component fills.
...
## /044 substrate composition rule (BUNDLE-only — applies whenever a BUNDLE iteration runs)
...
These files are ONE-TIME bootstrap deliverables of the first BUNDLE; subsequent BUNDLEs update them.
...
Under the current SPECIALIST/BUNDLE methodology there is **no 10:1 cadence** between specialists and bundles. SPECIALISTs run as needed at the 2h wall-clock cap; a BUNDLE iteration is launched whenever the running specialist roster's IS regime-coverage justifies assembly (QR judgment, anchored against `briefs-v1/_meta/baseline_seed_regime_matrix.csv`). BUNDLE iterations run at the 9h wall-clock cap (per `feedback_v1_confirmation_walltime_9h`). The SPECIALIST roster catalog lives at `briefs-v1/specialist_catalog.md`.
...
4. **New Critic checks 3c (MANDATORY both SPECIALIST+BUNDLE) and 3d (BUNDLE-only)**. Checks 3a/3b reframed informational; SKIPPED at SPECIALIST budget.
...
- **/043 (new skill)**: LM Master MUST emit the Item-0 table FIRST with bundle-role implications per regime. Critic SKIPs Checks 3a/3b (SPECIALIST), RUNS Check 3c (regime clarity is BLOCK-eligible if table missing/incoherent), and EXEMPTS Check 3d (component, not bundle). Verdict resolves on the 9-band tree per-regime — modal `REGIME-SPECIALIST-IS` for the predicted /043 LINK-only outcome (where the OLD framework would have stamped `SPECIALIST-NEGATIVE` for the predicted Δ ≈ −0.52 vs /036 portfolio, killing a legitimate bundle component). DSR/PBO/PSR regressions noted, not blocking. Diary documents `/044` bundle role candidacy (bull-2025-08, recovery-2025-11 specialist) directly. The TAIL-CONTROL annotation hack disappears entirely — TAIL-CONTROL is now band #4 in the canonical tree with a precise definition (`max_dd_R ≤ baseline − σ_dd_R` in vol-spike/liq-cascade).
