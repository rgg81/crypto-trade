...
Cycle-7 SPECIALIST-MINE #7. BNBUSDT un-reserved per user directive 2026-06-10:
  ...
  GATE 0 (INFORMATIONAL): avg corr 0.676 vs {DOT, ETH, BTC, AAVE} — NOTED not blocking
  GATE 1 (INFORMATIONAL): trivial short-horizon +0.0755 ≤ +0.15 — NOTED not blocking
...
NO V1_ITER087 local feature additions. The feature set is exactly V1_FEATURE_COLUMNS_PRUNED.
One-variable discipline: the variable being tested is UNIVERSE (BNB whitelist),
and the fail-fast mechanism is the structure gate.

This is a thin dispatch wrapper around ``run_baseline_v1.main()``. It hardwires the
SPECIALIST-mode arguments for iter-v1/087:

    - Cohort: BNBUSDT only (single-symbol SPECIALIST per skill mandate)
    ...
    - atr_tp=2.9, atr_sl=1.45 (Model A ETH cell; vol-class match for BNB)
    - Aggregator: mean-of-signed-weights across 50 seeds
    - FAIL-FAST: fail_fast_is_years=2.0 enabled

UNCHANGED from /076//084//085//086 methodology:
...
NO PARQUET REGENERATION REQUIRED:
    Global V1_FEATURE_COLUMNS_PRUNED stays at 48 — existing BNBUSDT parquet is valid.
    The parquet exists at data/features/BNBUSDT_8h_features.parquet (48/48 cols verified).
...
Fail-fast semantics:
    ...
    evaluate cumulative weighted_pnl of all IS test trades. If ≤ 0:
      ...
      - Runner catches it, writes minimal report to reports-v1/iteration_v1-087/
...
Invocation:
    # Default SPECIALIST run (50 seeds × 30 trials, BNB only, fail-fast enabled):
    uv run python run_iteration_087.py

    # Verify features-base-hash only (dry-run, no backtest):
    uv run python run_iteration_087.py --check-hash

Output paths:
    reports-v1/iteration_v1-087/in_sample/
    reports-v1/iteration_v1-087/out_of_sample/
    reports-v1/iteration_v1-087/comparison.csv
    reports-v1/iteration_v1-087/in_sample/specialist_dispersion.csv
    reports-v1/iteration_v1-087/out_of_sample/specialist_dispersion.csv
    reports-v1/iteration_v1-087/optuna_best_params.csv
    reports-v1/iteration_v1-087/run.log
    reports-v1/iteration_v1-087/fail_fast_report.csv  (written if BLOCKED-FAIL-FAST)
"""
...
# ---------------------------------------------------------------------------
from crypto_trade.features_v1 import V1_FEATURE_COLUMNS_PRUNED, V1_ITER087_UNIVERSE
...
ITERATION_LABEL: str = "v1-087"
ITERATION_NUMBER: int = 87
...
def _verify_features_hash() -> str:
    ...
    iter-v1/087 uses the STOCK 48-col global PRUNED set WITHOUT any additions.
    ...
    if len(V1_FEATURE_COLUMNS_PRUNED) != 48:
        print(
            f"[iter-v1/087] GLOBAL-PRUNED-COUNT MISMATCH\n"
            f"  expected : 48 (global, unchanged)\n"
            f"  actual   : {len(V1_FEATURE_COLUMNS_PRUNED)}\n"
            "  V1_FEATURE_COLUMNS_PRUNED must stay at 48; "
            "iter-v1/087 adds ZERO new features (one-variable discipline).",
            file=sys.stderr,
        )
        sys.exit(
            "[iter-v1/087] ABORT: V1_FEATURE_COLUMNS_PRUNED must be 48 (global unchanged). "
    ...
    print(
        f"[iter-v1/087] features-base-hash: {actual_hash[:16]}... "
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "iter-v1/087 SPECIALIST runner: "
            f"BNBUSDT 50-seed independent-Optuna bagging, STOCK 48-col stack, "
            ...
            f"specialist_mode=True; cycle-7 BNB SPECIALIST)"
        )
    )
    p.add_argument(
        "--check-hash",
...
def main() -> None:
    ...
    if args.check_hash:
        ...
        print("  cohort                    : BNBUSDT only (V1_ITER087_UNIVERSE)")
        print("  model                     : Model_A_BNB_specialist_087")
        ...
        print(f"  V1_ITER087_UNIVERSE       : {V1_ITER087_UNIVERSE}")
    ...
    assert len(V1_FEATURE_COLUMNS_PRUNED) == 48, (
        f"[iter-v1/087] PRE-FLIGHT FAIL: global V1_FEATURE_COLUMNS_PRUNED expected 48, "
        f"got {len(V1_FEATURE_COLUMNS_PRUNED)}. "
        "The global pruned set must stay at 48; iter-v1/087 adds ZERO new features."
    )
    assert V1_ITER087_UNIVERSE == ("BNBUSDT",), (
        f"[iter-v1/087] PRE-FLIGHT FAIL: V1_ITER087_UNIVERSE must be ('BNBUSDT',); "
        f"got {V1_ITER087_UNIVERSE}."
    )
    assert len(V1_SPECIALIST_SEEDS) == V1_SPECIALIST_SEED_COUNT, (
        f"[iter-v1/087] PRE-FLIGHT FAIL: V1_SPECIALIST_SEEDS length "
        f"{len(V1_SPECIALIST_SEEDS)} != V1_SPECIALIST_SEED_COUNT {V1_SPECIALIST_SEED_COUNT}."
    )
    assert V1_SPECIALIST_SEEDS[0] == 42, (
        f"[iter-v1/087] PRE-FLIGHT FAIL: V1_SPECIALIST_SEEDS[0] must be 42; "
        f"got {V1_SPECIALIST_SEEDS[0]}."
    )
    assert V1_SPECIALIST_SEEDS[-1] == 91, (
        f"[iter-v1/087] PRE-FLIGHT FAIL: V1_SPECIALIST_SEEDS[-1] must be 91; "
        f"got {V1_SPECIALIST_SEEDS[-1]}."
    )

    print("=" * 70)
    print("iter-v1/087 — BNBUSDT SPECIALIST (STOCK 48-col stack, fail-fast gate)")
    print(f"  ITERATION_LABEL            : {ITERATION_LABEL}")
    print("  specialist_mode            : True")
    print(f"  V1_SPECIALIST_SEED_COUNT   : {V1_SPECIALIST_SEED_COUNT}")
    print(f"  V1_SPECIALIST_OPTUNA_TRIALS: {V1_SPECIALIST_OPTUNA_TRIALS}")
    ...
    print("  atr_tp=2.9  atr_sl=1.45 (Model A ETH cell; vol-class match BNB)")
    print("  aggregator                 : mean-of-signed-weights across 50 seeds")
    ...
    print(f"[iter-v1/087] sys.argv set: {sys.argv}")
