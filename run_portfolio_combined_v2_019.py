"""Combined v1+v2 portfolio analysis with iter-v2/019 baseline (iter-v2/020).

Third-generation combined analysis. Previous:
- iter-v2/011: used iter-v2/005 (unbraked v2). Recommended 70/30.
- iter-v2/018: used iter-v2/017 (hit-rate gate). Unlocked 50/50.
- iter-v2/020: uses iter-v2/019 (BTC filter + hit-rate gate).

The iter-v2/019 baseline has:
- Hit-rate feedback gate (OOS only)
- BTC trend filter (full period, ±20% 14d)

Inputs:
  - /home/roberto/crypto-trade/reports/iteration_152_min33_max200/
    (v1 iter-152 canonical baseline, unchanged)
  - reports-v2/iteration_v2-019/ (v2 with both risk gates)

Usage:
    uv run python run_portfolio_combined_v2_019.py
"""
...
V2_REPORT = Path("reports-v2/iteration_v2-019")
OUT_DIR = Path("reports-v2/iteration_v2-020_combined_v019")
...
def main() -> None:
    ...
    summary = {
        ...
        "v2_oos": _summarize_trades(
            v2_oos, "v2 OOS (iter-v2/019 BTC+HR gates, DOGE+SOL+XRP+NEAR, seed 42)"
        ),
    ...
    print("\nv2 (iter-v2/019 braked) per-symbol OOS:")
